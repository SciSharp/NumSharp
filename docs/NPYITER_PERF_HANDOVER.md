...
| **Roadmap Wave 3 — strided roofline (Phase 2a shipped)** | (2026-06-10) | **DONE.** The Tier-3B shell's scalar-strided hole is closed: gather dispatcher + `EmitSimdGatherLoop` (per-input AVX2 vgather with hoisted byte-offset index vectors, runtime `\|stride\| ≤ int.MaxValue/8` guard, stride-0/negative valid, 4× unroll + remainder + scalar tail, contig-store + per-lane scatter-store variants, i32/u32/f32/i64/u64/f64 at V256+AVX2) reusing the caller's vectorBody; gather section in the 1-D strided flat reduction (4 accumulators, shared identity/combine/horizontal helpers → identical NaN semantics); `GATHER_ELIGIBLE` wired in `UpdateContiguityFlags`. **Kernel-level: strided add 318 µs (POC 334), strided 2-D sqrt 201 µs (POC 203); production e2e same-run vs NumPy: S1 parity (1080/1033), S2 1.36× faster (941/1282), S3 2.2× faster (117/259); preallocated-out iterator route 354 µs — residual e2e gap = Wave-2.4 allocator tax.** **Bonus bug found & fixed (§4.13): the NumSharp extension flags ALIASED shifted NumPy flags — `GATHER_ELIGIBLE==ONEITERATION` made ForEach run one inner loop and silently skip every later row; `CONTIGUOUS==GROWINNER` was latent. Renumbered to bits 3–6 + alias-freedom test.** **Suite: 9501 pass (11 new tests in `StridedGatherKernelTests.cs`).** |
| **Roadmap Wave 1.1 — COPY_IF_OVERLAP** | (2026-06-10) | **DONE.** Overlapping write/read operands no longer corrupt: full `mem_overlap.c` port (`NpyMemOverlap.cs` — extent fast path + GCD-pruned bounded-Diophantine DFS, Int128 intermediates, maxWork 0/-1/N) + FORCECOPY/write-back-on-Dispose in `NpyIter.cs` (nditer_constr.c:3083-3311 parity incl. OVERLAP_ASSUME_ELEMENTWISE exact-alias short-circuit and internal-overlap check); flags wired into production binary/unary/compare routes (NumPy ufunc parity). Solver 14/14 vs `np.shares_memory`/`may_share_memory`; ufunc behaviors B1–B7 NumPy-identical. **Bonus bug found & guarded:** Layer-2 typed helpers (`ExecuteBinary`/`ExecuteUnary`/`ExecuteComparison`/`ExecuteScan`) ignore output strides (whole-array-kernel contract) — strided write operands were silently written contiguously; now throw with guidance to the per-chunk routes (real fix = Phase 3). Write-back resolves at `Dispose` (WRITEBACKIFCOPY semantics) — consume results after the `using` scope. **Suite: 9490 pass (13 new tests), no perf regression.** |

| **Wave 4: windowed buffered iteration + DELAY_BUFALLOC + bug (b)** | this session | Buffered NON-REDUCE iteration was broken for IterSize > BufferSize (no buffered iternext existed — one eager fill at construction, then plain advancers: silent truncation; the bridge avoided it by construction). Now NumPy-shaped: `BufferedNext` (EXLOOP: flush→jump→refill) + `BufferedElementNext` (per-element nditer protocol), row-aligned fills via SIMD IL cast kernels, NumPy buffer-when-required criterion (linear operands keep true strides), DELAY_BUFALLOC, flush on Dispose/Reset/single-fill. **Bug (b) FIXED** (5 sites: SrcElementSizes multiplier). Plus two latent bugs: `NpyIterRef.GetDataPtr` partial-window recompute (stale reads on the final window), `IsSingleInnerLoop` EXLOOP shortcut dropping windows 2+. **A/B verdict: buffered-cast→SIMD loses to fused IL converts for cheap ops (kept fused: binary/compare) and wins for promoting unary math (sqrt(i32) 1.36×, now routed buffered).** vs NumPy: mixed add 0.79×, sqrt(i32) 0.81×, strided mixed 1.00×. Suite 9513 (12 new multi-fill gates). |

| **Wave 1.4: hygiene — size-1 stride invariant, flag wiring, dead code** | (2026-06-10) | The hygiene audit found three real bugs (each reproduced vs NumPy 2.4.2 before fixing): **(a) the missing `fill_axisdata` invariant** — NumPy forces stride 0 for every operand on any size-1 iterator axis and on broadcast-stretched dims (nditer_constr.c:1594-1615); NumSharp copied raw operand strides, so its relaxed coalesce condition + NumPy's merge rule could keep a size-1 axis' arbitrary stride — `RemoveMultiIndex` on a (1,4) view with element strides (1,2) iterated [0,1,2,3] where NumPy reads [0,2,4,6]. Fixed at the root: normalization at fill (standard + op_axes paths) and the coalesce condition reverted to NumPy's strict form; contiguous size-1 shapes still fully coalesce. **(b) the op_axes fill walked out of bounds** for an operand size-1 axis stretched to a larger iter dim (raw array stride instead of 0); ApplyOpAxes now also reduce-validates stretched dims like op_axis=-1. **(c) bug-(b) family sites #6/#7:** `FlipNegativeStrides` and `GetAxisStrideArray` multiplied source element strides by the BUFFER dtype size — K-order + BUFFERED int32→f64 over negative strides read garbage; now `SrcElementSizes` (production unary was immune: it forces C/F order so the flip never ran). Plus: NumPy write-broadcast validation (stretched WRITE dim requires REDUCE_OK + readable, REDUCE flags set — W1/W3/W7 parity); CA2014 stackalloc hoisted; dead `CalculateGrowInnerSize` (latent expectedStride-not-reset-per-operand bug) / `PrepareBuffers` / `FinalizeBuffers` deleted; `EARLY_EXIT` flag deleted (early exit is a kernel property), `PARALLEL_SAFE` wired for free off COPY_IF_OVERLAP's work (`IsParallelSafe`, ready for 6.2). **Review addendum:** the normalization made `GetMinStride` 0 for size-1 axes — the descending K-sort sank them innermost (fixed: key-0 → outermost in descending mode), and the review exposed a PRE-EXISTING pathology: forced-C/F non-coalesced iterators kept trailing size-1 axes innermost (NumSharp gates coalescing on all-contiguous; NumPy coalesces unconditionally) → (N,1)-strided ufuncs ran N one-element inner loops. Fixed via `RemoveUnitAxes` on the non-index-tracked branch: **(4M,1) strided f32: add 23.4→4.2 ms (5.5×), sqrt 20.2→4.1 ms (4.9×), interleaved best-of-3 vs 33058b83.** **Suite 9530 (17 new tests in `NpyIterSizeOneStrideTests.cs`); interleaved A/B vs the pre-wave tree perf-neutral on the grid (P15 1.309 vs 1.334 µs).** |

| **Wave 1.3: WRITEMASKED/ARRAYMASK execution + VIRTUAL** | (2026-06-10) | NumPy 2.4.2 probing settled the architecture before any code: **masking is enforced in exactly ONE place — the buffered copy-back** (nditer_api.c:2001-2026). Unbuffered WRITEMASKED operands write the array directly (kernel contract), and 2.x BUFNEVER means `'buffered'` + contiguous same-dtype operands ALSO skip enforcement — so the execution drivers needed NO changes; the flush carries everything. Shipped: masked copy-back (`FlushBufferWindow` → `CopyWindowFromBufferMasked` → `CopyRunFromBufferMasked`; mask cursor rides the flat window counter; mask from its buffer when buffered else its window-start array ptr — the BUFNEVER switch; TRUE-stretch decomposition keeps memcpy/SIMD-cast kernels, NumPy `_strided_masked_wrapper` structure); full validation parity with NumPy texts verbatim (WRITEMASKED-requires-WRITE, VIRTUAL-requires-READWRITE incl. the doubled "be", null⇄ALLOCATE∥VIRTUAL, ARRAYMASK-can't-reduce on both stride paths, deferred WRITEMASKED∧REDUCE mask-broadcast check matching NumPy's deferred loop at the tail of npyiter_allocate_arrays — the old inline check missed the standard broadcast path, `"Only bool and uint8 masks are supported."` when a WRITEMASKED op actually buffers); **VIRTUAL = NumPy 2.x allocate-equivalent** (source + probes: real array allocated for every null op, NEP-12 never landed, dtype request DISCARDED → common dtype, ARRAYMASK default bool, virtual+allocate → ALLOCATE wins). Buffered-REDUCE + WRITEMASKED: constructs (NumPy parity) but the legacy reduce write-back refuses loudly → Wave 5. **Perf:** expanded validation cost +11.9 ns/ctor on small-N (caught by the new targeted `benchmark/poc/ctor_probe.cs`, interleaved vs 7c8e0588) — eliminated via an OR-sweep fast-path gate; final **181.1 vs 181.2 ns/ctor, dead even**; S-rows at baseline. **Suite 9554 (24 new tests in `NpyIterWriteMaskedExecutionTests.cs`).** |

| **Wave 2: out=/where= + allocator (2.1 ✓ 5962a5e1, 2.2 partial, 2.4 ✓)** | (2026-06-10) | **2.1**: ufunc `out=`/`where=` across the elementwise np.* core (8 binary + 10 unary), all semantics probed vs NumPy 2.4.2 and pinned verbatim (19 tests) — out joins the broadcast without stretching, loop dtype from inputs + same_kind out validation with ufunc-name error texts, where = trailing ARRAYMASK + WRITEMASKED out + masked inner loop in ForEach (mask-TRUE run decomposition, NumPy execute_ufunc_loop(masked=1) structure); dtype-mismatched out rides the Wave-4 windowed cast flush. Bonus bug: ResolveInnerLoopCount read Shape[-1] for 0-d EXLOOP (AV — first reachable when out= routed scalars through ForEach). **2.4**: SizeBucketedBufferPool window 1 B–64 MiB (was 4 KiB–1 MiB: the 4000-byte small-N result missed the floor by 96 B; every 4M-element output missed the cap), large buckets cap 2; pool-owned GC pressure tracking the LIVE state (Add@Take/Remove@Return — NOT residency); SuppressFinalize(Disposer) once freed. **In-place toggle-verified ~2× on every allocation-heavy row: P1 3.37→1.74 ms, S2 790→432 µs, P9 6.4→3.4 ms; np.add(a,b,out)=446 ns vs 834 allocating.** **2.2 partial**: call-invariant flag arrays static at all 7 iterator sites; operand arrays stay per-call (nested-iterator aliasing via np.copyto in the overlap path). **2.3 open** — profiling says the P15 lever is the finalizer lifecycle (~500 ns of the 804 ns allocation: ~NDArray + Disposer registration/queue churn), NOT construction (177 ns); ~NDArray is pinned by ArcLifecycleTests (captured-slice reclamation). Suite 9573. |

| **Wave 6.1: fusion exposed — np.evaluate + NumPy result_type trees + fused reductions** | (2026-06-10) | The proven-but-unreachable fusion path (`ExecuteExpression`, POC 2.8–5.4×) is now a public API: **`np.evaluate(expr[, operands][, out])`** — ArrayNode leaves with implicit NDArray/scalar conversions (`(NpyExpr)a * b + 2`), reference-deduplicated binding ((a-b)/(a+b) = 3 operands), EXTERNAL_LOOP\|COPY_IF_OVERLAP, out= via same_kind validation + the Wave-4 windowed cast flush, overlap-safe aliasing. **The DSL's "compute everything at the output dtype" contract was NumPy-wrong for mixed trees** — `NpyExpr.Typing.cs` adds a per-node NEP50 result_type pass (probed vs 2.4.2: strong-strong incl. the int/float tier crossing i4+f4→f8; weak literals i4+2→i4 / f2+2.5→f2 / bool+2→i64 with the exact OverflowError; true_divide ints→f64; arctan2 tier floats; power/remainder/floor_divide bool→i8 + the literal negative-exponent ValueError; unary float tiers; bool add=OR / multiply=AND; invert/bitwise-float TypeErrors) and emission converts at node edges — pinned by the (i4*i4)+f8 int32-wraparound test; legacy `Compile()` byte-preserved (null typing table → OutputType everywhere). **Reduction roots** (Sum/Prod/Min/Max/Mean) compile a one-pass raw 4-accumulator kernel with a host aux slot (carry-in across EXLOOP chunks): sum(a*b) never materializes a*b; NumPy reduce dtypes; f16/f32 accumulate in f64 (cast back — more accurate than pairwise, documented); min/max NaN-propagate, And/Or for bool, double-roundtrip for Half; empty: sum=0/prod=1/mean=NaN/min-max raise NumPy's text. `ExecuteExpression` throws without EXTERNAL_LOOP unless single-chunk (~40× foot-gun guard). **Measured (Release, best-of-9, 4M f64 vs NumPy 2.4.2 same box): a*b+c 4.13 vs 13.23 ms (3.2×), (a-b)/(a+b) 3.24 vs 19.65 (6.1×), sum(a*b) 2.45 vs 8.72 (3.6×), i4*2+f8 2.94 vs 10.21 (3.5×); 1.2–4× over unfused NumSharp. Grid + ctor probe (183 ns) neutral — zero production-route files touched. Suite 9596 (23 new in `NpyEvaluateTests.cs`). Found-unfixed: `NpyIterCasting.PromoteTypes` NEP50 divergence ("float always wins" → i4+f4→f4; NumPy f8) — COMMON_DTYPE/VIRTUAL-common paths only, recorded in the roadmap matrix.** |

~~**Phase 0 leftover — bug (b), deferred (test-first)**~~ ✅ **RESOLVED (Wave 4, this session).** All five array-traversal sites (`Advance`, `GotoIterIndex`, `ExternalLoopNext`, `GotoIndex`, `GotoMultiIndex`) now multiply source-element strides by `SrcElementSizes` — `ElementSizes` is the buffer dtype size and diverged under a buffered cast. Gated by the multi-fill (>8192) tests in `NpyIterBufferedWindowTests.cs` (per-element + window-driver + strided + 2-D variants).
...
✅ b   Buffered-cast Advance (test-first)   DONE with Wave 4 — SrcElementSizes at 5 sites + multi-fill gates
```
...
12. **THE BENCHMARK-INVALIDATOR (found 2026-06-10): `dotnet run` file-based scripts build DEBUG by default — the script assembly *and* the `#:project`-referenced NumSharp.Core** (`DebuggableAttribute(DisableOptimizations)` = flags 263; the JIT honors it even over `[MethodImpl(AggressiveOptimization)]`). Measured: hand-written C# hot loops ~2× slower (POC C 789→319 µs, D 470→206, E 368→109); `MultiNew` construction ~40% slower (H 0.69→0.40 µs/call). `DynamicMethod`-emitted kernels are **immune** (emitted IL is always JIT-optimized) — which is why contiguous/IL-kernel/fusion numbers always looked fine while hand-written strided kernels looked mysteriously 2× slow, and why the cause survived scheduling-, GC-, stride-codegen- and alignment-elimination. `#:property Optimize=true` fixes only the script (Core stays Debug); `#:property Configuration=Release` changes output paths but binaries stay unoptimized; **only command-line `dotnet run -c Release` optimizes both**. Verify with `IsJITOptimizerDisabled` on both assemblies — the POC asserts it at startup.
    **Corrections this forces onto earlier conclusions:**
    ...
    - **Related runfile gotcha (Wave 1.4):** the file-based-app build cache does NOT invalidate when only `#:project`-referenced sources change — after editing NumSharp.Core, re-running an unchanged script executes the STALE Core build (fixes appear "not to work"). Touch the script (any byte) to force a rebuild.

13. **Flag-collision bug class (found & fixed in Wave 3): enum members that alias other members corrupt silently and survive every test that doesn't combine them.** The four NumSharp-extension `NpyIterFlags` (0x10000–0x80000) ALIASED the NumPy-parity flags shifted to bits 8–20: `CONTIGUOUS==GROWINNER`, `GATHER_ELIGIBLE==ONEITERATION`, `EARLY_EXIT==DELAYBUF`, `PARALLEL_SAFE==REDUCE`. Setting CONTIGUOUS had silently set GROWINNER since the extensions were introduced (latent — GROWINNER only matters with BUFFER); the first time GATHER_ELIGIBLE was actually set, `IsSingleInnerLoop()` saw ONEITERATION and `ForEach` executed ONE inner loop — 2-D iterations computed row 0 and silently skipped the rest (S2 "completed" in 10 µs with garbage rows). Renumbered to genuinely free bits 3–6; `NpyIterFlags_ExtensionFlags_DoNotAliasNumPyFlags` asserts pairwise alias-freedom over the whole enum so the class can't recur. Lesson: when an enum mixes value families (legacy bits, shifted ports, extensions), add an alias-freedom test the moment the third family appears.

14. **Windowed-buffering lessons (Wave 4).** (a) **Buffered non-reduce iteration never worked for IterSize > BufferSize** — there was no buffered iternext; everything beyond the first 8192-element fill was silently skipped (the §2 bridge avoided it by construction). Diagnosis pattern for "works small, garbage past 8192": missing refill machinery. (b) **NumPy's buffered protocol is TWO protocols**: with EXTERNAL_LOOP, iternext jumps a whole fill; without it, iternext presents ONE element at a time walking the buffer (nditer_templ.c.src) — kernel drivers must window-advance regardless (per-fill kernels + per-element advance = corruption). (c) **`NpyIterRef.GetDataPtr` vs `NpyIterState.GetDataPtr` divergence bug class**: the ref-level method recomputed the buffer position as `IterIndex − (BufIterEnd − BufferSize)`, correct only for FULL windows — the final partial window read stale data from the previous fill. DataPtrs (maintained by fill/advance) is the single source of truth; never recompute. (d) **The buffered-cast→SIMD architecture is NOT automatically faster**: A/B on i9-13900K/Release — cheap binary ops LOSE (~16 B/element of extra L2 round-trip > SIMD gain; add 2.20 vs fused 1.49 ms/2M) while compute-heavy promoting unary WINS (sqrt 1.65 vs 2.25 ms). NumPy buffers because AOT C loops cannot fuse casts; runtime IL can fuse — measure per op-class before adopting NumPy's config wholesale. (e) NumPy's `buffered` flag buffers **only when required** (cast/CONTIG/non-linear) — over-buffering linear strided operands wastes copies and defeats Wave-3 gather kernels.

15. **Port NumPy's INVARIANTS, not just its logic (Wave 1.4).** NumPy's coalesce/merge rules are only sound because `fill_axisdata` guarantees a fill-time invariant: every size-1 iterator axis (and every broadcast-stretched dim) carries stride 0 for every operand. NumSharp had ported the rules but not the invariant, then "fixed" the resulting (2,4,1)-class coalesce failures by RELAXING the condition — which made the merge rule silently wrong instead (a size-1 axis' arbitrary stride could win the merge). The corruption survived 9513 tests because the constructor's all-contiguous guard masked it everywhere except `RemoveMultiIndex`. Corollary for reviews: any site multiplying a state stride by an element size must use `SrcElementSizes` (sites #6 `FlipNegativeStrides` and #7 `GetAxisStrideArray` of the bug-(b) family were found in this sweep — the family keeps paying; grep for `ElementSizes[` when touching traversal code).

16. **Probe WHERE a semantic lives before porting it (Wave 1.3).** The roadmap assumed WRITEMASKED needed "a masked write contract in ForEach/Tier-3B" — probing NumPy 2.4.2 first showed masking is purely a **copy-back property of the buffer flush**, never an iteration/driver property: unbuffered WRITEMASKED writes the array directly (kernel contract), and 2.x BUFNEVER means even `'buffered'` + contiguous same-dtype operands skip enforcement (`'buffered'` is a permission, not a promise). Result: the entire feature landed in the flush + construction validation, zero driver changes, and the W1/W2 NOT-enforced cases are pinned as parity tests. Same wave, second instance of the pattern: VIRTUAL's documented "buffer-only, no backing array" semantics (NEP-12) never landed — `npyiter_allocate_arrays` allocates for every null op and the requested dtype is DISCARDED (`*op_dtype = NULL` on the non-ALLOCATE branch); only source + probes revealed it. Also: small-N construction is now guarded by `benchmark/poc/ctor_probe.cs` — per-op validation added to `Initialize` shows up there at the nanosecond scale (this wave's +11.9 ns was caught and erased with an OR-sweep fast-path gate before commit); run it interleaved against a worktree at the base SHA, never against stale baselines.

17. **Measurement traps, Wave-2 edition — three found in one afternoon.** (a) **The runfile stale-build trap struck TWICE inside one bisect**: `dotnet run -c Release --no-build` after editing Core sources (or after `git stash`!) runs the PREVIOUS build — both "the pool regressed everything" and "the revert didn't help" were artifacts of measuring a stale binary. Protocol: after ANY source change (including stash/checkout), `touch` the probe AND run once WITHOUT `--no-build` before measuring. (b) **Worktree directories are not environmentally equivalent**: the `NumSharp-wt-w2` worktree measured ~35% faster than the main repo on memory-heavy rows with byte-identical code (Defender real-time scanning / indexer / filesystem state differ per directory). Cross-worktree A/B lied; the trusted protocol is IN-PLACE stash-toggle A/B: same directory, same binary cache path, flip the working tree, rebuild, measure, flip back, confirm both directions. (c) **GC.AddMemoryPressure must track LIVE buffers, not pool residency**: registering pressure for idle pooled buffers told the GC ~100–200 MB was in use and drove near-constant gen2 collections — every row degraded 30–50% until pressure moved to Add-on-Take/Remove-on-Return. Pressure is a GC-scheduling signal about reclaimable-by-collection memory, and pooled-idle buffers are not reclaimable by collection.

18. **NumPy result_type is a PER-NODE property, and C# operator resolution can silently break it (Wave 6.1).** (a) A fused expression matches NumPy bit-for-bit only if every node computes at ITS OWN result_type — `(i4*i4)+f8` must wrap the multiply in int32 (1410065408) before promoting; computing the whole tree at the output dtype (the old DSL contract, "mirrors NumPy" per its own header) gives 1e10. The probe matrix is the spec: divide≠atan2 on the same int inputs (flat f64 vs tier float), power(bool,bool)→i8, weak literals adopt-with-overflow-check. (b) **Implicit-conversion betterness silently downgraded NEP50 weak literals**: with both `NDArray→NpyExpr` and numeric→NDArray implicit conversions in play, `expr + 2.5` bound to the `(NpyExpr, NDArray)` operator (NDArray is the "better" target because NDArray→NpyExpr exists, C# §12.6.4) — the literal became a strong scalar array and f4+2.5 promoted to f8 instead of staying f4. Exact-match scalar operator overloads are REQUIRED, not sugar; only dtype-pinned tests catch this class (values stay plausible). (c) Third strike of the runfile cache (§17a): the "failing" weak-scalar fixes were a stale Core build — touch the script first, always.
