...
1. ✅ **SHIPPED** — **Bitwise first** (`&|^` + np.bitwise_* creation + no-loop guards; 25 tests in `UfuncBitwiseOutWhereTests.cs`, suite 9641/0, ctor probe 171.7 ns, grid neutral. Note: the TensorEngine bitwise abstracts rode in one commit earlier entangled with the dtype= follow-up 6a566e49). Near-zero engine work — validates that the Wave-2.1 binary branch generalizes beyond arithmetic with no op-gating surprises, and closes the CS0117 hole. Files: `TensorEngine.cs` (3 sigs), `DefaultEngine.BitwiseOp.cs`, new `Math/np.bitwise.cs`, `UfuncOut.cs` (no-loop helper), tests (~18). **Smallest possible diff, highest doc-mismatch payoff.**
2. ✅ **SHIPPED** — **Unary batch** (22 sigs + invert + arctan2 + positive/sign bool guards; 29 tests in `UfuncUnaryBatchOutWhereTests.cs`, suite 9670/0 net10.0, net8.0 failure set shrank 5→4 — the Round/Truncate Vector BCL gap is now gated, ctor probe 174.0 ns, grid neutral). Deltas vs this plan, post-6a566e49: np.* follows the merged-overload convention (ONE NumPy-shaped `f(x, out, where, dtype)` + demoted legacy positional-dtype overloads) instead of required-out; dtype= loop-signature semantics landed with the batch (input-cast errors verbatim, float-only no-loop raises, invert dtype matrix); round_/around take out= only (np.round is NOT a ufunc — probed). Bonus fixes: f16-tier emit for sinh/cosh/tanh/arcsin/arccos/arctan/round/deg2rad/rad2deg on Half (was NotSupported — sinh(i1) threw), log2/log10/log1p `(x, Type)` dtype-drop bug, reciprocal 1/0→MinValue + strided-view crash (§2.10 defects), round(i4, d≥0) identity path.) Mechanical threading ×22 override files + np.* overloads ×22 files (one per existing np.<op>.cs) + the §4.3.2 identity-emit arm + §4.3.3 compositions + `UfuncName` additions + the ATan2 Into hookup. Tests ~31. Big-but-shallow; every op rides the proven `ExecuteUnaryUfuncInto`.
3. ✅ **SHIPPED** — **Comparisons + predicates** (`ExecuteComparisonUfuncInto` reusing the binary flag arrays + shared `npy_cmp_*` kernels; return-type option (i) as designed — additive plain-`NDArray` overloads, engine `@out` required-but-nullable; predicates ride the unary Into-path with a Boolean loop dtype and no `MakeGeneric`; 30 tests in `UfuncComparisonOutWhereTests.cs`, suite 9700/0 net10.0, net8.0 spot 30/30, ctor probe 178.3 ns, grid neutral by in-place stash-toggle A/B — ambient load inflated absolutes during the gate, both directions confirmed identical). Tests ~38. Last because it carries the one design decision and the most scenario rows; by then slices 1-2 have re-validated every shared mechanism.
