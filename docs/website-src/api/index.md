...
| Type | Description |
|------|-------------|
| @NumSharp.Backends.Iteration.NpyIterRef | NumPy-aligned multi-operand iterator (broadcasting, buffering, casting, reductions); drives every kernel |
| @NumSharp.Backends.Iteration.NpyFlatIterator | Flat C-order element iterator; backs `np.broadcast(...).iters` |
