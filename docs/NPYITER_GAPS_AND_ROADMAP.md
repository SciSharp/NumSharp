...
**C. Per-operand (8):** aliased/overlapping ✅ **fixed (Wave 1.1: COPY_IF_OVERLAP + write-back)** · **in-place `out=` ❌ no API** · REDUCE operand ✅ (Wave 1.4 adds NumPy's stretched-write validation: REDUCE_OK + readable required, REDUCE flags set) · **WRITEMASKED ❌ exec missing** · **VIRTUAL ❌ stub** · buffered/cast ✅ (Wave 4 windowed machinery; Advance bug (b) fixed Wave 1.2, remaining family sites fixed Wave 1.4) · read-only ✅.
