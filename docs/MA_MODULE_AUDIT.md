...
>
> **Progress — 2026-09-14 (pass 13 — a 6055-case oracle differential to PROVE "byte-perfect"; found + fixed 3 real
> bugs the pass-12 "527/527" claim missed, and established the honest byte-exact boundary).** Built a far larger
> differential than pass 12: the full implemented op surface × ALL 13 NumPy dtypes × 7 memory layouts
> {C,F,transposed,reversed,strided,1-D-row,offset} × edge masks (6055 deterministic cases), NumPy 2.4.2 as oracle,
> layout-independent per-element bit-token comparison (NaN canonicalized as documented non-contractual). It did NOT
> confirm byte-perfection — it surfaced **three genuine bugs**, all now fixed: **(1) a CLR memory-corruption CRASH**
> — `np.arctan2(complex, complex)` (a float-only ufunc) had no complex-input guard, so complex operands fell through
> to `ExecuteATan2Kernel` and corrupted the heap (fatal `ExecutionEngineException`); this is a CORE fix
> (`Backends/Default/Math/Default.ATan2.cs`), now throwing NumPy's verbatim `TypeError` like `hypot`/`floor`/`ceil`
> already did. (All the confusing "cumulative churn crashes" during the run were THIS bug surfacing
> non-deterministically at a later allocation.) **(2) a dtype REGRESSION introduced by pass 12** — `PromoteMaScalar`
> was applied blanket to every `Binary` op, but NumPy's `ma.left_shift`/`right_shift` are plain functions (NOT
> `_MaskedBinaryOperation`) and do NOT widen a bare shift-count scalar, so `ma.left_shift(int16, 1)` must stay int16
> (pass 12 wrongly made it int64, and `left_shift(uint64,1)` even threw); fixed by a `promoteScalar:false` flag on
> the shift callers. **(3) a dtype divergence** — masked `median` along an axis of a 1-D narrow-float array returned
> float64 (the 0-D-weak `/2` widened it) where NumPy preserves float16/float32; fixed by a bit-exact dtype restore
> (dividing a float by 2 is exact). After the fixes the differential is **5961/6055 byte-exact + 39 correct-error +
> 4 correct-masked-singleton = 6004 fully correct**; the remaining **51 are all ~1 ULP allclose, principled
> divergences** — 50 `var`/`std` (float64 accumulation-order 1 ULP, larger for float16 = the documented
> prefer-precise class, now shown to span all dtypes not just float16) and 1 complex `sinh` (the documented
> ≤3-ULP complex-unary envelope). **HONEST VERDICT: the ma module is NOT literally 100% byte-perfect** — `var`/`std`
> and the complex-unary transcendentals diverge by ~1 ULP (allclose, order-dependent / prefer-precise, documented
> §5.6/§5.8), so a strict byte comparison of those specific ops fails while every value stays correct. It IS
> byte-exact for the overwhelming majority (5961/6055 ≈ 98.4 % of this adversarial differential; 100 % of the
> integer/bool/manip/selection/set/product families), and the audit's own strict standard ("nothing is done until
> bit-exact") is met for all but the accumulation-order/transcendental-ULP cases. Gated by **3 more
> `MaskedArrayTests` (90 total, green net8.0/net10.0)** + 27 core arctan2 tests (no regression). **This pass is the
> counter-example to over-confident "N/N bit-exact" claims: a bigger, more diverse oracle found a crash, a
> regression, and a dtype bug that a smaller one certified clean.**
...
|---|---|---|---|
| `np.ma.*` module names (`__all__`) | **218** (reflected, pass-12 verified) | 226 | **8 names** — NONE a functional gap: 3 are C# types that exist (`MAError`/`MaskError`/`MaskedArray`), 2 are N/A submodule refs (`core`/`extras`), 3 need structured dtypes (`mvoid`/`fromflex`/`flatten_structured_array`). (`polyfit` CLOSED pass 7; `flatten_mask`/`make_mask_descr`/`mr_` CLOSED pass 10; **`masked_print_option` CLOSED pass 11**; `set_fill_value` is PRESENT — the pass-11/12 "missing" flag was a reflection-filter false positive, verified pass 12.) |
| `MaskedArray` instance members | **77** (+ the indexer; reflected, pass-12 verified) | 94 | **17 members** — ALL infra/policy-blocked, exhaustively: structured/record `getfield`/`setfield`/`torecords`/`toflex`; the `get_fill_value`/`set_fill_value` METHOD spellings (CS0082 — they exist as the `fill_value` property's accessors, confirmed `[Method special]` by reflection); `get_real`/`get_imag` (covered by the `real`/`imag` properties); `sharedmask` (unmodelable); the ndarray-infra mirrors with no masked semantics (`base`/`ctypes`/`flags`/`setflags`), pickle (`dump`/`dumps`) and array-API device (`device`/`to_device`) — all N/A. (Pass 9 added `tolist`/`tobytes`/`tofile`/`view`/`ids`/`iscontiguous`/`putmask`/`resize`/`flat`/`recordmask`/`baseclass`/`unshare_mask` + REAL `hardmask`; pass 11 added `fill`/`searchsorted`/`byteswap`/`partition`/`argpartition`/`itemsize`/`nbytes`/`strides`; **pass 12 added `copy`/`choose`/`product`**. The earlier "81/~13" tally was hand-counted and slightly inflated; 77/17 is the authoritative reflected count.) |

**Behavioral correctness of what IS implemented:** a 186-case differential (3 dtypes × reductions[×3 axes]/ufuncs/constructors/extras/sort/unique/set-ops) found **0 real value/mask/dtype mismatches**, plus 79 + 33 + 22 + 25 + 19 + (26 + 5) + (40 + 3) checks of every 2026-09-14 addition (incl. the pass-7 `polyfit`/masked-axis `median`/`dot.strict`/`take.mode`/`squeeze.axis`, the pass-9 conversion/export + hard-mask surface, the pass-10 `flatten_mask`/`make_mask_descr`/`mr_`, and the **pass-11 byte-exact masked `str`/`repr` + `masked_print_option` + `fill`/`searchsorted`/`byteswap`/`partition`/`argpartition`/`itemsize`/`nbytes`/`strides`**) against probed NumPy 2.4.2 output (0 failures). The only divergence is the intentional scalar-vs-0d return-type choice (§5.1). Set operations (`intersect1d`/`union1d`/`setxor1d`/`setdiff1d`, added 2026-09-14, commit 566fd303) are bit-exact. The shared numeric array-printing path is unregressed by the masked-print hook (241 printing unit tests + the 2094-case `dtype_text` oracle differential tier, both green). **A pass-13 6055-case behavioral differential (full op surface × ALL 13 NumPy dtypes × 7 memory layouts × edge masks) is 5961/6055 byte-exact + 39 correct-error + 4 correct-masked-singleton after fixing 3 real bugs it surfaced** (the `arctan2(complex)` CLR CRASH, the pass-12 shift-scalar dtype regression, and the 1-D narrow-float `median` dtype — pass-13 headline). The remaining **51 are all ~1 ULP allclose divergences** (50 `var`/`std` accumulation-order, 1 complex `sinh`), so the module is byte-exact EXCEPT `var`/`std` and the complex-unary transcendentals — **NOT literally 100 % byte-perfect, but every value correct and every divergence documented (§5.6/§5.8).** (The pass-12 "527/527" was a smaller, less diverse oracle that missed all three bugs — see the pass-13 lesson.) Gate: **90 `MaskedArrayTests`, green net8.0/net10.0.**
...
`sum/prod/mean/min/max/count/anom/cumsum/abs/negative/add/multiply/divide/sqrt/power/compressed/filled/average` are bit-exact for bool/int8/uint8/int16/uint16/int32/uint32/int64/uint64/float16/float32/float64/complex128 (modulo §4.1 scalar-vs-0d). masked-singleton propagation (`add(masked,x)`, `sqrt(masked)`, all-masked reduction → `masked`) is correct.

**`var`/`std` are the exception — NOT byte-exact (~1 ULP, allclose).** The pass-13 6055-case differential showed `var`/`std` diverge from NumPy by ~1 ULP at float64 (an accumulation-ORDER difference vs NumPy's exact two-pass `mean(|x-mean|²)` summation) across EVERY dtype that computes them in float64 (int→float64 included), and by more at float16 (the older "prefer-precise, NumSharp more accurate" note — real, but the divergence is broader than float16). This is the same class as the documented float32-`sum` NaN-bit / accumulation-order divergences (matching NumPy's exact float summation order would abandon the multi-accumulator reduction). The **complex-unary transcendentals** (`sqrt/log/exp/sin/cos/tan/sinh/cosh/tanh/arcsin/arccos/arctan/…` on complex128) likewise carry the documented **≤3-ULP** envelope (`NDComplexMath`), so a strict byte compare of e.g. `ma.sinh(complex)` can differ by 1 ULP while staying allclose. Every value is correct; only the last bit(s) differ. Do not re-flag these as bugs — closing them is a shared-reduction-core / complex-kernel change, not an ma matter.
...
15. **2026-09-14 pass 12** — a fresh multi-technique re-audit (do NOT assume "functionally complete" means "audited"). **Technique A — reflection surface diff:** dumped NumPy's `__all__` (226) + `MaskedArray` members (94) from Python, reflected NumSharp's `MaskedArrayModule`/`MaskedArray` public members via a signed `dotnet run` script, and set-differenced. Found instance `copy`/`choose`/`product` present only as MODULE functions → added the instance wrappers. **TRAP — the reflection filter's `set_`/`get_` accessor-name drop produced a FALSE POSITIVE for `set_fill_value`** (a real module function whose name starts with `set_`); always cross-check a flagged name against the source before "closing" it. **Technique B — a 527-case behavioral differential fuzz** (Python oracle emits per-element bit-tokens — `"M"` at masked slots, else the value's little-endian bit pattern — across 4 dtypes × 5 layouts × 28 ops + edges; C# replays and bit-compares, made layout-independent by `.copy()`-ing any non-C-contiguous result before `GetAtIndex`). Found the scalar-strong promotion divergence (§5.8). **Three harness TRAPS hit, all documented elsewhere but re-encountered:** (1) `NDArray != null` is an ELEMENTWISE compare returning `NDArray<bool>` (CS0266) — use `is not null`; (2) `GetInt64(long)` binds the COORDINATE overload (`long[]`), not a flat index → use `GetAtIndex(long)` for flat C-order reads; (3) `GetAtIndex(0)` on a 0-D result works where `GetInt64(0)` (coordinate) throws. **Discovery that made the fix tractable:** NumPy's ma scalar promotion is EXACTLY `result_type(array_dtype, np.asarray(scalar).dtype)` (verified predicted==actual across dtypes×scalars), and it must widen the INPUTS not the result (probed `ma.multiply(int32 2e9, 2) == 4e9`, so casting the result up would give the int32-wrapped value). Gated by 2 new `MaskedArrayTests` (**87 total**, green net8.0 + net10.0); post-fix fuzz 527/527. No worktree quarantine needed — only `Ma/MaskedArray.cs` + its test changed, and the ma change cannot affect non-ma code (single-file boundary). **Surface diff RE-VERIFIED with the correct filter** (`MethodInfo.IsSpecialName` distinguishes compiler-generated accessors from real methods, instead of the lossy `set_`/`get_` name-prefix drop): every prefixed member the old filter hid is a `[Method special]` accessor EXCEPT the one genuine `MaskedArrayModule.set_fill_value` (the sole false positive, already caught), so no real gap was missed — a name-prefix filter can only INFLATE the "missing" list, never hide a gap, and this proved it. The authoritative reflected counts are **218/226 module** and **77/94 instance**; the earlier hand-tallied "81/~13" instance figure was slightly inflated and is corrected to 77/17 (all 17 blocked/policy, enumerated in §0). `MaskedArray.get_fill_value`/`set_fill_value` reflect as `[Method special]` — they exist as the `fill_value` PROPERTY's accessors, which is the concrete CS0082 reason NumPy's method spellings can't be added.

16. **2026-09-16 pass 13** — the "prove byte-perfect" pass. Built a **6055-case** oracle differential (vs pass-12's 527): the full implemented op surface × ALL 13 NumPy dtypes × 7 layouts {C,F,T,rev,strided,1-D-row,offset} × edge masks, Python `emit(thunk)` (defers each op so any error/crash is captured, not raised), per-element bit-tokens with NaN canonicalized. **It disproved "527/527 byte-perfect" and found 3 real bugs, all fixed:** (A) `np.arctan2(complex,complex)` → **fatal CLR `ExecutionEngineException`** (heap corruption in `ExecuteATan2Kernel`, no complex guard) — a CORE bug; fixed in `Default.ATan2.cs` mirroring `hypot`'s guard (throws NumPy's `TypeError`). **Isolation was the hard part** — the crash surfaced non-deterministically at *later* allocations (usually `arctan2:float16`), and every op passed in isolation; bisecting by fresh-process chunks showed it needed op DIVERSITY, and looping single complex ufuncs 250× was clean, until a direct `np.arctan2(complex,complex)` reproduced it minimally. **All the "cumulative churn crashes" were THIS one bug.** (B) a **pass-12 REGRESSION**: `PromoteMaScalar` widened `ma.left_shift`/`right_shift` scalars to int64, but NumPy's shifts are plain functions that keep the array dtype — fixed with a `promoteScalar:false` flag; the stale-runfile-cache trap ([[dotnet-run-script-stale-cache]]) hid the fix until a fresh filename was used. (C) masked `median` of a 1-D narrow-float along an axis returned float64 (0-D-weak `/2` widening) vs NumPy's float16/float32 — fixed with a bit-exact dtype restore. **Post-fix: 5961/6055 byte-exact + 39 correct-error + 4 correct-masked-singleton; the 51 remainders are all ~1 ULP allclose (50 `var`/`std`, 1 complex `sinh`).** Gated by 3 new `MaskedArrayTests` (**90 total**, green both TFMs) + 27 core arctan2 tests (no regression). **Lesson: a smaller/less-diverse oracle certified clean what a bigger one found broken — always widen dtype×layout×op coverage before claiming "N/N bit-exact", and never trust a fresh-Core result from a cached runfile.**
