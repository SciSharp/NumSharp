...
**How it works.** Generators under `test/oracle/` run real NumPy 2.4.2 and emit a **committed, bytes-exact JSONL corpus**; the C# harness under `test/NumSharp.Tests/Fuzz/` **replays the operand bytes** and bit-compares. NumPy never runs at test time — the committed corpus *is* its frozen answer.
...
**Conversion-helper primitives** — two further NumPy-parity types adapted from vendored .NET BCL sources under `refs/dotnet/`, **standalone helpers** for now (not yet wired into `NPTypeCode`):
