...
(`test/NumSharp.Tests.Oracle/Fuzz/corpus/linalg_parity.jsonl`, 264 cases — `cholesky`/`eig`/`eigvals`/
`eigh`/`eigvalsh`/`svd`/`svdvals`/`pinv`/`matrix_rank`/`cond{None,2,-2}`/`lstsq`/`qr` + `norm{2,-2,nuc}`,
byte-exact vs NumPy 2.4.2 on NumPy's own pinned scipy-openblas at threads=1, matched by CONTENT sha256 and
**Inconclusive off the pinned host** — the `matmul_parity` model; complex-Hermitian `eigh` eigenvectors,
float32-`eig`-with-complex-eigenvalues and non-SVD `cond`/`norm` orders excluded as not byte-reproducible,
see `Fuzz/README.md` → "LAPACK factorisation family"). The 92 API/error-contract tests above remain unit
tests — they assert signatures and verbatim messages, not values.
...
- **LAPACK factorisations**: `linalg_parity.jsonl` (264) is the FIRST *corpus* value gate for `cholesky`/`eig`/`eigvals`/`eigh`/`eigvalsh`/`svd`/`svdvals`/`pinv`/`matrix_rank`/`cond{None,2,-2}`/`lstsq`/`qr` + `norm{2,-2,nuc}` — **HOST-PINNED like `matmul_parity`** (`linalg_parity.host.jsonl`, same `MatmulParityPin`), because Core ships no managed LU/QR/SVD/eigensolver: they compute ONLY through `NumSharp.Interop.OpenBLAS`, byte-exact on NumPy's own pinned scipy-openblas at **threads=1**, Inconclusive off the pinned host. tuple results (svd/eig/eigh/qr/lstsq) + array siblings; float32 byte-exact too (NumPy's "lite" double-compute-round). Excluded as not byte-reproducible: complex-Hermitian `eigh` eigenvectors (heevd phase, cross-process — covered by `eigvalsh`), float32-`eig`-complex-eigenvalues (complex64 vs complex128), non-SVD `cond`/`norm` orders (1-ULP reduction-order). 264/264 bit-exact on the pinned host.
