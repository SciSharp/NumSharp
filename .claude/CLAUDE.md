...
**Each of those ufuncs exposes ONE NumPy-shaped overload** — `f(x[, x2], NDArray out = null, NDArray where = null[, NPTypeCode? dtype = null])` mirroring NumPy's `f(x1[, x2], /, out=None, *, where=True, dtype=None)`: `out` is the second/third positional slot exactly like NumPy, `where`/`dtype` are reachable by name without `out`. `dtype=` selects the LOOP (NumPy loop-signature semantics, probed): computation runs at that precision even with `out=` (`sqrt([2.], out=f64, dtype=f32)` stores the f32-rounded value), `power(2, -1, dtype: f64) = 0.5` (the negative-int-exponent ValueError applies only to integer loops), `power(10, 11, dtype: f64) = 1e11` exactly (no compute-then-cast), `negative(bool, dtype: f64)` is legal, and inputs must reach the loop via same_kind casts (verbatim `Cannot cast ufunc '<name>' input [N] from ...` errors; float-only ufuncs raise `No loop matching ... ufunc <name>` for int/bool requests). Exception: the binary `add`/`subtract`/`multiply`/`divide`/`true_divide`/`mod` six don't take `dtype` yet (their TensorEngine signatures lack a typeCode — separate wave). Legacy positional-dtype overloads (`np.sqrt(x, NPTypeCode.Single)`, `(x, Type)`) remain for source compat but are non-NumPy call forms (NumPy's 2nd positional is `out`). Tests: `Math/UfuncDtypeOverloadTests.cs`.
