...
- **Compiled once per tree instance** (`NDExprProgram`, `Backends/Iterators/NDExpr.Program.cs`; plan `docs/plans/ndexpr-evaluate.md` §0.1): binding, the NEP50 typing pass, the vector plan and the kernel lookup run on the FIRST `np.evaluate` of a root and are cached on it (a positional tree re-validates its dtype signature per call and recompiles on a change; `NDExpr.ForceScalar` programs never serve the vector mode), so a prebuilt tree costs **~0.4 µs / 0.6 KB per call at n=8** (was 1.0 µs / 2.9 KB; unfused `a*b+c` 0.35 µs / 0.9 KB, NumPy 0.52 µs) and beats the unfused chain on 16 of 22 probe rows at 1K. The inline-rebuilt spelling `np.evaluate((NDExpr)a * b + c)` inside a loop builds a NEW root each call and still pays bind + typing (~0.9 µs) — hoist the tree, or `Compile()` it; Phase 6.1's structural hash is the lever. **`expr.Compile()` / `expr.Compile(params NPTypeCode[] inputTypes)` → `CompiledExpression`** is the explicit handle (numexpr's `NumExpr(expr)`): eager JIT, `Evaluate(out)` for embedded arrays / `Evaluate(operands, out)` for positional trees, `IsPositional`/`IsReduction`/`InputTypes`/`ResultType`; a positional handle PINS its dtype signature (`TypeError` naming both signatures on a mismatch — never a silent recompile). Not the Tier-3C `Compile(inputTypes, outputType, cacheKey)`, which emits a raw kernel computed at ONE dtype. The inputs' broadcast is one N-ary single-allocation pass (`BroadcastInputDims`) whose mismatch lists EVERY operand shape like NumPy (`operands could not be broadcast together with shapes (2,3) (2,3) (4,) `, `IncorrectShapeException`); `out=` joins through `ResolveUfuncIterationShape` unchanged. **Literal-as-parameter:** a 0-d `NDArray` operand (`NDExpr.Input(2)` + `NDArray.Scalar(k)`) serves a constant sweep with ONE kernel where a literal bakes one kernel per value (~1.1 ms JIT each) — it is a STRONG scalar (`np.float64(k)`: `int32 * 0-d int64 → int64`) where a literal is weak; it costs ~0.67 µs vs 0.42 for identical dims, the residual being NDIter's stride-0 operand construction (an iterator-level cost every scalar-operand ufunc pays). Gate: `NDEvaluateProgramTests` (14).
