...
|-------|---------|----------|
| 1 | `### 📦 New NuGet Packages` | Packages shipped for the first time in this range. **Multi-line elaboration** (§7A), not one-liners. |
| 2 | `### 📊 Dashboards & Docs` | Living, CI-generated dashboards + key doc surfaces (§7B). Linked, one-liner each. |
| 3 | `### ✨ New APIs & Modules` | Brand-new `np.*` / module surface introduced in the range. |
| 4 | `### 🧩 ndarray surface` | New/changed `ndarray.*` instance members (kept separate from `np.*` because it reads as one story). |
| 5 | `### ⚡ Performance` | Speed/allocation wins. **Leads with the ratio** (§8), under a one-line NPY/NS explainer. |
| 6 | `### 🎯 Parity & Fixes` | Correctness/behavior alignment with NumPy that isn't a new API. |
| 7 | `### 🧰 Testing & Tooling` | Oracle/benchmark/coverage infra - only the notable, deduplicated items. |
| 8 | `### 💥 Breaking Changes` | Anything that changes an existing signature, return type, or observable behavior a caller relied on. **Last** so upgraders see what they gain before what they must migrate. |
...
- **Lead with the API name**, `code-formatted`. Then a **space-hyphen-space (` - `) separator**, then
  the value in plain language, then the commit hash(es) last.
  ```
  - `np.kron` - Kronecker product, matches NumPy 2.4.2 - `7bcad845`.
  ```
- **Use the ASCII hyphen `-` only - never the em-dash (U+2014) or en-dash (U+2013) character.** The
  whole changelog - separators, numeric ranges, parentheticals, dates - uses the plain hyphen, chosen
  for greppability and clean diffs between revisions (the same rationale as the ASCII `->` in
  `xLOW->xHIGH`, §8). A stray long dash is a defect; normalize it to `-`.
...
- Always the **last** section (it closes the changelog). Mark the feature so a scanner cannot miss it.
...
- [ ] Breaking changes last, with old→new + migration (§9).
...
| 2026-08-23 | **Section order:** moved `### 💥 Breaking Changes` from first to **last** (order 8); `### 📦 New NuGet Packages` now leads. Rationale: upgraders see what they gain before what they must migrate. Updated the §3 table, §9 ("last section"), the §13 checklist, and the triage section-map. |
| 2026-08-23 | **Separator / charset:** the bullet separator is a **space-hyphen-space (` - `)**, and the changelog uses the **ASCII hyphen `-` only - never em-dash (U+2014) or en-dash (U+2013)** anywhere (separators, ranges, parentheticals, dates), for greppability + clean diffs (§4). |
