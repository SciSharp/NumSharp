...
1. `### 📦 New NuGet Packages` - packages shipped for the first time (§7A multi-line elaboration).
2. `### 📊 Dashboards & Docs` - living CI dashboards + key doc surfaces (§7B, linked, current numbers).
3. `### ✨ New APIs & Modules` - brand-new `np.*` / module surface.
4. `### 🧩 ndarray surface` - new/changed `ndarray.*` instance members.
5. `### ⚡ Performance` - measured speed/allocation wins; **lead with `xLOW->xHIGH`** under the
   one-line NPY/NS explainer.
6. `### 🎯 Parity & Fixes` - correctness/behavior alignment with NumPy that isn't a new API.
7. `### 🧰 Testing & Tooling` - only notable, deduplicated oracle/benchmark/coverage items.
8. `### 💥 Breaking Changes` - changed signature/return type/observable behavior; deprecations; env
   renames. **Last.** State old → new + migration.
