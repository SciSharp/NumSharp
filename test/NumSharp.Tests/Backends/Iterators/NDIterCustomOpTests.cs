...
{
    /// <summary>
    /// Exercises the three-tier custom-op API on NDIterRef:
    ...
    ///   Tier 3C — ExecuteExpression (NDExpr DSL compiled to inner-loop IL)
    /// </summary>
    [TestClass]
    public unsafe class NDIterCustomOpTests
    {
        // =====================================================================
        // Tier 3A: Raw IL
        // =====================================================================
        ...
        public void Tier3B_ElementWiseUnary_Sqrt_Float32_Simd()
        {
            ...
            for (int k = 0; k < 32; k++)
                Assert.AreEqual((float)System.Math.Sqrt(k + 1), output.GetSingle(k), 1e-5f, $"out[{k}] wrong");
        }
        ...
        public void Tier3B_CacheReuse_SameKeyReturnsIdenticalDelegate()
        {
            ...
            using (var iter = NDIterRef.MultiNew(3, new[] { a1, b1, c1 },
            ...
            using (var iter2 = NDIterRef.MultiNew(3, new[] { a2, b2, c2 },
                NDIterGlobalFlags.EXTERNAL_LOOP, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                new[] { NDIterPerOpFlags.READONLY, NDIterPerOpFlags.READONLY, NDIterPerOpFlags.WRITEONLY }))
            {
        ...
        public void Tier3C_Expression_AddConstant()
        {
            ...
            var expr = NDExpr.Input(0) + NDExpr.Const(5.0f);
        ...
        public void Tier3C_Expression_CompoundFma()
        {
            ...
            var expr = (NDExpr.Input(0) + NDExpr.Input(1)) * NDExpr.Input(2) + NDExpr.Const(1.0f);
        ...
        public void Tier3C_Expression_SqrtOfSumSquares()
        {
            ...
            var expr = NDExpr.Sqrt(NDExpr.Square(NDExpr.Input(0)) + NDExpr.Square(NDExpr.Input(1)));
            iter.ExecuteExpression(expr,
                new[] { NPTypeCode.Single, NPTypeCode.Single }, NPTypeCode.Single);
        ...
        public void Tier3C_Expression_NegateAndAbs()
        {
            ...
            // out = -|a|
            var expr = -NDExpr.Abs(NDExpr.Input(0));
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single);
        ...
        public void Tier3C_Expression_DoubleDtype()
        {
            ...
            var expr = NDExpr.Input(0) * NDExpr.Const(2.0) + NDExpr.Const(3.0);
        ...
        public void Tier3C_Expression_StridedPath()
        {
            ...
            var expr = NDExpr.Input(0) * NDExpr.Input(0);   // square
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single);
