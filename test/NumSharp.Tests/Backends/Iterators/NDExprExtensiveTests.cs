...
{
    /// <summary>
    /// Battletest coverage for the expanded NDExpr DSL.
    /// Each op class has:
    ...
    /// </summary>
    [TestClass]
    public unsafe class NDExprExtensiveTests
    {
        // =====================================================================
        // Helpers
        // =====================================================================
            ...
            double tol = 1e-9, string? key = null)
        {
            ...
            for (int i = 0; i < xs.Length; i++)
            {
                ...
                if (double.IsNaN(want))
                    ...
                    Assert.IsTrue(double.IsInfinity(got) && System.Math.Sign(got) == System.Math.Sign(want),
        ...
        private static void RunBinary_f64(
            double[] xs, double[] ys, Func<NDExpr, NDExpr, NDExpr> fn,
            Func<double, double, double> expected, double tol = 1e-9, string? key = null)
        {
            ...
            iter.ExecuteExpression(fn(NDExpr.Input(0), NDExpr.Input(1)),
        ...
        public void Mod_OperatorOverload_Percent()
        {
            ...
            var c = np.empty_like(a);
            using var iter = Iter3(a, b, c);
            iter.ExecuteExpression(NDExpr.Input(0) % NDExpr.Input(1),
        ...
        public void Mod_Int32_FlooredSemantics()
        {
            ...
            var c = np.empty_like(a);
            using var iter = Iter3(a, b, c);
            iter.ExecuteExpression(NDExpr.Mod(NDExpr.Input(0), NDExpr.Input(1)),
        ...
        public void Power_Double_IntegerAndFractional()
        {
            RunBinary_f64(
                ...
                NDExpr.Power, System.Math.Pow, key: "pow_f64_v1");
        }
        ...
        public void Power_Double_NaNInput()
        {
            ...
            var c = np.empty_like(a);
            using var iter = Iter3(a, b, c);
            iter.ExecuteExpression(NDExpr.Power(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "pow_nan_v1");
        ...
        public void FloorDivide_Double_NegativeFloorsDown()
        {
            RunBinary_f64(
                ...
                NDExpr.FloorDivide,
                (x, y) => System.Math.Floor(x / y), key: "floordiv_f64_v1");
        }
        ...
        public void FloorDivide_Int32_SignedFloor()
        {
            ...
            var c = np.empty_like(a);
            using var iter = Iter3(a, b, c);
            iter.ExecuteExpression(NDExpr.FloorDivide(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Int32, NPTypeCode.Int32 }, NPTypeCode.Int32,
        ...
        public void ATan2_Quadrants()
        {
            RunBinary_f64(
                ...
                NDExpr.ATan2, System.Math.Atan2, tol: 1e-9, key: "atan2_f64_v1");
        }
        ...
        public void Min_Double_ReturnsSmaller()
        {
            RunBinary_f64(
                ...
                NDExpr.Min, System.Math.Min, key: "min_f64_v1");
        }
        ...
        public void Max_Int32_ReturnsLarger()
        {
            ...
            var c = np.empty_like(a);
            using var iter = Iter3(a, b, c);
            iter.ExecuteExpression(NDExpr.Max(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Int32, NPTypeCode.Int32 }, NPTypeCode.Int32,
                cacheKey: "max_i32_v1");
        ...
        public void Min_Double_NaNPropagation()
        {
            ...
            var c = np.empty_like(a);
            using var iter = Iter3(a, b, c);
            iter.ExecuteExpression(NDExpr.Min(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "min_nan_v1");
            Assert.AreEqual(1.0, c.GetDouble(0), 1e-9);
        ...
        public void Clamp_Double_ToRange()
        {
            ...
            iter.ExecuteExpression(
                NDExpr.Clamp(NDExpr.Input(0), NDExpr.Const(0.0), NDExpr.Const(1.0)),
        ...
        public void Where_SelectsByCondition()
        {
            ...
            using var it = NDIterRef.MultiNew(4, new[] { cond, a, b, r },
                NDIterGlobalFlags.EXTERNAL_LOOP, NPY_ORDER.NPY_KEEPORDER,
                NPY_CASTING.NPY_SAFE_CASTING,
            ...
            it.ExecuteExpression(
                NDExpr.Where(NDExpr.Input(0), NDExpr.Input(1), NDExpr.Input(2)),
        ...
        public void Where_ReLUComposition()
        {
            ...
            var expr = NDExpr.Where(NDExpr.Greater(x, NDExpr.Const(0.0)),
                x, NDExpr.Const(0.0));
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "relu_f64_v1");
            for (int i = 0; i < xs.Length; i++)
                Assert.AreEqual(System.Math.Max(0, xs[i]), output.GetDouble(i), 1e-9);
        }
        ...
        [TestMethod] public void Exp_Double() => RunUnary_f64(
            new double[] { 0, 1, 2, -1, System.Math.Log(10) }, NDExpr.Exp, System.Math.Exp, tol: 1e-9, key: "exp_f64_v1");

        [TestMethod] public void Exp2_Double() => RunUnary_f64(
            new double[] { 0, 1, 2, 3, -1, 0.5 }, NDExpr.Exp2,
            x => System.Math.Pow(2, x), tol: 1e-9, key: "exp2_f64_v1");

        [TestMethod] public void Expm1_Double_AccurateNearZero() => RunUnary_f64(
            new double[] { 0, 1e-10, 1, -1 }, NDExpr.Expm1,
            x => System.Math.Exp(x) - 1, tol: 1e-9, key: "expm1_f64_v1");

        [TestMethod] public void Log_Double_SpecialValues()
        {
            var xs = new double[] { 1.0, System.Math.E, 10.0, 0.1 };
            RunUnary_f64(xs, NDExpr.Log, System.Math.Log, tol: 1e-9, key: "log_f64_v1");
        }
        ...
        [TestMethod] public void Log_Double_NegativeIsNaN()
        {
            ...
            it.ExecuteExpression(NDExpr.Log(NDExpr.Input(0)),
        ...
        [TestMethod] public void Log2_Double() => RunUnary_f64(
            new double[] { 1, 2, 4, 8, 1024 }, NDExpr.Log2, System.Math.Log2, tol: 1e-9, key: "log2_f64_v1");

        [TestMethod] public void Log10_Double() => RunUnary_f64(
            new double[] { 1, 10, 100, 1000, 1e-3 }, NDExpr.Log10, System.Math.Log10, tol: 1e-9, key: "log10_f64_v1");

        [TestMethod] public void Log1p_Double_AccurateNearZero() => RunUnary_f64(
            new double[] { 0, 1e-10, 1, -0.5 }, NDExpr.Log1p,
            x => System.Math.Log(1 + x), tol: 1e-9, key: "log1p_f64_v1");
        ...
        [TestMethod] public void Sin_Double() => RunUnary_f64(
            new double[] { 0, System.Math.PI / 2, System.Math.PI, -System.Math.PI / 2 }, NDExpr.Sin, System.Math.Sin,
            tol: 1e-9, key: "sin_f64_v1");

        [TestMethod] public void Cos_Double() => RunUnary_f64(
            new double[] { 0, System.Math.PI / 2, System.Math.PI, -System.Math.PI / 2 }, NDExpr.Cos, System.Math.Cos,
            tol: 1e-9, key: "cos_f64_v1");

        [TestMethod] public void Tan_Double() => RunUnary_f64(
            new double[] { 0, System.Math.PI / 4, -System.Math.PI / 4 }, NDExpr.Tan, System.Math.Tan,
            tol: 1e-9, key: "tan_f64_v1");

        [TestMethod] public void Sinh_Double() => RunUnary_f64(
            new double[] { 0, 1, -1, 2 }, NDExpr.Sinh, System.Math.Sinh, tol: 1e-9, key: "sinh_f64_v1");

        [TestMethod] public void Cosh_Double() => RunUnary_f64(
            new double[] { 0, 1, -1, 2 }, NDExpr.Cosh, System.Math.Cosh, tol: 1e-9, key: "cosh_f64_v1");

        [TestMethod] public void Tanh_Double() => RunUnary_f64(
            new double[] { 0, 1, -1, 100, -100 }, NDExpr.Tanh, System.Math.Tanh, tol: 1e-9, key: "tanh_f64_v1");

        [TestMethod] public void ASin_Double() => RunUnary_f64(
            new double[] { 0, 0.5, 1, -1 }, NDExpr.ASin, System.Math.Asin, tol: 1e-9, key: "asin_f64_v1");

        [TestMethod] public void ACos_Double() => RunUnary_f64(
            new double[] { 0, 0.5, 1, -1 }, NDExpr.ACos, System.Math.Acos, tol: 1e-9, key: "acos_f64_v1");

        [TestMethod] public void ATan_Double() => RunUnary_f64(
            new double[] { 0, 1, -1, 1000 }, NDExpr.ATan, System.Math.Atan, tol: 1e-9, key: "atan_f64_v1");

        [TestMethod] public void Deg2Rad_Double() => RunUnary_f64(
            new double[] { 0, 90, 180, 360, -90 }, NDExpr.Deg2Rad,
            x => x * System.Math.PI / 180.0, tol: 1e-9, key: "d2r_f64_v1");

        [TestMethod] public void Rad2Deg_Double() => RunUnary_f64(
            new double[] { 0, System.Math.PI / 2, System.Math.PI, -System.Math.PI }, NDExpr.Rad2Deg,
            x => x * 180.0 / System.Math.PI, tol: 1e-9, key: "r2d_f64_v1");
        ...
        [TestMethod] public void Floor_Double() => RunUnary_f64(
            new double[] { 1.7, -1.7, 2.5, -2.5, 0, 1 }, NDExpr.Floor, System.Math.Floor,
            tol: 0, key: "floor_f64_v1");

        [TestMethod] public void Ceil_Double() => RunUnary_f64(
            new double[] { 1.3, -1.3, 2.5, -2.5, 0, 1 }, NDExpr.Ceil, System.Math.Ceiling,
            tol: 0, key: "ceil_f64_v1");

        [TestMethod] public void Round_Double_Banker() => RunUnary_f64(
            new double[] { 0.5, 1.5, 2.5, -0.5, -1.5 }, NDExpr.Round,
            x => System.Math.Round(x), tol: 0, key: "round_f64_v1");

        [TestMethod] public void Truncate_Double() => RunUnary_f64(
            new double[] { 1.7, -1.7, 2.5, -2.5, 0 }, NDExpr.Truncate, System.Math.Truncate,
        ...
        [TestMethod] public void Sign_Double() => RunUnary_f64(
            new double[] { -5, -1, 0, 1, 5 }, NDExpr.Sign, x => (double)System.Math.Sign(x),
        ...
        [TestMethod] public void Sign_Int32()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Sign(NDExpr.Input(0)),
        ...
        [TestMethod] public void Reciprocal_Double() => RunUnary_f64(
            new double[] { 1, 2, 4, 0.5, -1 }, NDExpr.Reciprocal,
            x => 1.0 / x, tol: 1e-9, key: "recip_f64_v1");

        [TestMethod] public void Cbrt_Double() => RunUnary_f64(
            new double[] { 0, 1, 8, 27, -27, -8 }, NDExpr.Cbrt, System.Math.Cbrt,
        ...
        public void BitwiseNot_Int32_Operator()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(~NDExpr.Input(0),
        ...
        public void BitwiseNot_Int64()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.BitwiseNot(NDExpr.Input(0)),
        ...
        public void LogicalNot_Int64()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.LogicalNot(NDExpr.Input(0)),
        ...
        public void IsNaN_Double()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.IsNaN(NDExpr.Input(0)),
        ...
        public void IsFinite_Double()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.IsFinite(NDExpr.Input(0)),
        ...
        public void IsInf_Double()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.IsInf(NDExpr.Input(0)),
        ...
        public void Equal_Double()
        {
            ...
            it.ExecuteExpression(NDExpr.Equal(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "eq_f64_v1");
            Assert.AreEqual(1.0, r.GetDouble(0), 1e-9);
            Assert.AreEqual(0.0, r.GetDouble(1), 1e-9);
        ...
        public void NotEqual_Int32()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter3(a, b, r);
            it.ExecuteExpression(NDExpr.NotEqual(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Int32, NPTypeCode.Int32 }, NPTypeCode.Int32,
                cacheKey: "ne_i32_v1");
            Assert.AreEqual(0, r.GetInt32(0));
            Assert.AreEqual(1, r.GetInt32(1));
        ...
        public void Less_Double()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter3(a, b, r);
            it.ExecuteExpression(NDExpr.Less(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "lt_f64_v1");
            Assert.AreEqual(0.0, r.GetDouble(0), 1e-9);
            Assert.AreEqual(1.0, r.GetDouble(1), 1e-9);
        ...
        public void LessEqual_Double()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter3(a, b, r);
            it.ExecuteExpression(NDExpr.LessEqual(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "le_f64_v1");
            Assert.AreEqual(1.0, r.GetDouble(0), 1e-9);
            Assert.AreEqual(1.0, r.GetDouble(1), 1e-9);
        ...
        public void Greater_Double()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter3(a, b, r);
            it.ExecuteExpression(NDExpr.Greater(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "gt_f64_v1");
            Assert.AreEqual(0.0, r.GetDouble(0), 1e-9);
            Assert.AreEqual(1.0, r.GetDouble(1), 1e-9);
        ...
        public void GreaterEqual_Int32()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter3(a, b, r);
            it.ExecuteExpression(NDExpr.GreaterEqual(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Int32, NPTypeCode.Int32 }, NPTypeCode.Int32,
                cacheKey: "ge_i32_v1");
            Assert.AreEqual(1, r.GetInt32(0));
            Assert.AreEqual(1, r.GetInt32(1));
        ...
        public void Mod_StridedInput_UsesScalarFallback()
        {
            ...
            iter.ExecuteExpression(NDExpr.Mod(NDExpr.Input(0), NDExpr.Const(3.0)),
                new[] { NPTypeCode.Double }, NPTypeCode.Double, cacheKey: "mod_strided_v1");

            for (int i = 0; i < 10; i++)
            {
                double x = 2.0 * i;
                double want = x - System.Math.Floor(x / 3.0) * 3.0;
        ...
        public void Floor_ReversedStride_ProducesCorrectOutput()
        {
            ...
            iter.ExecuteExpression(NDExpr.Floor(NDExpr.Input(0)),
        ...
        public void Composition_Sigmoid_Double()
        {
            ...
            var x = NDExpr.Input(0);
            // 1 / (1 + exp(-x))
            var expr = NDExpr.Const(1.0) / (NDExpr.Const(1.0) + NDExpr.Exp(-x));
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "sigmoid_f64_v1");

            for (int i = 0; i < xs.Length; i++)
            {
                double want = 1.0 / (1.0 + System.Math.Exp(-xs[i]));
                Assert.AreEqual(want, output.GetDouble(i), 1e-9, $"[{i}]");
            }
        }
        ...
        public void Composition_Softplus_Double()
        {
            ...
            var output = np.empty_like(input);
            using var iter = Iter(input, output);
            var expr = NDExpr.Log1p(NDExpr.Exp(NDExpr.Input(0)));
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "softplus_f64_v1");

            for (int i = 0; i < xs.Length; i++)
            {
                double want = System.Math.Log(1.0 + System.Math.Exp(xs[i]));
        ...
        public void Composition_Hypot_Double()
        {
            ...
            using var it = Iter3(a, b, r);
            var x = NDExpr.Input(0);
            var y = NDExpr.Input(1);
            it.ExecuteExpression(NDExpr.Sqrt(x * x + y * y),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
        ...
        public void Composition_WhereWithComparison_Abs()
        {
            ...
            using var iter = Iter(input, output);
            var x = NDExpr.Input(0);
            var expr = NDExpr.Where(NDExpr.Less(x, NDExpr.Const(0.0)), -x, x);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Double }, NPTypeCode.Double,
                cacheKey: "where_abs_v1");

            for (int i = 0; i < xs.Length; i++)
                Assert.AreEqual(System.Math.Abs(xs[i]), output.GetDouble(i), 1e-9);
        }
        ...
        public void BitwiseAnd_IntegerDtypes(NPTypeCode dtype)
        {
            ...
            it.ExecuteExpression(NDExpr.Input(0) & NDExpr.Input(1),
        ...
        public void Exp_Float32()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Exp(NDExpr.Input(0)),
        ...
        {
            var a = np.array(new float[] { 0f, (float)(System.Math.PI / 2), (float)System.Math.PI });
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Sin(NDExpr.Input(0)),
        ...
        public void Power_Int32_OverflowWraps()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter3(a, b, r);
            it.ExecuteExpression(NDExpr.Power(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Int32, NPTypeCode.Int32 }, NPTypeCode.Int32,
                cacheKey: "pow_i32_v1");
        ...
        public void Cache_DistinctExpressionsProduceDistinctKernels()
        {
            ...
            using (var it = Iter(a, r1))
                it.ExecuteExpression(NDExpr.Exp(NDExpr.Input(0)),
                    new[] { NPTypeCode.Double }, NPTypeCode.Double);
            int afterExp = GeneratedDelegates.InnerLoopCount;

            using (var it = Iter(a, r2))
                it.ExecuteExpression(NDExpr.Log(NDExpr.Input(0)),
                    new[] { NPTypeCode.Double }, NPTypeCode.Double);
            int afterLog = GeneratedDelegates.InnerLoopCount;
        ...
        public void DeepNesting_20Layers_Math()
        {
            ...
            NDExpr expr = NDExpr.Input(0);
            for (int i = 0; i < 10; i++)
                expr = NDExpr.Cos(NDExpr.Sin(expr));
            ...
            double want = 0.5;
            for (int i = 0; i < 10; i++)
                want = System.Math.Cos(System.Math.Sin(want));
        ...
        public void Polynomial_Degree5_Int32()
        {
            ...
            var x = NDExpr.Input(0);
            var expr = (((x + NDExpr.Const(2)) * x + NDExpr.Const(3)) * x +
                       NDExpr.Const(4)) * x + NDExpr.Const(5);
        ...
        public void Stress_Power_AcrossSizes(int size)
        {
            ...
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Power(NDExpr.Input(0), NDExpr.Const(2.0)),
        ...
        public void Empty_Mod_NoCrash()
        {
            ...
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Mod(NDExpr.Input(0), NDExpr.Const(3.0)),
        ...
        public void Single_Element_Power()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Power(NDExpr.Input(0), NDExpr.Const(4.0)),
        ...
        public void AutoKey_EquivalentExpressionsShareKernel()
        {
            ...
            using (var it = Iter(a, r))
            {
                var expr = NDExpr.Sqrt(NDExpr.Input(0));
                it.ExecuteExpression(expr, new[] { NPTypeCode.Double }, NPTypeCode.Double);
            }
            int after1 = GeneratedDelegates.InnerLoopCount;
        ...
        public void AutoKey_DifferentConstantsProduceDifferentKernels()
        {
            ...
            using (var it = Iter(a, r))
                it.ExecuteExpression(NDExpr.Input(0) + NDExpr.Const(1.0),
                    new[] { NPTypeCode.Double }, NPTypeCode.Double);
            int after1 = GeneratedDelegates.InnerLoopCount;

            using (var it = Iter(a, r))
                it.ExecuteExpression(NDExpr.Input(0) + NDExpr.Const(2.0),
                    new[] { NPTypeCode.Double }, NPTypeCode.Double);
            int after2 = GeneratedDelegates.InnerLoopCount;
        ...
        public void Max_StridedInput()
        {
            ...
            using var it = Iter3(a, b, r);
            it.ExecuteExpression(NDExpr.Max(NDExpr.Input(0), NDExpr.Input(1)),
                new[] { NPTypeCode.Double, NPTypeCode.Double }, NPTypeCode.Double,
        ...
        public void Where_StridedInput()
        {
            ...
            it.ExecuteExpression(
                NDExpr.Where(
                    NDExpr.Greater(NDExpr.Input(0), NDExpr.Const(0.0)),
                    NDExpr.Input(1), NDExpr.Input(2)),
                new[] { NPTypeCode.Double, NPTypeCode.Double, NPTypeCode.Double },
        ...
        public void Sqrt_Int32Input_Float64Output()
        {
            ...
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Sqrt(NDExpr.Input(0)),
        ...
        public void Exp_Int32Input_Float64Output()
        {
            ...
            Assert.AreEqual(1.0, r.GetDouble(0), 1e-9);
            Assert.AreEqual(System.Math.E, r.GetDouble(1), 1e-9);
            Assert.AreEqual(System.Math.E * System.Math.E, r.GetDouble(2), 1e-9);
        }
        ...
        public void Validation_InputOutOfRange_ThrowsAtCompile()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Input(5),  // out of range
                new[] { NPTypeCode.Double }, NPTypeCode.Double,
        ...
        public void Composition_ScalarTopSimdBottom()
        {
            ...
            var x = NDExpr.Input(0);
            var expr = NDExpr.Mod(NDExpr.Square(x + NDExpr.Const(1.0)), NDExpr.Const(7.0));
            it.ExecuteExpression(expr, new[] { NPTypeCode.Double }, NPTypeCode.Double,
        ...
        public void Composition_PredicateUsedInArithmetic()
        {
            ...
            using var it = Iter(a, r);
            var x = NDExpr.Input(0);
            var expr = NDExpr.Where(NDExpr.IsNaN(x), NDExpr.Const(0.0), x);
            it.ExecuteExpression(expr, new[] { NPTypeCode.Double }, NPTypeCode.Double,
        ...
        public void Abs_Int64_MinValue_Overflows()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Abs(NDExpr.Input(0)),
        ...
        public void Square_Float32_LargeContiguous()
        {
            ...
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Square(NDExpr.Input(0)),
                new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        // =====================================================================
        // Reuse same NDExpr instance across two executes
        // =====================================================================

        [TestMethod]
        public void Reuse_SameExprInstance_ExecutesTwice()
        {
            var expr = NDExpr.Exp(NDExpr.Input(0));
            ...
            Assert.AreEqual(System.Math.E, r1.GetDouble(1), 1e-9);
            Assert.AreEqual(System.Math.E * System.Math.E, r2.GetDouble(0), 1e-9);
            Assert.AreEqual(System.Math.E * System.Math.E * System.Math.E, r2.GetDouble(1), 1e-9);
        }
        ...
        public void Constant_Only_Expression_BroadcastsConstant()
        {
            ...
            var r = np.empty_like(a);
            using var it = Iter(a, r);
            it.ExecuteExpression(NDExpr.Const(42.0) + NDExpr.Const(0.0) * NDExpr.Input(0),
                new[] { NPTypeCode.Double }, NPTypeCode.Double,
