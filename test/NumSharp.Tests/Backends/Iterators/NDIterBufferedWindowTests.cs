...
/// </summary>
[TestClass]
public class NDIterBufferedWindowTests
{
    private const int MultiFillN = 20_005; // 3 windows: 8192 + 8192 + 3621

    private static readonly NDIterPerOpFlags Elw = NDIterPerOpFlags.OVERLAP_ASSUME_ELEMENTWISE_PER_OP;
    ...
    public void PerElement_BufferedCast_MultiWindow_Int32ToFloat64()
    {
        ...
        using var iter = NDIterRef.New(
            arr, NDIterGlobalFlags.BUFFERED,
    ...
    public void PerElement_BufferedCast_StridedSource_MultiWindow()
    {
        ...
        using var iter = NDIterRef.New(
            view, NDIterGlobalFlags.BUFFERED,
            NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
            NPTypeCode.Double);
    ...
    public void WindowDriver_BufferedCastBinary_MultiFill_Values()
    {
        ...
        using (var iter = NDIterRef.MultiNew(3, new[] { a, b, outNd },
            NDIterGlobalFlags.EXTERNAL_LOOP | NDIterGlobalFlags.BUFFERED |
            NDIterGlobalFlags.GROWINNER | NDIterGlobalFlags.DELAY_BUFALLOC,
            NPY_ORDER.NPY_CORDER, NPY_CASTING.NPY_UNSAFE_CASTING,
            new[] { NDIterPerOpFlags.READONLY | Elw, NDIterPerOpFlags.READONLY | Elw, NDIterPerOpFlags.WRITEONLY | Elw },
    ...
    public void WindowDriver_BufferedCastOutput_WriteFlushAcrossWindows()
    {
        ...
        using (var iter = NDIterRef.MultiNew(2, new[] { src, outNd },
            NDIterGlobalFlags.EXTERNAL_LOOP | NDIterGlobalFlags.BUFFERED |
            NDIterGlobalFlags.GROWINNER | NDIterGlobalFlags.DELAY_BUFALLOC,
            NPY_ORDER.NPY_CORDER, NPY_CASTING.NPY_UNSAFE_CASTING,
            new[] { NDIterPerOpFlags.READONLY | Elw, NDIterPerOpFlags.WRITEONLY | Elw },
    ...
    public void WindowDriver_2D_StridedCastSource_RowAlignedWindows()
    {
        ...
        using (var iter = NDIterRef.AdvancedNew(2, new[] { v, outNd },
            NDIterGlobalFlags.EXTERNAL_LOOP | NDIterGlobalFlags.BUFFERED | NDIterGlobalFlags.GROWINNER,
            NPY_ORDER.NPY_CORDER, NPY_CASTING.NPY_UNSAFE_CASTING,
            new[] { NDIterPerOpFlags.READONLY | Elw, NDIterPerOpFlags.WRITEONLY | Elw },
            new[] { NPTypeCode.Double, NPTypeCode.Double },
            bufferSize: 4096))
    ...
    public unsafe void DelayBufalloc_DefersAllocation_AutoEnsuresOnExecute()
    {
        ...
        using var iter = NDIterRef.MultiNew(2, new[] { arr, outNd },
            NDIterGlobalFlags.EXTERNAL_LOOP | NDIterGlobalFlags.BUFFERED | NDIterGlobalFlags.DELAY_BUFALLOC,
            NPY_ORDER.NPY_CORDER, NPY_CASTING.NPY_UNSAFE_CASTING,
            new[] { NDIterPerOpFlags.READONLY | Elw, NDIterPerOpFlags.WRITEONLY | Elw },
            new[] { NPTypeCode.Double, NPTypeCode.Double });
        ...
        Assert.IsTrue((iter.RawState->ItFlags & (uint)NDIterFlags.DELAYBUF) != 0, "DELAYBUF flag set");
        ...
        Assert.IsTrue((iter.RawState->ItFlags & (uint)NDIterFlags.DELAYBUF) == 0, "DELAYBUF cleared after ensure");
    ...
    public unsafe void DelayBufalloc_Reset_MaterializesBuffers()
    {
        ...
        using var iter = NDIterRef.New(
            arr, NDIterGlobalFlags.BUFFERED | NDIterGlobalFlags.DELAY_BUFALLOC,
            NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
            NPTypeCode.Double);
    ...
    public void Production_PromotingUnary_Sqrt_Int32_MultiFill()
    {
        ...
        for (int i = 0; i < MultiFillN; i += 983)
            Assert.AreEqual(System.Math.Sqrt(i), r.GetDouble(i), 1e-12, $"element {i}");
        Assert.AreEqual(System.Math.Sqrt(MultiFillN - 1), r.GetDouble(MultiFillN - 1), 1e-12, "last element (multi-fill)");
    }
    ...
    public void Production_PromotingUnary_Sqrt_Strided2D()
    {
        ...
        for (int i = 0; i < 100; i += 7)
            for (int j = 0; j < 100; j += 11)
                Assert.AreEqual(System.Math.Sqrt(v.GetInt32(i, j)), r.GetDouble(i, j), 1e-12, $"({i},{j})");
        Assert.AreEqual(System.Math.Sqrt(v.GetInt32(99, 99)), r.GetDouble(99, 99), 1e-12);
    }
    ...
    public unsafe void Buffered_LinearSameDtypeOperand_StaysUnbuffered()
    {
        ...
        using var iter = NDIterRef.New(view, NDIterGlobalFlags.BUFFERED);
        ...
        using var iter2 = NDIterRef.New(view2, NDIterGlobalFlags.BUFFERED,
