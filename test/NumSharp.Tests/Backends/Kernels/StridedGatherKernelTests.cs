    ...
    public void StridedUnary_2D_AllRowsComputed_FlagCollisionRegression()
    {
        ...
        for (int i = 0; i < 100; i += 7)
            for (int j = 0; j < 100; j += 11)
                Assert.AreEqual(System.Math.Sqrt(v.GetDouble(i, j)), r.GetDouble(i, j), 1e-12, $"({i},{j})");
        Assert.AreEqual(System.Math.Sqrt(v.GetDouble(99, 99)), r.GetDouble(99, 99), 1e-12, "last element");
    }
