...
public class SimdRoundingMultiVersionTests
{
    ...
    private static double Oracle(string op, double x) => op switch
    {
        ...
        "trunc" => System.Math.Truncate(x),
        "round" => System.Math.Round(x, MidpointRounding.ToEven), // NumPy round == banker's
    ...
    public void Fused_Chained_FloorOfProduct_MatchesNumPy()
    {
        ...
        for (int i = 0; i < n; i++)
            r.GetDouble(i).Should().Be(System.Math.Floor(src[i] * src[i]),
