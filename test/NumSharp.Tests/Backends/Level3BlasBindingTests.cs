    ...
    public unsafe class Level3BlasBindingTests
    {
        ...
        public void DegenerateAndEmptySizes_QuickReturnCorrectly()
        {
            ...
            foreach (bool dbl in new[] { true, false })
            {
                ...
                {
                    int adim = System.Math.Max(m, 1);
                    double[] a = RandTri(rng, adim), b = Rand(rng, m, n);
                    double[] got = NativeTriangular(dbl, false, CBlasSide.Left, CBlasUpLo.Upper,
                        CBlasTranspose.NoTrans, CBlasDiag.NonUnit, m, n, a, adim, adim, b, System.Math.Max(n, 1), true);
        ...
        {
            RequireBlas();
            var rng = new System.Random(solve ? 202 : 101);
            var fails = new List<string>();
        ...
        private static double[] Rand(System.Random rng, int r, int c)
        {
            var a = new double[r * c];
            for (int i = 0; i < a.Length; i++) a[i] = System.Math.Round(rng.NextDouble() * 2 - 1, 3);
            return a;
        }

        private static double[] RandTri(System.Random rng, int n)
        {
            var a = Rand(rng, n, n);
        ...
        private static void Compare(List<string> fails, string tag, double[] got, double[] want, bool dbl, bool[] mask)
        {
            ...
            for (int i = 0; i < want.Length; i++)
            {
                ...
                double d = System.Math.Abs(got[i] - want[i]) / (1 + System.Math.Abs(want[i]));
                if (d > maxd) maxd = d;
            }
