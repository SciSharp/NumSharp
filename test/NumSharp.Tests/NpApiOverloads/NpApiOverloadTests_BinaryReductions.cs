...
public class NpApiOverloadTests_BinaryReductions
{
    ...
    public void Arctan2_TwoArrays_Compiles()
    {
        ...
        result.Should().NotBeNull();
        result.GetDouble(0).Should().BeApproximately(System.Math.PI / 2, 1e-10);   // arctan2(1, 0) = pi/2
        result.GetDouble(1).Should().BeApproximately(0.0, 1e-10);            // arctan2(0, 1) = 0
        result.GetDouble(2).Should().BeApproximately(-System.Math.PI / 2, 1e-10);  // arctan2(-1, 0) = -pi/2
    }
    ...
    public void Std_WithKeepdimsDdofDtype_Compiles()
    {
        ...
        result.GetDouble(0).Should().BeApproximately(System.Math.Sqrt(2.0), 1e-10);
    }
