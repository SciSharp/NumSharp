    ...
    public class NpFftFloat32ParityTest : TestClass
    {
        ...
        private static Complex[] DirectDft(Complex[] x, bool forward)
        {
            ...
            for (int k = 0; k < n; k++)
            {
                ...
                for (int m = 0; m < n; m++)
                {
                    double ang = sign * 2.0 * System.Math.PI * k * m / n;
                    s += x[m] * new Complex(System.Math.Cos(ang), System.Math.Sin(ang));
                }
                y[k] = s;
            }
            return y;
        ...
        private static Complex[] DeterministicComplex(int n)
        {
            ...
            var x = new Complex[n];
            for (int m = 0; m < n; m++)
                x[m] = new Complex(System.Math.Sin(0.7 * m + 0.1), System.Math.Cos(0.37 * m - 0.2));
            return x;
        }
        ...
        public void Fft_Float32_ReturnsComplex128_ValuesNearDouble()
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                ...
                double scale = System.Math.Max(1.0, System.Math.Abs(yd[k].Real) + System.Math.Abs(yd[k].Imaginary));
                System.Math.Abs(yv[k].Real - yd[k].Real).Should().BeLessThan(1e-5 * scale);
                System.Math.Abs(yv[k].Imaginary - yd[k].Imaginary).Should().BeLessThan(1e-5 * scale);
            }
        }
