...
namespace NumSharp.Tests
{
    ...
    public partial class OpenBugs : TestClass
    {
        // ================================================================
        //
        //  BUG 1 (RESOLVED): ToString on broadcasted sliced arrays showed
        //  wrong values (reversed slice printed forward, step slice scrambled,
        //  sliced-column dropped its last row, double-slice showed garbage).
        //  Fixed: array_str/array_repr now walk a broadcast-of-a-view in
        //  logical C-order via Shape.TransformOffset, so a stride=0 axis over
        //  non-trivial (negative / >1 / column) view strides renders correctly.
        //  The four reproductions are now byte-exact PASSING regression tests
        //  (str AND repr) in Casting/np.ArrayPrint.ParityTests.cs (region
        //  "broadcast-of-a-view") and Casting/np.ToString.BattleTests.cs
        ...
        public void Bug_Std_Axis0_ColBroadcast_WrongValues()
        {
            ...
            var expected_std = System.Math.Sqrt(2.0 / 3.0);
