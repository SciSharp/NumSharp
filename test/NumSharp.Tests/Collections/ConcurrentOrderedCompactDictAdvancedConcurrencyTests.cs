// MIRROR of ConcurrentOrderedDictAdvancedConcurrencyTests.cs for ConcurrentOrderedCompactDict — the two types share one public surface and one
...
{
    /// <summary>
    ///     The second, adversarial tier of concurrency stress for <see cref="ConcurrentOrderedCompactDict{TKey,TValue}" />.
    ///     Where <see cref="ConcurrentOrderedCompactDictConcurrencyTests" /> fires broad storms, every scenario here
    ...
    ///     integrity under ordered churn (the guarantee <see cref="ConcurrentOrderedCompactDict{TKey,TValue}.TryRemoveSwapBack" />
    ...
    /// </remarks>
    [TestClass]
    public class ConcurrentOrderedCompactDictAdvancedConcurrencyTests
    {
        ...
        public void Gun_TwoInstancesOfTheSameClosedType_ShareNoObservableState()
        {
            ...
            var b = new ConcurrentOrderedCompactDict<int, long>();
            long done = 0;
        ...
        public void Reentrancy_WriteInsideAddRangeSource_ThrowsLockRecursion_AndAppliedPrefixIsConsistent()
        {
            ...
            static IEnumerable<KeyValuePair<int, int>> EvilSource(ConcurrentOrderedCompactDict<int, int> target)
            {
        ...
        public void Reentrancy_EveryMutator_IsGuardedInsideThePredicate()
        {
            ...
            var attacks = new (string Name, Action<ConcurrentOrderedCompactDict<int, int>> Attack)[]
            {
            ...
            foreach ((string name, Action<ConcurrentOrderedCompactDict<int, int>> attack) in attacks)
            {
                var d = new ConcurrentOrderedCompactDict<int, int>();
                for (int k = 0; k < 4; k++)
                {
        ...
        public void Reentrancy_EvilComparerWritingDuringALockedLookup_Throws_AndStateIsUntouched()
        {
            ...
            var d = new ConcurrentOrderedCompactDict<string, int>(comparer);
            comparer.Owner = d;
            d.Add("a", 1);
            d.Add("b", 2);
        ...
        private sealed class EvilComparer : IEqualityComparer<string>
        {
            ...
            internal ConcurrentOrderedCompactDict<string, int>? Owner;
