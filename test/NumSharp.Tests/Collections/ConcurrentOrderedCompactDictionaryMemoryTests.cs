// MIRROR of ConcurrentOrderedDictionaryMemoryTests.cs for ConcurrentOrderedCompactDictionary — the two types share one public surface and one
...
{
    /// <summary>
    ///     Memory-contract pins for <see cref="ConcurrentOrderedCompactDictionary{TKey,TValue}" /> — the allocation and
    ...
    ///     and the framework dictionary live in <c>ConcurrentOrderedDictionary.COMPACT.md § Memory analysis</c>; these tests
    ///     pin only the collection's OWN contracts, which hold on any 64-bit runtime and in both Debug and Release.
    /// </summary>
    [TestClass]
    public class ConcurrentOrderedCompactDictionaryMemoryTests
    {
        /// <summary>
        ...
        {
            var d = new ConcurrentOrderedCompactDictionary<int, int>(4_096);
        ...
        public void AtomicValueReplace_IsAllocationFree()
        {
            ...
            var d = new ConcurrentOrderedCompactDictionary<int, long>(1_024);
        ...
        public void PresizedAppend_AllocatesOnlyTheHashNode()
        {
            ...
            var d = new ConcurrentOrderedCompactDictionary<int, int>(200_000);
            int next = 0;
            Action adds = () =>
            {
                for (int i = 0; i < 1_000; i++)
            ...
            Assert.AreEqual(0, perAdd, $"a presized in-capacity append allocated {perAdd} B — the compact table has no per-entry node, an in-capacity append must allocate NOTHING");
        }
        ...
        public void TailPop_And_SwapBack_AllocateOnlyOneSnapshotHolder()
        {
            ...
            var d = new ConcurrentOrderedCompactDictionary<int, int>(64_000);
        ...
        public void InteriorRemove_PaysCapacitySizedArrays_TailPopDoesNot()
        {
            ...
            var d = new ConcurrentOrderedCompactDictionary<int, int>(Capacity);
        ...
        public void NonAtomicReplace_AddRangeBatchesManyClonesIntoOne()
        {
            ...
            var d = new ConcurrentOrderedCompactDictionary<int, decimal>(Capacity);
            for (int i = 0; i < Capacity; i++)
            {
                d.Add(i, i);
            }
        ...
        private static WeakReference AddThenTailPop(ConcurrentOrderedCompactDictionary<int, object> d)
        {
        ...
        {
            var d = new ConcurrentOrderedCompactDictionary<int, object>();
            d.Add(0, new object());
            d.Add(1, new object());
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static WeakReference AddThenInteriorRemove(ConcurrentOrderedCompactDictionary<int, object> d)
        {
            var value = new object();
            d.Add(10, value);
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static WeakReference AddThenClear(ConcurrentOrderedCompactDictionary<int, object> d)
        {
            var value = new object();
            d.Add(0, value);
            var wr = new WeakReference(value);
        ...
        /// </remarks>
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static WeakReference HoldEnumeratorCheckPinnedThenDrop(ConcurrentOrderedCompactDictionary<int, object> d)
        {
            var value = new object();
            d.Add(0, value);
            var wr = new WeakReference(value);
