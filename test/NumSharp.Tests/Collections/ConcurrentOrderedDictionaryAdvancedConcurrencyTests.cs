...
{
    /// <summary>
    ///     The second, adversarial tier of concurrency stress for <see cref="ConcurrentOrderedDictionary{TKey,TValue}" />.
    ///     Where <see cref="ConcurrentOrderedDictionaryConcurrencyTests" /> fires broad storms, every scenario here
    ...
    ///     integrity under ordered churn (the guarantee <see cref="ConcurrentOrderedDictionary{TKey,TValue}.TryRemoveSwapBack" />
    ...
    /// </remarks>
    [TestClass]
    public class ConcurrentOrderedDictionaryAdvancedConcurrencyTests
    {
        ...
        public void Gun_TwoInstancesOfTheSameClosedType_ShareNoObservableState()
        {
            ...
            int threads = GunThreads;
            var a = new ConcurrentOrderedDictionary<int, long>();
            var b = new ConcurrentOrderedDictionary<int, long>();
            long done = 0;
        ...
        public void Reentrancy_WriteInsideAddRangeSource_ThrowsLockRecursion_AndAppliedPrefixIsConsistent()
        {
            ...
            static IEnumerable<KeyValuePair<int, int>> EvilSource(ConcurrentOrderedDictionary<int, int> target)
            {
        ...
        public void Reentrancy_EveryMutator_IsGuardedInsideThePredicate()
        {
            ...
            var attacks = new (string Name, Action<ConcurrentOrderedDictionary<int, int>> Attack)[]
            {
            ...
            foreach ((string name, Action<ConcurrentOrderedDictionary<int, int>> attack) in attacks)
            {
                var d = new ConcurrentOrderedDictionary<int, int>();
                for (int k = 0; k < 4; k++)
                {
        ...
        public void Reentrancy_EvilComparerWritingDuringALockedLookup_Throws_AndStateIsUntouched()
        {
            ...
            var d = new ConcurrentOrderedDictionary<string, int>(comparer);
            comparer.Owner = d;
            d.Add("a", 1);
            d.Add("b", 2);
        ...
        private sealed class EvilComparer : IEqualityComparer<string>
        {
            ...
            internal ConcurrentOrderedDictionary<string, int>? Owner;
