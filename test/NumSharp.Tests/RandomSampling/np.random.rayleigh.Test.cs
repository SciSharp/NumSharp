...
namespace NumSharp.Tests.RandomSampling
{
    ...
    public class NpRandomRayleighTests : TestClass
    {
        ...
        public void Rayleigh_DefaultParameters_HasCorrectStatistics()
        {
            ...
            double expectedMean = System.Math.Sqrt(System.Math.PI / 2.0); // ≈ 1.2533

            // Allow some tolerance for statistical sampling
            Assert.IsTrue(System.Math.Abs(mean - expectedMean) < 0.05, $"Mean should be near {expectedMean}, got {mean}");
            Assert.IsTrue(System.Math.Abs(std - 0.655) < 0.05, $"Std should be near 0.655, got {std}");
        }
        ...
        public void Rayleigh_WithScale_TransformsCorrectly()
        {
            ...
            var mean = (double)np.mean(samples);
            double expectedMean = scale * System.Math.Sqrt(System.Math.PI / 2.0);

            Assert.IsTrue(System.Math.Abs(mean - expectedMean) < 0.1, $"Mean should be near {expectedMean}, got {mean}");
        }
