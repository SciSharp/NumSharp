    ...
    public class NpRandomDirichletTests : TestClass
    {
        ...
        public void Dirichlet_EachRowSumsToOne()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.IsTrue(System.Math.Abs(rowSum - 1.0) < 1e-10, $"Row {i} sum should be 1.0, got {rowSum}");
            }
        }
        ...
        public void Dirichlet_HasCorrectMean()
        {
            ...
            double alphaSum = 6.0;
            Assert.IsTrue(System.Math.Abs(means[0] - 1.0 / alphaSum) < 0.01, $"Mean[0] should be ~0.167, got {means[0]}");
            Assert.IsTrue(System.Math.Abs(means[1] - 2.0 / alphaSum) < 0.01, $"Mean[1] should be ~0.333, got {means[1]}");
            Assert.IsTrue(System.Math.Abs(means[2] - 3.0 / alphaSum) < 0.01, $"Mean[2] should be ~0.5, got {means[2]}");
        }
        ...
        public void Dirichlet_UniformAlpha_HasEqualMeans()
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                Assert.IsTrue(System.Math.Abs(means[j] - 1.0 / 3.0) < 0.02, $"Mean[{j}] should be ~0.333, got {means[j]}");
            }
        }
        ...
        public void Dirichlet_NumPy_SamplesSumToOne()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.IsTrue(System.Math.Abs(sum - 1.0) < 1e-10, $"Sample {i} should sum to 1.0");
            }
        }
