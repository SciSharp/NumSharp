...
namespace NumSharp.Tests.RandomSampling
{
    ...
    public class NpRandomLaplaceTests : TestClass
    {
        ...
        public void Laplace_DefaultParameters_HasCorrectStatistics()
        {
            ...
            Assert.IsTrue(System.Math.Abs(mean) < 0.05, $"Mean should be near 0, got {mean}");
            Assert.IsTrue(System.Math.Abs(std - System.Math.Sqrt(2)) < 0.05, $"Std should be near sqrt(2)≈1.414, got {std}");
            Assert.IsTrue(System.Math.Abs(variance - 2.0) < 0.1, $"Variance should be near 2, got {variance}");
        }
        ...
        public void Laplace_WithLocScale_TransformsCorrectly()
        {
            ...
            Assert.IsTrue(System.Math.Abs(mean - loc) < 0.1, $"Mean should be near {loc}, got {mean}");
            Assert.IsTrue(System.Math.Abs(variance - expectedVariance) < 0.5, $"Variance should be near {expectedVariance}, got {variance}");
        }
