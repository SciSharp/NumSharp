...
namespace NumSharp.Tests.RandomSampling
{
    ...
    public class NpRandomStandardCauchyTest : TestClass
    {
        ...
        public void StandardCauchy_MedianNearZero()
        {
            ...
            Assert.IsTrue(System.Math.Abs(median) < 0.05, $"Median {median} should be close to 0");
        }
        ...
        public void StandardCauchy_InterquartileRange()
        {
            ...
            Assert.IsTrue(System.Math.Abs(iqr - 2.0) < 0.1, $"IQR {iqr} should be close to 2");
            // Q1 should be approximately -1
            Assert.IsTrue(System.Math.Abs(q1 + 1.0) < 0.05, $"Q1 {q1} should be close to -1");
            // Q3 should be approximately 1
            Assert.IsTrue(System.Math.Abs(q3 - 1.0) < 0.05, $"Q3 {q3} should be close to 1");
        }
