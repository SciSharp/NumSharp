...
namespace NumSharp.Tests.RandomSampling
{
    ...
    public class NpRandomGumbelTests : TestClass
    {
        ...
        public void Gumbel_DefaultParameters_HasCorrectStatistics()
        {
            ...
            double expectedStd = System.Math.PI / System.Math.Sqrt(6); // ≈ 1.2825

            // Allow some tolerance for statistical sampling
            Assert.IsTrue(System.Math.Abs(mean - EulerGamma) < 0.05, $"Mean should be near {EulerGamma}, got {mean}");
            Assert.IsTrue(System.Math.Abs(std - expectedStd) < 0.05, $"Std should be near {expectedStd}, got {std}");
        }
        ...
        public void Gumbel_WithLocScale_TransformsCorrectly()
        {
            ...
            Assert.IsTrue(System.Math.Abs(mean - expectedMean) < 0.15, $"Mean should be near {expectedMean}, got {mean}");
        }
