...
{
    /// <summary>
    /// Battletest suite for NDIter_CreateCompatibleStrides (nditer_api.c:1058).
    ///
    ...
    /// </summary>
    [TestClass]
    public class NDIterCreateCompatibleStridesTests
    {
        [TestMethod]
        ...
        {
            var a = np.arange(6).reshape(2, 3).astype(np.int32);
            using var it = NDIterRef.New(a); // No MULTI_INDEX
        ...
        public unsafe void CreateCompatibleStrides_WithDontNegateStrides_Succeeds()
        {
            ...
            var a = np.arange(5).astype(np.int32)["::-1"];
            using var it = NDIterRef.New(a,
                flags: NDIterGlobalFlags.MULTI_INDEX | NDIterGlobalFlags.DONT_NEGATE_STRIDES,
                order: NPY_ORDER.NPY_KEEPORDER);
