...
{
    /// <summary>
    /// Coverage for the 0-dimensional / dropped-all-axes NDIter configuration —
    /// i.e. <c>NDIterRef.AdvancedNew(..., opAxesNDim: 0, opAxes: [[]])</c>, the shape produced
    /// by the IterAllButAxis pattern (NumPy's <c>NDIter_RemoveAxis</c>) when the operand is 1-D
    /// and its only axis is dropped.
    ///
    /// NumPy semantics: removing every iterated axis leaves a 0-dimensional iterator with
    /// <c>NDIter_GetIterSize() == 1</c> — exactly ONE iteration whose data pointer is the operand
    ...
    /// </summary>
    [TestClass]
    public class NDIterZeroDimOpAxesTests
    {
        ...
        private static unsafe Probe RunProbe(NDArray[] ops, NDIterPerOpFlags[] flags, int oaNdim, int[][] opAxes)
        {
            Probe pr = default;
        ...
        public unsafe void OneD_DropOnlyAxis_IteratesExactlyOnce()
        {
            ...
            var pr = RunProbe(new[] { a }, new[] { NDIterPerOpFlags.READONLY }, 0, new[] { new int[0] });
        ...
        public unsafe void OneD_DropOnlyAxis_SingleCall_BasePointerWalksWholeLine()
        {
            ...
            var iter = NDIterRef.AdvancedNew(1, new[] { a }, NDIterGlobalFlags.None,
                NPY_ORDER.NPY_CORDER, NPY_CASTING.NPY_NO_CASTING, new[] { NDIterPerOpFlags.READONLY }, null, 0, new[] { new int[0] });
            try
            {
                NDInnerLoopFunc k = (p, s, c, a2) =>
                {
        ...
        public unsafe void OneD_DropOnlyAxis_ReadWrite_SingleCall_WritesWholeLine()
        {
            ...
            var iter = NDIterRef.AdvancedNew(1, new[] { a }, NDIterGlobalFlags.None,
                NPY_ORDER.NPY_CORDER, NPY_CASTING.NPY_NO_CASTING, new[] { NDIterPerOpFlags.READWRITE }, null, 0, new[] { new int[0] });
            try
            {
                NDInnerLoopFunc k = (p, s, c, a2) =>
                {
        ...
        public unsafe void TwoOperand_DropOnlyAxis_IteratesOnce_BothBasesValid()
        {
            ...
            var iter = NDIterRef.AdvancedNew(2, new[] { src, dst }, NDIterGlobalFlags.None,
                NPY_ORDER.NPY_CORDER, NPY_CASTING.NPY_NO_CASTING,
                new[] { NDIterPerOpFlags.READONLY, NDIterPerOpFlags.WRITEONLY }, null, 0, new[] { new int[0], new int[0] });
            try
            {
                NDInnerLoopFunc k = (p, s, c, a2) =>
                {
        ...
        public unsafe void Regression_NoOpAxes_1D_IteratesNTimes()
        {
            ...
            var pr = RunProbe(new[] { a }, new[] { NDIterPerOpFlags.READONLY }, -1, null);
        ...
        public unsafe void Regression_OpAxes_2D_DropAxis1_IteratesPerRow()
        {
            ...
            var pr = RunProbe(new[] { a }, new[] { NDIterPerOpFlags.READWRITE }, 1, new[] { new[] { 0 } });
        ...
        {
            var a = np.arange(40).reshape(8, 5).astype(np.int32);
            var pr = RunProbe(new[] { a }, new[] { NDIterPerOpFlags.READWRITE }, 1, new[] { new[] { 1 } });
