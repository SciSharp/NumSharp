...
namespace NumSharp.UnitTest.Backends.Iterators
{
    ...
    ///   • NDExpr corners (deep nesting, input reuse, constant-only)
    ///   • Cache behavior + argument validation
    /// </summary>
    [TestClass]
    public unsafe class NDIterCustomOpEdgeCaseTests
    {
        // =====================================================================
        // Common helpers
        // =====================================================================
        ...
        private static NDIterRef Iter(NDArray a, NDArray b, NDArray c)
        ...
        private static void RunLinear(int count)
        {
            ...
            var expr = NDExpr.Input(0) * NDExpr.Const(2.0f) + NDExpr.Const(1.0f);
        ...
        public void Strided_EveryOther_ScalarFallback()
        {
            ...
            var expr = NDExpr.Input(0) * NDExpr.Input(0);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        public void Broadcast_StrideZero_Input()
        {
            ...
            var expr = NDExpr.Input(0) * NDExpr.Const(3.0f);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        [TestMethod]
        public void NDExpr_DeeplyNested_TenAdditions()
        {
            ...
            NDExpr e = NDExpr.Input(0);
            for (int k = 1; k <= 10; k++)
                e = e + NDExpr.Const((float)k);
        ...
        [TestMethod]
        public void NDExpr_InputReusedThreeTimes()
        {
            // a*a + a = a² + a
            var a = np.arange(16).astype(np.float32);
            var b = np.empty(new Shape(16), np.float32);
            ...
            using var iter = Iter(a, b);
            var expr = NDExpr.Input(0) * NDExpr.Input(0) + NDExpr.Input(0);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        [TestMethod]
        public void NDExpr_ConstantOnly_IgnoresInput()
        {
            ...
            using var iter = Iter(a, b);
            iter.ExecuteExpression(NDExpr.Const(42.0f),
        ...
        [TestMethod]
        public void NDExpr_NegativeConstant()
        {
            var a = np.arange(8).astype(np.float32);
            var b = np.empty(new Shape(8), np.float32);

            using var iter = Iter(a, b);
            var expr = NDExpr.Input(0) + NDExpr.Const(-100.0f);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        [TestMethod]
        public void NDExpr_DivideByConstant()
        {
            var a = np.arange(1, 17).astype(np.float64);
            var b = np.empty(new Shape(16), np.float64);

            using var iter = Iter(a, b);
            var expr = NDExpr.Input(0) / NDExpr.Const(4.0);
        ...
        [TestMethod]
        public void NDExpr_UnaryChain_AbsThenNegate()
        {
            var a = np.array(new float[] { -3, 4, -5, 6, -7, 8 });
            var b = np.empty(new Shape(6), np.float32);

            using var iter = Iter(a, b);
            var expr = -NDExpr.Abs(NDExpr.Input(0));                // -|a|
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        public void NDExpr_InputIndexOutOfRange_Throws()
        {
            ...
            var expr = NDExpr.Input(5);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single);
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void NDExpr_InputNegativeIndex_ThrowsOnConstruction()
        {
            NDExpr.Input(-1);
        }
        ...
        public void Cache_AutoDerivedKey_StructurallyEquivalentTreesShareDelegate()
        {
            ...
            var e1 = NDExpr.Input(0) * NDExpr.Const(5.0f);
            var e2 = NDExpr.Input(0) * NDExpr.Const(5.0f);
        ...
        public void Cache_DistinctStructure_DistinctEntries()
        {
            ...
            var e1 = NDExpr.Input(0) * NDExpr.Const(2.0f);
            var e2 = NDExpr.Input(0) * NDExpr.Const(3.0f);      // different constant
            var e3 = NDExpr.Input(0) + NDExpr.Const(2.0f);      // different op
        ...
        public void Cache_SameTreeDifferentInputTypes_DistinctEntries()
        {
            ...
            var tree = NDExpr.Input(0) + NDExpr.Const(1.0);
        ...
        public void MultiDim_Contiguous3D_CoalescesToSimd()
        {
            ...
            using var iter = Iter(a, b);
            var expr = NDExpr.Input(0) * NDExpr.Const(2.0f);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        {
            // [::-1] gives a view with negative stride. NDIter flips these
        ...
        public void StridedOutput_ViewOfEveryOther()
        {
            ...
            var expr = NDExpr.Input(0) * NDExpr.Const(-1.0f);
            iter.ExecuteExpression(expr, new[] { NPTypeCode.Single }, NPTypeCode.Single,
        ...
        [TestMethod]
        public void NDExpr_Int32_ArithmeticChain()
        {
            var a = np.arange(16).astype(np.int32);
            var b = np.empty(new Shape(16), np.int32);

            using var iter = Iter(a, b);
            var expr = NDExpr.Input(0) * NDExpr.Const(3) + NDExpr.Const(7);
        ...
        [TestMethod]
        public void NDExpr_Int16_OverflowWraps()
        {
        ...
        [TestMethod]
        public void NDExpr_UpcastIntToFloat_ViaInputConversion()
        {
            ...
            using var iter = Iter(a, b);
            var expr = NDExpr.Input(0) * NDExpr.Const(0.5f);
        ...
        [TestMethod]
        public void NDExpr_30LevelNested()
        {
            var a = np.arange(8).astype(np.float32);
            var b = np.empty(new Shape(8), np.float32);
            ...
            NDExpr e = NDExpr.Input(0);
            for (int k = 1; k <= 30; k++) e = e + NDExpr.Const(1.0f);   // adds 30
        ...
        // =====================================================================
        // NDExpr: auto-derived cache key with default null argument
        // =====================================================================

        [TestMethod]
        public void NDExpr_AutoKey_NullParam_ProducesValidDelegate()
        {
