...
/// </summary>
[TestClass]
public class NDIterWriteMaskedExecutionTests
{
    private static NDArray Mask8() =>
        np.array(new[] { true, false, true, false, true, false, true, false });

    private static NDIterPerOpFlags[] MaskedPair() => new[]
    {
    ...
    public void MaskedWrite_Unbuffered_MaskNotEnforced()
    {
        ...
        using (var it = NDIterRef.MultiNew(2, new[] { a, Mask8() },
    ...
    public void MaskedWrite_BufferedButLinearSameDtype_MaskNotEnforced()
    {
        ...
        var a = np.zeros(new Shape(8), np.float64);
        using (var it = NDIterRef.MultiNew(2, new[] { a, Mask8() },
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
            MaskedPair()))
        {
            do { it.SetValue(99.0, 0); } while (it.Iternext());
        }
    ...
    public void MaskedWrite_BufferedCast_MaskEnforced()
    {
        ...
        var a = np.zeros(new Shape(8), np.float64);
        using (var it = NDIterRef.MultiNew(2, new[] { a, Mask8() },
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAME_KIND_CASTING,
            MaskedPair(),
    ...
    public void MaskedWrite_WriteOnly_UnmaskedSlotsKeepOriginals()
    {
        ...
        using (var it = NDIterRef.MultiNew(2, new[] { b, Mask8() },
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAME_KIND_CASTING,
            new[]
            {
                NDIterPerOpFlags.WRITEONLY | NDIterPerOpFlags.WRITEMASKED,
                NDIterPerOpFlags.READONLY | NDIterPerOpFlags.ARRAYMASK,
            },
            new[] { NPTypeCode.Single, NPTypeCode.Empty }))
        {
    ...
    public void MaskedWrite_StridedMask_MaskItselfBuffered()
    {
        ...
        using (var it = NDIterRef.MultiNew(2, new[] { a, m2 },
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAME_KIND_CASTING,
            MaskedPair(),
            new[] { NPTypeCode.Single, NPTypeCode.Empty }))
        {
    ...
    public void MaskedWrite_BroadcastMaskRow_Over2D()
    {
        ...
        using (var it = NDIterRef.MultiNew(2, new[] { a, mrow },
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAME_KIND_CASTING,
            MaskedPair(),
            new[] { NPTypeCode.Single, NPTypeCode.Empty }))
        {
    ...
    public void MaskedWrite_MultiWindow_AllWindowsFlushMasked()
    {
        ...
        using (var it = NDIterRef.MultiNew(2, new[] { a, mN },
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAME_KIND_CASTING,
            MaskedPair(),
            new[] { NPTypeCode.Single, NPTypeCode.Empty }))
        {
    ...
    public void MaskedWrite_Uint8Mask_NonzeroIsTrue()
    {
        ...
        using (var it = NDIterRef.MultiNew(2, new[] { a, m8 },
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAME_KIND_CASTING,
            MaskedPair(),
            new[] { NPTypeCode.Single, NPTypeCode.Empty }))
        {
    ...
    public void WriteMasked_ReadOnly_ThrowsNumPyText()
    {
        var ex = Assert.ThrowsException<ArgumentException>(() =>
            NDIterRef.MultiNew(2, new[] { np.zeros(new Shape(8), np.float64), Mask8() },
                NDIterGlobalFlags.None, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                new[]
                {
                    NDIterPerOpFlags.READONLY | NDIterPerOpFlags.WRITEMASKED,
                    NDIterPerOpFlags.READONLY | NDIterPerOpFlags.ARRAYMASK,
                }).Dispose());
    ...
    public void WriteMaskedPairing_ThrowsNumPyTexts()
    {
        ...
        var e2 = Assert.ThrowsException<ArgumentException>(() =>
            NDIterRef.MultiNew(2, new[] { a, m },
                NDIterGlobalFlags.None, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                new[]
                {
                    NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.WRITEMASKED | NDIterPerOpFlags.ARRAYMASK,
        ...
        var e3 = Assert.ThrowsException<ArgumentException>(() =>
            NDIterRef.MultiNew(3, new[] { a, m, m.copy() },
                NDIterGlobalFlags.None, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                new[]
                {
                    NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.WRITEMASKED,
    ...
    public void ArrayMask_WouldReduce_ThrowsNumPyText()
    {
        ...
        var ex = Assert.ThrowsException<ArgumentException>(() =>
            NDIterRef.MultiNew(2, new[] { a, mr },
                NDIterGlobalFlags.REDUCE_OK, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                new[]
                {
                    NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.WRITEMASKED,
                    NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.ARRAYMASK,
                }).Dispose());
    ...
    public void WriteMaskedReduce_MaskBroadcastsWider_StandardPath_Throws()
    {
        ...
        var ex = Assert.ThrowsException<InvalidOperationException>(() =>
            NDIterRef.MultiNew(2, new[] { asm, mbig },
                NDIterGlobalFlags.REDUCE_OK, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                MaskedPair()).Dispose());
    ...
    public void NonBoolMask_BufferedWrite_ThrowsNumPyText()
    {
        ...
        var ex = Assert.ThrowsException<NotSupportedException>(() =>
            NDIterRef.MultiNew(2, new[] { a, m16 },
                NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAME_KIND_CASTING,
                MaskedPair(),
    ...
    public void NonBoolMask_Unbuffered_NoError()
    {
        ...
        using var it = NDIterRef.MultiNew(2, new[] { a, m16 },
            NDIterGlobalFlags.None, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
            MaskedPair());
    ...
    public void Virtual_NullOp_AllocatesCommonDtype()
    {
        ...
        using var it = NDIterRef.MultiNew(2, ops,
            NDIterGlobalFlags.None, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
            new[]
            {
                NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.VIRTUAL,
                NDIterPerOpFlags.READONLY,
            });
    ...
    public void Virtual_RequestedDtype_IsDiscarded()
    {
        ...
        using var it = NDIterRef.MultiNew(2, ops,
            NDIterGlobalFlags.BUFFERED, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_UNSAFE_CASTING,
            new[]
            {
                NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.VIRTUAL,
                NDIterPerOpFlags.READONLY,
    ...
    public void Virtual_PlusAllocate_HonorsRequestedDtype()
    {
        ...
        using var it = NDIterRef.MultiNew(2, ops,
            ...
            new[]
            {
                NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.VIRTUAL | NDIterPerOpFlags.ALLOCATE,
                NDIterPerOpFlags.READONLY,
            },
    ...
    public void Virtual_WithoutReadwrite_ThrowsNumPyText()
    {
        ...
        var ex = Assert.ThrowsException<ArgumentException>(() =>
            NDIterRef.MultiNew(2, new NDArray[] { null, src },
                NDIterGlobalFlags.None, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                new[]
                {
                    NDIterPerOpFlags.READONLY | NDIterPerOpFlags.VIRTUAL,
                    NDIterPerOpFlags.READONLY,
                }).Dispose());
    ...
    public void Virtual_OnNonNullOperand_ThrowsNumPyText()
    {
        ...
        var ex = Assert.ThrowsException<ArgumentException>(() =>
            NDIterRef.MultiNew(2, new[] { a, a.copy() },
                NDIterGlobalFlags.None, NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
                new[]
                {
                    NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.VIRTUAL,
    ...
    public void Virtual_ArrayMask_DefaultsToBool()
    {
        ...
        using var it = NDIterRef.MultiNew(2, ops,
            ...
            {
                NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.WRITEMASKED,
                NDIterPerOpFlags.READWRITE | NDIterPerOpFlags.VIRTUAL | NDIterPerOpFlags.ARRAYMASK,
            });
