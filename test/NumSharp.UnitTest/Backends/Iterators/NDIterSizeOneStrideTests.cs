...
namespace NumSharp.UnitTest.Backends.Iterators
{
    ...
    /// </summary>
    [TestClass]
    public class NDIterSizeOneStrideTests
    {
        ...
        public void WriteBroadcast_WriteOnlyWithReduceOk_Throws()
        {
            ...
            var ex = Assert.ThrowsException<ArgumentException>(() =>
            {
                using var iter = NDIterRef.MultiNew(
                    nop: 2,
                    op: new[] { a, outArr },
                    flags: NDIterGlobalFlags.REDUCE_OK,
                    order: NPY_ORDER.NPY_KEEPORDER,
                    casting: NPY_CASTING.NPY_SAFE_CASTING,
        ...
        public void ParallelSafe_SingleWriteWithCopyIfOverlap_IsSet()
        {
            ...
            using var iter = NDIterRef.MultiNew(
                nop: 3,
                op: new[] { a, b, dst },
                flags: NDIterGlobalFlags.COPY_IF_OVERLAP,
                order: NPY_ORDER.NPY_KEEPORDER,
                casting: NPY_CASTING.NPY_SAFE_CASTING,
                opFlags: new[]
                {
        ...
        public void ParallelSafe_ReduceIterator_NotSet()
        {
            ...
            using var iter = NDIterRef.MultiNew(
                nop: 2,
                op: new[] { a, outArr },
                flags: NDIterGlobalFlags.REDUCE_OK | NDIterGlobalFlags.COPY_IF_OVERLAP,
                order: NPY_ORDER.NPY_KEEPORDER,
                casting: NPY_CASTING.NPY_SAFE_CASTING,
                opFlags: new[] { NDIterPerOpFlags.READONLY, NDIterPerOpFlags.READWRITE });
