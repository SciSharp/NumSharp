...
/// <summary>
/// COPY_IF_OVERLAP + memory-overlap solver tests (NDMemOverlap — a port of
/// NumPy's mem_overlap.c, and the FORCECOPY/write-back machinery in
/// NDIterRef construction/Dispose, NumPy nditer_constr.c:3083-3311).
///
...
/// </summary>
[TestClass]
public class NDIterOverlapTests
{
    private const NDIterPerOpFlags ELW = NDIterPerOpFlags.OVERLAP_ASSUME_ELEMENTWISE_PER_OP;
    ...
    public void Overlap_ExactAlias_NoCopy()
    {
        ...
        var a = Arange8();
        using (var iter = NDIterRef.MultiNew(3, new[] { a, a, a },
            NDIterGlobalFlags.EXTERNAL_LOOP | NDIterGlobalFlags.COPY_IF_OVERLAP,
            NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
    ...
    public void Overlap_InterleavedDisjoint_NoCopy_SolverProvesNo()
    {
        ...
        var even = a["::2"];
        var odd = a["1::2"];
        using (var iter = NDIterRef.MultiNew(3, new[] { even, odd, even },
            NDIterGlobalFlags.EXTERNAL_LOOP | NDIterGlobalFlags.COPY_IF_OVERLAP,
            NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
            new[] { NDIterPerOpFlags.READONLY | ELW, NDIterPerOpFlags.READONLY | ELW, NDIterPerOpFlags.WRITEONLY | ELW }))
        {
    ...
    {
        var rExact = NDMemOverlap.SolveMayShareMemory(u, v, -1);
        var rBounds = NDMemOverlap.SolveMayShareMemory(u, v, 0);
    ...
    public void Solver_EmptyArrays_NeverShare()
    {
        var a = np.arange(64).astype(np.float64);
        var empty = a["3:3"];
        Assert.AreEqual(MemOverlap.No, NDMemOverlap.SolveMayShareMemory(empty, a, -1));
        Assert.AreEqual(MemOverlap.No, NDMemOverlap.SolveMayShareMemory(a, empty, 0));
    }
    ...
    public void Solver_InternalOverlap()
    {
        ...
        Assert.AreEqual(MemOverlap.Yes, NDMemOverlap.SolveMayHaveInternalOverlap(bc, -1), "broadcast view");
        Assert.AreEqual(MemOverlap.No, NDMemOverlap.SolveMayHaveInternalOverlap(a, -1), "contiguous");
        Assert.AreEqual(MemOverlap.No, NDMemOverlap.SolveMayHaveInternalOverlap(a["::2"], -1), "strided");
        Assert.AreEqual(MemOverlap.No, NDMemOverlap.SolveMayHaveInternalOverlap(a["::-1"], -1), "reversed");
    }
    ...
    public void ExecuteBinary_StridedOutput_Throws()
    {
        ...
        {
            using var iter = NDIterRef.MultiNew(3, new[] { a["::2"], a["1::2"], c["::2"] },
    ...
    public void ExecuteBinary_OffsetContiguousOutput_Works()
    {
        ...
        var a = Arange8();
        using (var iter = NDIterRef.MultiNew(3, new[] { a[":-1"], a[":-1"], a["1:"] },
            NDIterGlobalFlags.EXTERNAL_LOOP | NDIterGlobalFlags.COPY_IF_OVERLAP,
            NPY_ORDER.NPY_KEEPORDER, NPY_CASTING.NPY_SAFE_CASTING,
            new[] { NDIterPerOpFlags.READONLY | ELW, NDIterPerOpFlags.READONLY | ELW, NDIterPerOpFlags.WRITEONLY | ELW }))
        {
