...
namespace NumSharp.UnitTest.Backends.Iterators
{
    ...
    public class NpyEvaluateTests
    {
        ...
        public void PerNode_Int32MultiplyWraps_BeforeFloatPromotion()
        {
            ...
            var r = np.evaluate((NpyExpr)a * b + c);
        ...
        public void Dtypes_SpecialUfuncResolvers()
        {
            ...
            Assert.AreEqual(NPTypeCode.Double, np.evaluate(NpyExpr.Divide(NpyExpr.Arr(i1), i1)).typecode);
            Assert.AreEqual(NPTypeCode.Double, np.evaluate(NpyExpr.Divide(NpyExpr.Arr(i4), i4)).typecode);

            // arctan2: tier float (i1→f16, i4→f64) — unlike divide's flat f64
            Assert.AreEqual(NPTypeCode.Half, np.evaluate(NpyExpr.ATan2(NpyExpr.Arr(i1), i1)).typecode);
            Assert.AreEqual(NPTypeCode.Double, np.evaluate(NpyExpr.ATan2(NpyExpr.Arr(i4), i4)).typecode);

            // power: plain promotion for ints; NEP50 i4+f4 crossing → f64
            Assert.AreEqual(NPTypeCode.Int32, np.evaluate(NpyExpr.Power(NpyExpr.Arr(i4), i4)).typecode);
            Assert.AreEqual(NPTypeCode.Double, np.evaluate(NpyExpr.Power(NpyExpr.Arr(f4), i4)).typecode);

            // unary float tiers: bool/i8→f16, i16→f32, i32+→f64
            Assert.AreEqual(NPTypeCode.Half, np.evaluate(NpyExpr.Sqrt(NpyExpr.Arr(i1))).typecode);
            Assert.AreEqual(NPTypeCode.Single, np.evaluate(NpyExpr.Sqrt(NpyExpr.Arr(i2))).typecode);
            Assert.AreEqual(NPTypeCode.Double, np.evaluate(NpyExpr.Sqrt(NpyExpr.Arr(i4))).typecode);

            // minimum follows plain NEP50 promotion: i4+f4 → f64
            Assert.AreEqual(NPTypeCode.Double, np.evaluate(NpyExpr.Min(NpyExpr.Arr(i4), NpyExpr.Arr(f4))).typecode);
        }
        ...
        public void Dtypes_BoolUfuncQuirks()
        {
            ...
            var sum = np.evaluate(NpyExpr.Add(NpyExpr.Arr(b), NpyExpr.Arr(b)));
            ...
            Assert.AreEqual(NPTypeCode.SByte, np.evaluate(NpyExpr.Power(NpyExpr.Arr(b), NpyExpr.Arr(b))).typecode);
            var mod = np.evaluate(NpyExpr.Mod(NpyExpr.Arr(b), NpyExpr.Arr(b)));
            Assert.AreEqual(NPTypeCode.SByte, mod.typecode);
            Assert.AreEqual((sbyte)0, mod.GetSByte(0)); // remainder(True,True) = 0
            var fdiv = np.evaluate(NpyExpr.FloorDivide(NpyExpr.Arr(b), NpyExpr.Arr(b)));
            ...
            Assert.AreEqual(NPTypeCode.SByte, np.evaluate(NpyExpr.Square(NpyExpr.Arr(b))).typecode);
            Assert.AreEqual(NPTypeCode.Boolean, np.evaluate(NpyExpr.Abs(NpyExpr.Arr(b))).typecode);
            var inv = np.evaluate(NpyExpr.BitwiseNot(NpyExpr.Arr(b)));
        ...
        public void Dtypes_InvertAndBitwise_FloatInputsRaise()
        {
            ...
            var invEx = Assert.ThrowsException<NotSupportedException>(
                () => np.evaluate(NpyExpr.BitwiseNot(NpyExpr.Arr(f4))));
            ...
            var andEx = Assert.ThrowsException<NotSupportedException>(
                () => np.evaluate(NpyExpr.BitwiseAnd(NpyExpr.Arr(f4), NpyExpr.Arr(f4))));
        ...
        public void WeakScalars_AdoptArrayDtype()
        {
            ...
            Assert.AreEqual(NPTypeCode.Int32, np.evaluate((NpyExpr)i4 + 2).typecode);       // i4+2 → i4
            Assert.AreEqual(NPTypeCode.Double, np.evaluate((NpyExpr)i4 + 2.5).typecode);    // i4+2.5 → f8
            Assert.AreEqual(NPTypeCode.Double, np.evaluate((NpyExpr)i4 / 2).typecode);      // i4/2 → f8
            Assert.AreEqual(NPTypeCode.Int32, np.evaluate(NpyExpr.Power(NpyExpr.Arr(i4), NpyExpr.Const(2))).typecode);
            Assert.AreEqual(NPTypeCode.Single, np.evaluate((NpyExpr)f4 + 2).typecode);      // f4+2 → f4
            Assert.AreEqual(NPTypeCode.Single, np.evaluate((NpyExpr)f4 + 2.5).typecode);    // f4+2.5 → f4
            Assert.AreEqual(NPTypeCode.Half, np.evaluate((NpyExpr)f2 + 2.5).typecode);      // f2+2.5 → f2
            Assert.AreEqual(NPTypeCode.Byte, np.evaluate((NpyExpr)u1 + 2).typecode);        // u1+2 → u1
            Assert.AreEqual(NPTypeCode.Int64, np.evaluate((NpyExpr)b1 + 2).typecode);       // bool+2 → i64
            Assert.AreEqual(NPTypeCode.Double, np.evaluate((NpyExpr)b1 + 2.5).typecode);    // bool+2.5 → f8
        }
        ...
        {
            var u1 = np.ones(new Shape(3), np.uint8);
            var ex = Assert.ThrowsException<OverflowException>(() => np.evaluate((NpyExpr)u1 + 300));
        ...
        public void Power_NegativeIntegerLiteralExponent_RaisesLikeNumPy()
        {
            ...
            var ex = Assert.ThrowsException<ArgumentException>(
                () => np.evaluate(NpyExpr.Power(NpyExpr.Arr(i4), NpyExpr.Const(-2))));
        ...
        public void Fused_NormalizedDifference_MatchesUnfused()
        {
            ...
            var fused = np.evaluate((NpyExpr.Arr(a) - b) / (NpyExpr.Arr(a) + b));
            var unfused = (a - b) / (a + b);
        ...
        public void Fused_ComparisonProducesBool_AndComposesIntoArithmetic()
        {
            ...
            var mask = np.evaluate(NpyExpr.Greater(NpyExpr.Arr(a), NpyExpr.Const(2.0)));
            ...
            var relu = np.evaluate((NpyExpr)a * NpyExpr.Greater(NpyExpr.Arr(a), 2.0));
        ...
        public void Fused_WhereNode_PromotesBranches_TestsCondAtOwnDtype()
        {
            ...
            var r = np.evaluate(NpyExpr.Where(NpyExpr.Arr(cond), NpyExpr.Arr(i4), NpyExpr.Arr(f4)));
            Assert.AreEqual(NPTypeCode.Double, r.typecode);
            ...
            var r2 = np.evaluate(NpyExpr.Where(NpyExpr.Arr(icond), NpyExpr.Arr(f4), NpyExpr.Const(0.0f)));
        ...
        public void Fused_BroadcastInputs()
        {
            ...
            var r = np.evaluate((NpyExpr)a + row);
        ...
        public void Fused_StridedAndTransposedInputs()
        {
            ...
            var r = np.evaluate((NpyExpr)even + odd);
            for (int i = 0; i < 20; i++)
            ...
            var rt = np.evaluate((NpyExpr)t * 2.0);
        ...
        public void Fused_PositionalOverload_AndBindingValidation()
        {
            ...
            var r = np.evaluate(NpyExpr.Input(0) * NpyExpr.Input(1), new[] { a, b });
            Assert.AreEqual(40.0, r.GetDouble(1));

            // constant-only tree → no arrays to iterate
            Assert.ThrowsException<ArgumentException>(() => np.evaluate(NpyExpr.Const(2) + NpyExpr.Const(3)));

            // mixing embedded leaves with a positional list
            Assert.ThrowsException<ArgumentException>(
                () => np.evaluate(NpyExpr.Arr(a) * NpyExpr.Input(0), new[] { b }));
        }
        ...
        public void Out_ReferenceIdentity_Cast_AndAliasing()
        {
            ...
            var r = np.evaluate((NpyExpr)a + b, @out: outArr);
            ...
            np.evaluate((NpyExpr)a + b, @out: outF4);
            ...
            np.evaluate((NpyExpr)x * 2 + 1, @out: x);
        ...
        public void Out_InvalidCast_RaisesSameKindText()
        {
            ...
            var ex = Assert.ThrowsException<ArgumentException>(
                () => np.evaluate((NpyExpr)a + 1.0, @out: outI4));
        ...
        public void Reduce_SumOfProduct_OnePass()
        {
            ...
            var s = np.evaluate(NpyExpr.Sum((NpyExpr)a * b));
        ...
        public void Reduce_DtypeRules()
        {
            ...
            Assert.AreEqual(NPTypeCode.Int64, np.evaluate(NpyExpr.Sum(NpyExpr.Arr(i4))).typecode);
            Assert.AreEqual(10L, np.evaluate(NpyExpr.Sum(NpyExpr.Arr(i4))).GetInt64(0));
            Assert.AreEqual(NPTypeCode.UInt64, np.evaluate(NpyExpr.Sum(NpyExpr.Arr(u1))).typecode);
            Assert.AreEqual(NPTypeCode.Single, np.evaluate(NpyExpr.Sum(NpyExpr.Arr(f4))).typecode);
            Assert.AreEqual(NPTypeCode.Half, np.evaluate(NpyExpr.Sum(NpyExpr.Arr(f2))).typecode);
            Assert.AreEqual(NPTypeCode.Int64, np.evaluate(NpyExpr.Prod(NpyExpr.Arr(i4))).typecode);
            Assert.AreEqual(NPTypeCode.Int32, np.evaluate(NpyExpr.Min(NpyExpr.Arr(i4))).typecode);
            Assert.AreEqual(NPTypeCode.Double, np.evaluate(NpyExpr.Mean(NpyExpr.Arr(i4))).typecode);
            Assert.AreEqual(2.0, np.evaluate(NpyExpr.Mean(NpyExpr.Arr(i4))).GetDouble(0), 1e-12);
            Assert.AreEqual(NPTypeCode.Single, np.evaluate(NpyExpr.Mean(NpyExpr.Arr(f4))).typecode);
            Assert.AreEqual(2f, np.evaluate(NpyExpr.Mean(NpyExpr.Arr(f4))).GetSingle(0), 1e-6f);
        ...
        public void Reduce_MultiChunk_Strided_AndNaN()
        {
            ...
            var s = np.evaluate(NpyExpr.Sum(NpyExpr.Arr(big)));
            Assert.AreEqual(20004.0 * 20005.0 / 2.0, s.GetDouble(0), 1e-6);

            var strided = big["::2"];
            double expect = 0;
            for (int i = 0; i < 20005; i += 2) expect += i;
            Assert.AreEqual(expect, np.evaluate(NpyExpr.Sum(NpyExpr.Arr(strided))).GetDouble(0), 1e-6);

            // NaN propagates through min/max like np.min/np.max
            var nan = np.array(new double[] { double.NaN, 1.0, -5.0 });
            Assert.IsTrue(double.IsNaN(np.evaluate(NpyExpr.Min(NpyExpr.Arr(nan))).GetDouble(0)));
            Assert.IsTrue(double.IsNaN(np.evaluate(NpyExpr.Max(NpyExpr.Arr(nan))).GetDouble(0)));

            // f16 accumulation overflows to inf exactly like np.sum(f2)
            var ones = np.ones(new Shape(70000), np.float16);
            var f2sum = np.evaluate(NpyExpr.Sum(NpyExpr.Arr(ones)));
        ...
        public void Reduce_EmptyInputs_MatchNumPyIdentities()
        {
            ...
            Assert.AreEqual(0.0, np.evaluate(NpyExpr.Sum(NpyExpr.Arr(empty))).GetDouble(0));
            Assert.AreEqual(1.0, np.evaluate(NpyExpr.Prod(NpyExpr.Arr(empty))).GetDouble(0));

            var ex = Assert.ThrowsException<ArgumentException>(
                () => np.evaluate(NpyExpr.Min(NpyExpr.Arr(empty))));
            ...
            var m = np.evaluate(NpyExpr.Mean(NpyExpr.Arr(emptyF4)));
        ...
        public void Reduce_MustBeRoot()
        {
            ...
            Assert.ThrowsException<NotSupportedException>(
                () => np.evaluate(NpyExpr.Sum(NpyExpr.Sum((NpyExpr)a * 2))));
            Assert.ThrowsException<NotSupportedException>(
                () => np.evaluate(NpyExpr.Sum((NpyExpr)a) + 1.0));
        }
        ...
        public unsafe void ExecuteExpression_LegacyOutputDtypeContract_Unchanged()
        {
            ...
            iter.ExecuteExpression(NpyExpr.Sqrt(NpyExpr.Input(0)), new[] { NPTypeCode.Double }, NPTypeCode.Double);
