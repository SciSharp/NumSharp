...
namespace NumSharp.UnitTest.Backends.Unmanaged.Math
{
    ...
    public class np_add
    {
        ...
        {
            var left = np.arange(3).reshape((3, 1)).astype(NPTypeCode.Double);
            var right = np.arange(3).reshape((1, 3)).astype(NPTypeCode.Double);
            var ret = left + right;
        ...
        {
            var left = np.arange(2 * 3 * 3).reshape((2, 1, 3, 3)).astype(NPTypeCode.Double);
            var right = np.arange(3).reshape((1, 3)).astype(NPTypeCode.Double);
            var ret = left + right;
            for (int i = 0; i < ret.size; i++)
            {
        ...
        {
            var left = np.arange(6).reshape((2, 3, 1)).astype(NPTypeCode.Double);
            var right = np.arange(6).reshape((2, 1, 3)).astype(NPTypeCode.Double);
            var ret = left + right;
            ret.size.Should().Be(18);
            ret.dtype.Should().Be<double>();
        ...
        {
            var left = np.zeros(new Shape(5, 5), NPTypeCode.Char);
            var right = np.ones(new Shape(5, 5), NPTypeCode.Boolean);
            var ret = left + right;
