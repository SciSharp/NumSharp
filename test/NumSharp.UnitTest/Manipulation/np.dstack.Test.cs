...
namespace NumSharp.UnitTest.Manipulation
{
    ...
    public class np_dstack_tests
    {
        ...
        public void DStackNDArrays()
        {
            ...
            n.Should().BeOfSize((int)(n1.size + n2.size)).And.BeOfValues(1, 2, 2, 3, 3, 4).And.BeShaped(1, 3, 2);

            //2D
            n = np.dstack(n1.reshape(3, 1), n2.reshape(3, 1)).MakeGeneric<double>();
            n.Should().BeOfSize((int)(n1.size + n2.size)).And.BeOfValues(1, 2, 2, 3, 3, 4).And.BeShaped(3, 1, 2);
        }
    }
}
