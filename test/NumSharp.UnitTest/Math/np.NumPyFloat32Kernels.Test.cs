...
namespace NumSharp.UnitTest.MathSuite
{
    ...
    public class NumPyFloat32KernelsTest : TestClass
    {
        ...
        public void PortedKernels_Float64_IsUnaffected()
        {
            ...
            np.log(np.array(new[] { 2.0 })).GetDouble(0).Should().Be(Math.Log(2.0));
            np.sin(np.array(new[] { 2.0 })).GetDouble(0).Should().Be(Math.Sin(2.0));
            np.cos(np.array(new[] { 2.0 })).GetDouble(0).Should().Be(Math.Cos(2.0));
        }
    }
}
