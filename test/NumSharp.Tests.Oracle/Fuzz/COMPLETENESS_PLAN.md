...
Origin: a completeness review of `test/oracle/` + `test/NumSharp.Tests/Fuzz/` performed
...
4. **The gate** (from `<worktree>/test/NumSharp.Tests`):
...
5. **Corpus regeneration** (deterministic — same input ⇒ byte-identical output; from worktree root):
   ...
   - After ANY regen: `git diff --stat -- test/NumSharp.Tests/Fuzz/corpus` and confirm ONLY the
...
`test/NumSharp.Tests/Fuzz/MisalignedRegistry.cs`):
...
(if helpers needed), NEW `test/NumSharp.Tests/OpenBugs.FuzzGate.cs` for pins, and any
...
regeneration), `Fuzz/OpRegistry.cs`, NEW `test/NumSharp.Tests/OpenBugs.FuzzGaps.cs` for pins,
