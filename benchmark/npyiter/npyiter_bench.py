# =============================================================================
# npyiter_bench.py — NumPy side of THE canonical NpyIter benchmark (identical
# ids to npyiter_bench.cs). Section-addressable via the NPYITER_SECTION env var
# so the orchestrator (npyiter_sheet.py) can run each category in its own
# process. Emits machine-readable "id<TAB>milliseconds" rows on stdout.
#
# Run a section:  NPYITER_SECTION=elementwise python npyiter_bench.py
# Run everything: python npyiter_bench.py
# =============================================================================
import os
import sys
import time
...
print(f"[npyiter_bench.py] section={SECTION} numpy={np.__version__}", file=sys.stderr)
