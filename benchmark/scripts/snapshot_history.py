...
def build_manifest(snap_name, run_ts, head, subject, dirty, dirty_files, env,
    ...
    L += [f"| Date | {date} |", "",
          ...
          "  exceeds 10 ms) — merge-results.py compares the mins; per-case means are retained in the",
          "  JSON (`numpy_mean_ms`/`numsharp_mean_ms`) as interference/tail diagnostics.",
