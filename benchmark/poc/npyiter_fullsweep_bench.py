# =============================================================================
# npyiter_fullsweep_bench.py — NumPy side of the full family sweep (identical
# ids to npyiter_fullsweep_bench.cs). Every distinct NpyIter operation family
# from rounds 1-3, across scalar(1)/1K/100K/1M.
#
# Raw-iterator NumSharp families map to the production np.* that does the same
# logical op (np.add / np.sqrt / np.positive); production NumSharp families map
# 1:1. Dividends (fuse7/reuse/par8) have no NumPy equivalent — NumPy can only do
# fuse7 as a chain of 6 adds and never threads its iterator; those baselines are
# emitted so the chart can show the gap.
#
# Run: python benchmark/poc/npyiter_fullsweep_bench.py
# =============================================================================
import time
import numpy as np
...
print(f"NumPy {np.__version__} NpyIter FULL sweep")
print(f"{'id':<14} {'per call':>10}")
print("-" * 28)
...
for tag, n in SIZES:
    ...
    # ---- ELEMENTWISE (np.add / np.sqrt / np.positive) -----------------------
    np.add(a, b, out=o)
    ...
    # ---- DIVIDENDS (NumPy baselines: chain for fuse7; single-thread for par8;
    #      reuse has no Python equivalent so we reuse the add baseline) --------
    ...
    # reuse: NumPy cannot reset+rerun a built nditer from Python; the honest
    # baseline is the same one-shot add it must rebuild each call.
