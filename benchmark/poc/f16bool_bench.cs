...
var ba=((np.arange(R*C)%17)+1).astype(NPTypeCode.Half).reshape(R,C);
...
foreach(var l in new[]{"C","F","T","sliced","negrow","negcol","strided","bcast"}){
    ...
    sb.AppendLine($"f16|{l}|bool\t{ns:F5}");
}
File.WriteAllText(@"K:\source\NumSharp\benchmark\poc\_xref\f16bool_ns.tsv",sb.ToString());
