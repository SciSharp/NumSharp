...
// =============================================================================
// nditer_bench.cs — THE canonical NumSharp NDIter benchmark (NumSharp side).
// Companion: nditer_bench.py (identical ids). Orchestrated by nditer_sheet.py
// into a single results sheet.
//
// Covers, in one place, every NDIter aspect probed across POC rounds 1-3:
...
if ((dbgScript?.IsJITOptimizerDisabled ?? false) || (dbgCore?.IsJITOptimizerDisabled ?? false))
{
    ...
    Console.WriteLine("Run:   dotnet run -c Release - < benchmark/nditer/nditer_bench.cs");
    return;
}
...
double BestMs(Action body, int iters, int warm, int rounds)
{
    ...
    // Caller's (iters, rounds) from Pick survive as floors/hints only — iters is superseded by the
    // pilot-derived batch (kept in the signature for call-site stability).
...
var RO1 = new[] { NDIterPerOpFlags.READONLY };
...
Console.Error.WriteLine($"[nditer_bench] section={section}  cores={Environment.ProcessorCount} V256={Vector256.IsHardwareAccelerated}");
...
unsafe
{
    ...
    if (wantOps)
    {
        foreach (var (tag, n) in SIZES)
        {
            ...
            if (Want("elementwise"))
            {
                ...
                Row($"add@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(3, add3, EXL, KO, SAFE, RO_RO_WO); it.ForEach(K.AddF64); }, iters, warm, rounds));
                Row($"sqrt@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(2, copy2, EXL, KO, SAFE, RO_WO); it.ForEach(K.SqrtF64); }, iters, warm, rounds));
                Row($"copy@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(2, copy2, EXL, KO, SAFE, RO_WO); it.ForEach(K.CopyF64); }, iters, warm, rounds));
                np.add(sa, sb, so); Check(so.GetDouble(n - 1) == sa.GetDouble(n - 1) + sb.GetDouble(n - 1), $"sadd@{tag}");
                Row($"sadd@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(3, sadd3, EXL, KO, SAFE, RO_RO_WO); it.ForEach(K.AddF64); }, iters, warm, rounds));
                Row($"bcast@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(3, bc3, EXL, KO, SAFE, RO_RO_WO); it.ForEach(K.AddF64); }, iters, warm, rounds));
                { using var itw = NDIterRef.MultiNew(2, rev2, EXL, KO, SAFE, RO_WO); itw.ForEach(K.CopyF64); }
                Check(dstRev.GetDouble(0) == a.GetDouble(n - 1), $"frev@{tag}");
                Row($"frev@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(2, rev2, EXL, KO, SAFE, RO_WO); it.ForEach(K.CopyF64); }, iters, warm, rounds));
                Row($"castbuf@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(2, cast2, BUFEXL, KO, SAFE, RO_WO, f64x2); it.ForEach(K.CopyF64); }, iters, warm, rounds));
                Row($"mixbuf@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(3, mix3, BUFEXL, KO, SAFE, RO_RO_WO, f64x3); it.ForEach(K.AddF64); }, iters, warm, rounds));
            }
            ...
            if (Want("dividends"))
            {
                ...
                var flags8 = new NDIterPerOpFlags[8];
                for (int i = 0; i < 7; i++) flags8[i] = NDIterPerOpFlags.READONLY;
                flags8[7] = NDIterPerOpFlags.WRITEONLY;
                Row($"fuse7@{tag}", BestMs(() => { using var it = NDIterRef.MultiNew(8, ins, EXL, KO, SAFE, flags8); it.ForEach(K.Sum7F64); }, iters, warm, rounds));
                ...
                if (n >= 8)
                {
                    ...
                    Row($"par8@{tag}", BestMs(() =>
                    {
                        Parallel.For(0, 8, i =>
                        {
                            ...
                            using var it = NDIterRef.MultiNew(2, ops, EXL, KO, SAFE, RO_WO);
                            it.ForEach(K.SinF64);
                        });
    ...
    if (Want("construction"))
    {
        ...
        var ro8 = new NDIterPerOpFlags[8]; for (int i = 0; i < 7; i++) ro8[i] = NDIterPerOpFlags.READONLY; ro8[7] = NDIterPerOpFlags.WRITEONLY;
        ...
        Row("ctor.1op", BestMs(() => { using var it = NDIterRef.New(a); }, 400_000, 50_000, 5));
        Row("ctor.3op_exl", BestMs(() => { using var it = NDIterRef.MultiNew(3, ops3, EXL, KO, SAFE, RO_RO_WO); }, 400_000, 50_000, 5));
        Row("ctor.ufunc", BestMs(() => { using var it = NDIterRef.MultiNew(3, ops3, ufuncFlags, KO, SAFE, RO_RO_WO); }, 200_000, 25_000, 5));
        Row("ctor.bufcast", BestMs(() => { using var it = NDIterRef.MultiNew(2, opsCast, BUFEXL, KO, SAFE, RO_WO, f64x2); }, 100_000, 12_000, 5));
        Row("ctor.multiindex", BestMs(() => { using var it = NDIterRef.New(g32, NDIterGlobalFlags.MULTI_INDEX); }, 400_000, 50_000, 5));
        Row("ctor.8op", BestMs(() => { using var it = NDIterRef.MultiNew(8, ops8, EXL, KO, SAFE, ro8); }, 200_000, 25_000, 5));
        Row("ctor.4d", BestMs(() => { using var it = NDIterRef.MultiNew(2, ops4d, EXL, KO, SAFE, RO_WO); }, 200_000, 25_000, 5));
        Row("ctor.8d", BestMs(() => { using var it = NDIterRef.MultiNew(2, ops8d, EXL, KO, SAFE, RO_WO); }, 200_000, 25_000, 5));
        Row("ctor.strided2d", BestMs(() => { using var it = NDIterRef.MultiNew(2, opsSv, EXL, KO, SAFE, RO_WO); }, 200_000, 25_000, 5));
        GC.Collect(); GC.WaitForPendingFinalizers();
    }
    ...
    if (Want("pathology"))
    {
        ...
        // ALLOCATE out: NumSharp zeros (np.zeros) vs NumPy empty
        {
            ...
            var allocFlags = new[] { NDIterPerOpFlags.READONLY, NDIterPerOpFlags.READONLY, NDIterPerOpFlags.WRITEONLY | NDIterPerOpFlags.ALLOCATE };
            NDArray sink = null;
            Row("path.allocate", BestMs(() =>
            {
                var ops = new NDArray[] { a, b, null };
                using var it = NDIterRef.MultiNew(3, ops, EXL, KO, SAFE, allocFlags, f64x3b);
                it.ForEach(K.AddF64);
                sink = ops[2];
            }, 10, 4, 7));
        ...
        // F-order out: iterator add C+C -> F-order out (order resolution)
        {
            ...
            { using var it = NDIterRef.MultiNew(3, opsX, EXL, KO, SAFE, RO_RO_WO); it.ForEach(K.AddF64); }
            Check(Math.Abs(oF.GetDouble(5, 7) - (aC.GetDouble(5, 7) + bC.GetDouble(5, 7))) < 1e-9, "path.forder_out");
            Row("path.forder_out", BestMs(() =>
            {
                using var it = NDIterRef.MultiNew(3, opsX, EXL, KO, SAFE, RO_RO_WO);
                it.ForEach(K.AddF64);
            }, 12, 4, 7));
        }
