#!/usr/bin/env python3
# =============================================================================
# nditer_sheet.py — THE canonical NDIter benchmark orchestrator + renderer.
#
# Runs every section of nditer_bench.{cs,py} (NumSharp vs NumPy), each in its
...
# the NumSharp-only dividends. Saves the sheet to nditer_results.md and the raw
# pairs to nditer_results.tsv.
#
#   python benchmark/nditer/nditer_sheet.py                 # full run + sheet
#   python benchmark/nditer/nditer_sheet.py --skip-build    # reuse Release build
#   python benchmark/nditer/nditer_sheet.py --render-only   # re-render last .tsv
...
CS = os.path.join(HERE, "nditer_bench.cs")
PY = os.path.join(HERE, "nditer_bench.py")
CORE_CSPROJ = os.path.join(REPO, "src", "NumSharp.Core", "NumSharp.Core.csproj")
TSV = os.path.join(HERE, "nditer_results.tsv")
SHEET = os.path.join(HERE, "nditer_results.md")
...
def render(pairs):
    ...
    out(f"NumSharp NDIter — canonical benchmark · {stamp} · speedup = NumPy ÷ NumSharp (>1.0× = NumSharp faster)")
