...
public class NDArrayMethodBenchmarks : OfficialBenchmarkBase
{
    private NDArray _a = null!;
    private NDArray _b = null!;
    private NDArray _productInput = null!;
    ...
    public void Setup()
    {
        ...
        _productInput = (np.random.rand(WorkN) * 0.5 + 0.5).astype(DType);
        int rows = 10;
        ...
        _sorted = np.arange(WorkN).astype(DType);
        _putTarget = _a.copy();
        _fillTarget = _a.copy();
        _fieldTarget = _a.copy();
        _clipMin = NDArray.Scalar(GetScalar(DType, -10), DType);
        _clipMax = NDArray.Scalar(GetScalar(DType, 10), DType);
    ...
    [Benchmark(Description = "a.cumprod() [ndarray]")] public NDArray CumProd() => _productInput.cumprod();
    ...
    [Benchmark(Description = "a.prod() [ndarray]")] public NDArray Prod() => _productInput.prod();
    ...
    [Benchmark(Description = "a.dot(b) [ndarray]")] public object Dot() => DType != NPTypeCode.Char
        ? _a.dot(_b)
        : VerifyUnsupportedDtype(() => _a.dot(_b));
    [Benchmark(Description = "a.fill(value) [ndarray]")] public void Fill() => _fillTarget.fill(GetScalar(DType, 3));
    [Benchmark(Description = "a.getfield(dtype) [ndarray]")] public NDArray GetField() => _a.getfield(DType.AsType());
    ...
    [Benchmark(Description = "a.setfield(value, dtype) [ndarray]")] public void SetField() => _fieldTarget.setfield(GetScalar(DType, 1), DType.AsType());
