...
- Managed effective cells: **3,641**
- OpenBLAS effective cells: **104**
- Missing backend records: **48**
- Not supported records: **0**

| Operation | DType | N | Managed C# | OpenBLAS | Effective | NPY/NS |
|---|---|---:|---:|---:|---|---:|
| `np.polyfit(x, y, 3)` | float64 | 1,000 | missing_backend | 0.06550 ms | openblas | 0.689x |
| `np.polyfit(x, y, 3)` | float64 | 100,000 | missing_backend | 0.6064 ms | openblas | 0.873x |
| `np.polyfit(x, y, 3)` | float64 | 10,000,000 | missing_backend | 4.2946 ms | openblas | 1.228x |
| `np.roots(p)` | float64 | 1,000 | missing_backend | 0.03440 ms | openblas | 1.064x |
| `np.roots(p)` | float64 | 100,000 | missing_backend | 0.09530 ms | openblas | 1.025x |
| `np.roots(p)` | float64 | 10,000,000 | missing_backend | 0.4132 ms | openblas | 0.995x |
...
| `np.einsum('ij,jk->ik', a, b)` | float64 | 1,000 | 0.00410 ms | 0.00280 ms | openblas | 2.214x |
| `np.einsum('ij,jk->ik', a, b)` | float64 | 100,000 | 1.4043 ms | 0.7998 ms | openblas | 4.414x |
| `np.einsum('ij,jk->ik', a, b)` | float64 | 10,000,000 | 3.1350 ms | 1.4120 ms | openblas | 4.614x |
...
| `np.linalg.cholesky(a)` | float64 | 1,000 | missing_backend | 0.00170 ms | openblas | 2.588x |
| `np.linalg.cholesky(a)` | float64 | 100,000 | missing_backend | 0.01470 ms | openblas | 1.245x |
| `np.linalg.cholesky(a)` | float64 | 10,000,000 | missing_backend | 0.03100 ms | openblas | 1.832x |
| `np.linalg.cond(a)` | float64 | 1,000 | missing_backend | 0.03170 ms | openblas | 1.107x |
| `np.linalg.cond(a)` | float64 | 100,000 | missing_backend | 0.2608 ms | openblas | 1.002x |
| `np.linalg.cond(a)` | float64 | 10,000,000 | missing_backend | 0.5533 ms | openblas | 1.007x |
...
| `np.linalg.det(a)` | float64 | 1,000 | 0.00200 ms | 0.00450 ms | managed | 2.900x |
| `np.linalg.det(a)` | float64 | 100,000 | 0.03010 ms | 0.02570 ms | openblas | 1.047x |
| `np.linalg.det(a)` | float64 | 10,000,000 | 0.08600 ms | 0.05340 ms | openblas | 1.019x |
...
| `np.linalg.eig(a)` | float64 | 1,000 | missing_backend | 0.1017 ms | openblas | 0.990x |
| `np.linalg.eig(a)` | float64 | 100,000 | missing_backend | 1.3708 ms | openblas | 1.016x |
| `np.linalg.eig(a)` | float64 | 10,000,000 | missing_backend | 2.8319 ms | openblas | 1.036x |
| `np.linalg.eigh(a)` | float64 | 1,000 | missing_backend | 0.04490 ms | openblas | 1.067x |
| `np.linalg.eigh(a)` | float64 | 100,000 | missing_backend | 0.3893 ms | openblas | 1.013x |
| `np.linalg.eigh(a)` | float64 | 10,000,000 | missing_backend | 0.7102 ms | openblas | 1.034x |
| `np.linalg.eigvals(a)` | float64 | 1,000 | missing_backend | 0.05980 ms | openblas | 1.070x |
| `np.linalg.eigvals(a)` | float64 | 100,000 | missing_backend | 0.7965 ms | openblas | 1.021x |
| `np.linalg.eigvals(a)` | float64 | 10,000,000 | missing_backend | 1.5520 ms | openblas | 0.990x |
| `np.linalg.eigvalsh(a)` | float64 | 1,000 | missing_backend | 0.01880 ms | openblas | 1.133x |
| `np.linalg.eigvalsh(a)` | float64 | 100,000 | missing_backend | 0.1593 ms | openblas | 1.003x |
| `np.linalg.eigvalsh(a)` | float64 | 10,000,000 | missing_backend | 0.2925 ms | openblas | 1.005x |
| `np.linalg.inv(a)` | float64 | 1,000 | 0.00510 ms | 0.00820 ms | managed | 2.118x |
| `np.linalg.inv(a)` | float64 | 100,000 | 0.09470 ms | 0.07580 ms | openblas | 1.033x |
| `np.linalg.inv(a)` | float64 | 10,000,000 | 0.2987 ms | 0.1602 ms | openblas | 1.135x |
| `np.linalg.lstsq(a, b)` | float64 | 1,000 | missing_backend | 0.07700 ms | openblas | 1.032x |
| `np.linalg.lstsq(a, b)` | float64 | 100,000 | missing_backend | 0.8111 ms | openblas | 0.982x |
| `np.linalg.lstsq(a, b)` | float64 | 10,000,000 | missing_backend | 1.4309 ms | openblas | 1.011x |
...
| `np.linalg.matrix_norm(a, ord=2)` | float64 | 1,000 | missing_backend | 0.03150 ms | openblas | 1.165x |
| `np.linalg.matrix_norm(a, ord=2)` | float64 | 100,000 | missing_backend | 0.2561 ms | openblas | 1.031x |
| `np.linalg.matrix_norm(a, ord=2)` | float64 | 10,000,000 | missing_backend | 0.5589 ms | openblas | 0.993x |
| `np.linalg.matrix_power(a, -1)` | float64 | 1,000 | 0.00530 ms | 0.00860 ms | managed | 2.151x |
| `np.linalg.matrix_power(a, -1)` | float64 | 100,000 | 0.1194 ms | 0.07730 ms | openblas | 1.019x |
| `np.linalg.matrix_power(a, -1)` | float64 | 10,000,000 | 0.2419 ms | 0.1629 ms | openblas | 1.123x |
...
| `np.linalg.matrix_rank(a)` | float64 | 1,000 | missing_backend | 0.03250 ms | openblas | 1.126x |
| `np.linalg.matrix_rank(a)` | float64 | 100,000 | missing_backend | 0.2619 ms | openblas | 1.006x |
| `np.linalg.matrix_rank(a)` | float64 | 10,000,000 | missing_backend | 0.5586 ms | openblas | 0.991x |
...
| `np.linalg.norm(a, ord=2)` | float64 | 1,000 | missing_backend | 0.03030 ms | openblas | 1.208x |
| `np.linalg.norm(a, ord=2)` | float64 | 100,000 | missing_backend | 0.2570 ms | openblas | 1.027x |
| `np.linalg.norm(a, ord=2)` | float64 | 10,000,000 | missing_backend | 0.5601 ms | openblas | 0.987x |
...
| `np.linalg.pinv(a)` | float64 | 1,000 | missing_backend | 0.08090 ms | openblas | 1.053x |
| `np.linalg.pinv(a)` | float64 | 100,000 | missing_backend | 0.7224 ms | openblas | 1.018x |
| `np.linalg.pinv(a)` | float64 | 10,000,000 | missing_backend | 1.4111 ms | openblas | 1.007x |
| `np.linalg.qr(a)` | float64 | 1,000 | missing_backend | 0.01860 ms | openblas | 1.226x |
| `np.linalg.qr(a)` | float64 | 100,000 | missing_backend | 0.1455 ms | openblas | 1.114x |
| `np.linalg.qr(a)` | float64 | 10,000,000 | missing_backend | 0.3389 ms | openblas | 1.189x |
| `np.linalg.slogdet(a)` | float64 | 1,000 | 0.00200 ms | 0.00450 ms | managed | 3.250x |
| `np.linalg.slogdet(a)` | float64 | 100,000 | 0.03190 ms | 0.02600 ms | openblas | 1.065x |
| `np.linalg.slogdet(a)` | float64 | 10,000,000 | 0.08650 ms | 0.05330 ms | openblas | 1.045x |
| `np.linalg.solve(a, b)` | float64 | 1,000 | 0.00350 ms | 0.00470 ms | managed | 2.086x |
| `np.linalg.solve(a, b)` | float64 | 100,000 | 0.04640 ms | 0.02710 ms | openblas | 1.089x |
| `np.linalg.solve(a, b)` | float64 | 10,000,000 | 0.1125 ms | 0.05540 ms | openblas | 1.051x |
| `np.linalg.svd(a)` | float64 | 1,000 | missing_backend | 0.07210 ms | openblas | 1.035x |
| `np.linalg.svd(a)` | float64 | 100,000 | missing_backend | 0.6822 ms | openblas | 1.001x |
| `np.linalg.svd(a)` | float64 | 10,000,000 | missing_backend | 1.2951 ms | openblas | 1.030x |
| `np.linalg.svdvals(a)` | float64 | 1,000 | missing_backend | 0.02950 ms | openblas | 1.081x |
| `np.linalg.svdvals(a)` | float64 | 100,000 | missing_backend | 0.2589 ms | openblas | 0.996x |
| `np.linalg.svdvals(a)` | float64 | 10,000,000 | missing_backend | 0.5574 ms | openblas | 0.981x |
...
| `np.linalg.tensorinv(a)` | float64 | 1,000 | 0.00520 ms | 0.00860 ms | managed | 2.212x |
| `np.linalg.tensorinv(a)` | float64 | 100,000 | 0.09490 ms | 0.07610 ms | openblas | 1.039x |
| `np.linalg.tensorinv(a)` | float64 | 10,000,000 | 0.2988 ms | 0.1609 ms | openblas | 1.137x |
| `np.linalg.tensorsolve(a, b)` | float64 | 1,000 | 0.00390 ms | 0.00500 ms | managed | 2.128x |
| `np.linalg.tensorsolve(a, b)` | float64 | 100,000 | 0.04670 ms | 0.02750 ms | openblas | 1.109x |
| `np.linalg.tensorsolve(a, b)` | float64 | 10,000,000 | 0.1126 ms | 0.05560 ms | openblas | 1.072x |
...
| `np.tensordot(a, b, axes=1)` | float64 | 100,000 | 1.4011 ms | 0.7973 ms | openblas | 1.014x |
| `np.tensordot(a, b, axes=1)` | float64 | 10,000,000 | 3.1346 ms | 1.4118 ms | openblas | 1.152x |
...
| `a.dot(b) [ndarray] (float64)` | float64 | 100,000 | 0.01701 ms | 0.00970 ms | openblas | 1.614x |
| `a.dot(b) [ndarray] (float64)` | float64 | 10,000,000 | 0.3974 ms | 0.1731 ms | openblas | 1.924x |
...
| `np.convolve(a, kernel) (float64)` | float64 | 1,000 | 0.00934 ms | 0.01560 ms | managed | 2.493x |
| `np.convolve(a, kernel) (float64)` | float64 | 100,000 | 0.1252 ms | 0.9208 ms | managed | 16.606x |
| `np.convolve(a, kernel) (float64)` | float64 | 10,000,000 | 1.2349 ms | 9.7410 ms | managed | 17.809x |
...
| `np.correlate(a, kernel) (float64)` | float64 | 1,000 | 0.00490 ms | 0.01330 ms | managed | 4.552x |
| `np.correlate(a, kernel) (float64)` | float64 | 100,000 | 0.3823 ms | 0.9140 ms | managed | 5.419x |
| `np.correlate(a, kernel) (float64)` | float64 | 10,000,000 | 3.8500 ms | 9.2459 ms | managed | 4.922x |
...
| `np.inner(a, b) (float64)` | float64 | 100,000 | 0.01155 ms | 0.00970 ms | openblas | 1.869x |
| `np.inner(a, b) (float64)` | float64 | 10,000,000 | 0.3765 ms | 0.1724 ms | openblas | 1.940x |
