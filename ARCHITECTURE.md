...
flowchart LR
    ...
    Weaver["optional NumSharp.Build (build-time)"] -. "weaves [NDScoped] scopes into the compile" .-> App
...
|---|---|---|---|
| [`NumSharp.Core`](src/NumSharp.Core/NumSharp.Core.csproj) | Array model, `np` surface, managed engine, iterators, kernels, random, FFT, linalg, and IO | .NET runtime only | Default package; no native dependency or P/Invoke. The nupkg also carries the `[NDScoped]` compile-time analyzer (`analyzers/dotnet/cs/`, from `tools/NumSharp.Build.Analyzer`) and the NDW013 weaver-missing guard (`build/NumSharp.targets`) — build-time assets, still zero dependency entries |
...
| [`NumSharp.Build`](tools/NumSharp.Build/NumSharp.Build.csproj) | Build-time IL weaver for `[NDScoped]` (synchronous) / `[NDScopedAsync]` (async) deterministic reclamation (Core self-weave + the tools-only consumer package). Weaver-ONLY: ships no `analyzers/` — the compile-time analyzer rides the `NumSharp` package | Mono.Cecil (packed into `tools/`, expressed to consumers as nothing) | Build-time only: no `lib/`, no dependency entries, installed with `PrivateAssets="all"` — never a runtime boundary |
| [`NumSharp.Build.Analyzer`](tools/NumSharp.Build.Analyzer/NumSharp.Build.Analyzer.csproj) | Roslyn analyzer: the `[NDScoped]`-target gate (NDW002/003/005/006/009/010/011/015, compile errors) + the NDW012 leak nudge + the NDW016/NDW017 type-level ownership pass (a type that stores NDArrays must be disposable and dispose them; ownership is contagious; opt-out `[NDBorrowed]`) | Microsoft.CodeAnalysis (compiler-host-provided, never shipped) | Not its own package (`IsPackable=false`): the DLL is staged into the **NumSharp** nupkg's `analyzers/dotnet/cs/` by `_NumSharpCorePackAnalyzer`, so referencing NumSharp alone activates it |
...
|---|---|---|---|---|
| **A1 API and parity contract** | partial | NumPy-shaped public API, overloads, operators, dtype aliases, and parity policy | [`np.cs`](src/NumSharp.Core/APIs/np.cs), [`NDArray.Primitive.cs`](src/NumSharp.Core/Operations/Elementwise/NDArray.Primitive.cs), [fuzz README](test/NumSharp.Tests/Fuzz/README.md) | Every supported API has NumPy 2.x signature, dtype, value, exception, and layout parity |
...
| **A3 Storage and lifetime** | risk | Allocation, pooling, external buffers, reference counting, and deterministic release | [`UnmanagedStorage.cs`](src/NumSharp.Core/Backends/Unmanaged/UnmanagedStorage.cs), [`UnmanagedMemoryBlock<T>`](src/NumSharp.Core/Backends/Unmanaged/UnmanagedMemoryBlock%601.cs), [lifetime tests](test/NumSharp.Tests/Lifetime/) | No use-after-free, double-free, leak, or ownership ambiguity for arrays, views, and interop buffers |
| **A4 Layout, views, indexing, and broadcasting** | operational | Element-to-address mapping and shared-memory semantics | [`Shape.Broadcasting.cs`](src/NumSharp.Core/View/Shape.Broadcasting.cs), [`UnmanagedStorage.Slicing.cs`](src/NumSharp.Core/Backends/Unmanaged/UnmanagedStorage.Slicing.cs), [`NDArray.Indexing.cs`](src/NumSharp.Core/Selection/NDArray.Indexing.cs), [view tests](test/NumSharp.Tests/View/) | Every operation honors C/F, offset, positive/negative stride, zero stride, scalar, and empty layouts |
...
Current gap: API presence is not equivalent to parity. The committed corpus and `OpenBugs` tests deliberately record remaining dtype, exception, random-stream, complex-edge, and host-dependent differences. The authoritative live ledger is the [fuzz README](test/NumSharp.Tests/Fuzz/README.md), not a static “implemented functions” list here.
...
- **CAP-ITER-01 — Multi-operand NDIter core (operational).** `NDIterRef` is a stack-only iterator over unmanaged state. It performs broadcasting, axis remapping, dimension coalescing, C/F/A/K ordering, buffering, casting, reduction setup, overlap handling, masks, and synchronized pointer advancement. Evidence: [`NDIter.cs`](src/NumSharp.Core/Backends/Iterators/NDIter.cs), [`NDIterCasting.cs`](src/NumSharp.Core/Backends/Iterators/NDIterCasting.cs), [`NDIterBufferManager.cs`](src/NumSharp.Core/Backends/Iterators/NDIterBufferManager.cs), and [iterator parity tests](test/NumSharp.Tests/Backends/Iterators/NDIterNumPyParityTests.cs).
...
- **CAP-RNG-01 — NumPy RandomState-style module (partial).** `np.random` is a shared `NumPyRandom` backed by MT19937. Portable stream operations are corpus-gated; libm-dependent streams can be host-specific, and several samplers remain documented divergences. Evidence: [`np.random.cs`](src/NumSharp.Core/RandomSampling/np.random.cs), [`MT19937.cs`](src/NumSharp.Core/RandomSampling/MT19937.cs), and [`random_parity.jsonl`](test/NumSharp.Tests/Fuzz/corpus/random_parity.jsonl).
...
- **CAP-SIGN-01 — Strong-name verification (operational).** CI validates the committed key and inspects packed assemblies for the expected public key and a real strong-name signature. Evidence: [the `test` job's "Strong naming:" steps, ubuntu leg](.github/workflows/build-and-release.yml) and [`verify_strong_name.cs`](.github/scripts/verify_strong_name.cs).
...
|---|---|---|---|
| **P0** | Unsafe storage and external zero-copy paths can turn metadata/lifetime defects into process corruption. | [memory code](src/NumSharp.Core/Backends/Unmanaged/), [interop validation tests](test/NumSharp.Tests.Interop/ArrayInterfaceValidationTests.cs) | Every new owner mode has adversarial metadata, aliasing, concurrent disposal, and leak coverage. |
...
| **P1** | NumPy parity still has bounded/carved divergences and coverage gaps. | [fuzz README](test/NumSharp.Tests/Fuzz/README.md), [`COVERAGE_GAPS.md`](test/NumSharp.Tests/Fuzz/COVERAGE_GAPS.md), `OpenBugs` tests | Reduce the registry without broadening excuses; each fix promotes its repro into a normal gate. |
...
dotnet test test/NumSharp.Tests/NumSharp.Tests.csproj --configuration Release --no-build --framework net8.0 --filter "TestCategory!=OpenBugs&TestCategory!=HighMemory"
dotnet test test/NumSharp.Tests.Interop/NumSharp.Tests.Interop.csproj --configuration Release --framework net8.0
git diff --check
```
