...
| [`NumSharp.Build.Analyzer`](tools/NumSharp.Build.Analyzer/NumSharp.Build.Analyzer.csproj) | Roslyn analyzer: the `[NDScoped]`-target gate (NDW002/003/005/006/009/010/011, compile errors) + the NDW012 leak nudge | Microsoft.CodeAnalysis (compiler-host-provided, never shipped) | Not its own package (`IsPackable=false`): the DLL is staged into the **NumSharp** nupkg's `analyzers/dotnet/cs/` by `_NumSharpCorePackAnalyzer`, so referencing NumSharp alone activates it |
