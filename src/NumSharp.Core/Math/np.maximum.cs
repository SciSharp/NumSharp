...
namespace NumSharp
{
    ...
    {
        /// <summary>
        ///     Element-wise maximum of array elements (NaN-PROPAGATING).
        ///     Compare two arrays and return a new array containing the element-wise maxima. If either
        ...
        ///     <see cref="fmax(NDArray, NDArray, NDArray, NDArray, DType)"/> to IGNORE NaNs instead.
        ///     Mirrors NumPy's ufunc signature: <c>maximum(x1, x2, /, out=None, *, where=True, dtype=None)</c>.
        /// </summary>
        ...
        /// <returns>The element-wise maximum of <paramref name="x1"/> and <paramref name="x2"/> (or <paramref name="@out"/> when supplied). A scalar if both inputs are scalars.</returns>
        ...
        /// </para>
        /// </remarks>
        public static NDArray maximum(NDArray x1, NDArray x2, NDArray @out = null, NDArray where = null, DType dtype = null)
            => x1.TensorEngine.Maximum(x1, x2, dtype, @out, where);

        /// <summary>
        ///     Element-wise maximum of array elements, IGNORING NaNs.
        ///     Compare two arrays and return a new array containing the element-wise maxima. If one element
        ///     is a NaN the non-NaN element is returned (both NaN → the first), so NaNs are ignored where
        ///     possible. Use <see cref="maximum(NDArray, NDArray, NDArray, NDArray, DType)"/> to propagate
        ///     NaNs instead. Mirrors NumPy's ufunc signature: <c>fmax(x1, x2, /, out=None, *, where=True, dtype=None)</c>.
        /// </summary>
        ...
        /// <param name="dtype">Explicit loop dtype (NumPy ufunc dtype=): the comparison runs at this precision.</param>
        /// <returns>The element-wise NaN-ignoring maximum (or <paramref name="@out"/> when supplied). A scalar if both inputs are scalars.</returns>
        ...
        /// <remarks>https://numpy.org/doc/stable/reference/generated/numpy.fmax.html</remarks>
        public static NDArray fmax(NDArray x1, NDArray x2, NDArray @out = null, NDArray where = null, DType dtype = null)
            => x1.TensorEngine.FMax(x1, x2, dtype, @out, where);
    }
}
