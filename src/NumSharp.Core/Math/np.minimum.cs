...
namespace NumSharp
{
    public static partial class np
    {
        ...
        public static NDArray minimum(NDArray x1, NDArray x2, NDArray @out)
            => x1.TensorEngine.Minimum(x1, x2, null, @out);

        /// <summary>
        ///     Element-wise minimum of array elements, ignoring NaNs.
        ///     Compare two arrays and returns a new array containing the element-wise minima. If one of the elements being compared is a NaN, then the non-nan element is returned. If both elements are NaNs then the first is returned. The net effect is that NaNs are ignored when possible.
        /// </summary>
        /// <param name="x1">The arrays holding the elements to be compared. If x1.shape != x2.shape, they must be broadcastable to a common shape (which becomes the shape of the output).</param>
        /// <param name="x2">The arrays holding the elements to be compared. If x1.shape != x2.shape, they must be broadcastable to a common shape (which becomes the shape of the output).</param>
        /// <param name="dtype">Loop dtype (NumPy ufunc dtype=): the comparison runs at this precision.</param>
        ...
        public static NDArray fmin(NDArray x1, NDArray x2, NDArray @out)
            => x1.TensorEngine.FMin(x1, x2, null, @out);
    }
}
