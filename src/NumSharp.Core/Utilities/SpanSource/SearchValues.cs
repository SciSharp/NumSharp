...
namespace System.Buffers
{
    ...
    public static class SearchValues
    {
        ...
        public static SearchValues<byte> Create(params ReadOnlyUnmanagedSpan<byte> values)
        {
            if (values.IsEmpty)
            {
        ...
        /// <param name="values">The set of values.</param>
        /// <returns>The optimized representation of <paramref name="values"/> used for efficient searching.</returns>
        public static SearchValues<char> Create(params ReadOnlyUnmanagedSpan<char> values)
        {
            if (values.IsEmpty)
            {
            ...
            ReadOnlyUnmanagedSpan<short> shortValues = MemoryMarshal.Cast<char, short>(values);

            if (values.Length == 1)
            {
                char value = values[0];

                return PackedUnmanagedSpanHelpers.PackedIndexOfIsSupported && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(value)
            ...
            {
                return PackedUnmanagedSpanHelpers.PackedIndexOfIsSupported && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(minInclusive) && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(maxInclusive)
            ...
            if (values.Length == 2)
            {
                ...
                if (PackedUnmanagedSpanHelpers.PackedIndexOfIsSupported && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(value0) && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(value1))
                {
            ...
            if (values.Length == 3)
            {
                ...
                return PackedUnmanagedSpanHelpers.PackedIndexOfIsSupported && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(value0) && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(value1) && PackedUnmanagedSpanHelpers.CanUsePackedIndexOf(value2)
            ...
            // reduces the number of comparisons by masking off the bit that differs between lower and upper case (0x20).
            // While this most commonly applies to ASCII letters, it also works for other values that differ by 0x20 (e.g. "[]{}" => "{}").
            if (IndexOfAnyAsciiSearcher.IsVectorizationSupported && PackedUnmanagedSpanHelpers.PackedIndexOfIsSupported &&
                maxInclusive < 128 && values.Length == 4 && minInclusive > 0)
            {
                UnmanagedSpan<char> copy = stackalloc char[4];
                values.CopyTo(copy);
                copy.Sort();
        ...
        public static SearchValues<string> Create(ReadOnlyUnmanagedSpan<string> values, StringComparison comparisonType)
        {
        ...
        private static bool TryGetSingleRange<T>(ReadOnlyUnmanagedSpan<T> values, out T minInclusive, out T maxInclusive)
            where T : struct, INumber<T>, IMinMaxValue<T>
        {
            ...
            UnmanagedSpan<bool> seenValues = range <= 256 ? stackalloc bool[256] : new bool[range];
