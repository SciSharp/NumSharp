...
namespace System.Buffers
{
    public readonly partial struct ReadOnlySequence<T>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private ReadOnlyUnmanagedSpan<T> GetFirstSpan()
        {
            object? startObject = _startObject;
            ...
            if ((startIndex | endIndex) >= 0)
            {
                ReadOnlyUnmanagedSpan<T> span = ((ReadOnlySequenceSegment<T>)startObject).Memory.Span;
                if (isMultiSegment)
                {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private ReadOnlyUnmanagedSpan<T> GetFirstSpanSlow(object startObject, bool isMultiSegment)
        {
            if (isMultiSegment)
                ThrowHelper.ThrowInvalidOperationException_EndPositionNotReached();
            ...
            {
                Debug.Assert(endIndex < 0);
                ReadOnlyUnmanagedSpan<T> span = (T[])startObject;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void GetFirstSpan(out ReadOnlyUnmanagedSpan<T> first, out SequencePosition next)
        {
            first = default;
            next = default;
            object? startObject = _startObject;
            ...
            if (startObject != null)
            {
                ...
                if (startIndex >= 0)
                {
                    ...
                    else
                    {
                        ...
                        first = new ReadOnlyUnmanagedSpan<T>((T[])startObject, startIndex, (endIndex & ReadOnlySequence.IndexBitMask) - startIndex);
                    }
                }
                else
                {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static ReadOnlyUnmanagedSpan<T> GetFirstSpanSlow(object startObject, int startIndex, int endIndex, bool hasMultipleSegments)
        {
            ...
            if (typeof(T) == typeof(char) && endIndex < 0)
            {
                ...
                ReadOnlyUnmanagedSpan<char> spanOfChar = ((string)startObject).AsUnmanagedSpan(startIndex & ReadOnlySequence.IndexBitMask, endIndex - startIndex);
                return MemoryMarshal.CreateReadOnlyUnmanagedSpan(ref Unsafe.As<char, T>(ref MemoryMarshal.GetReference(spanOfChar)), spanOfChar.Length);
            }
            else
            {
