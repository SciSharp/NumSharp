    ...
    public static partial class np
    {
        ...
        private static NDArray nanstd_axis(NDArray arr, int axis, bool keepdims, int ddof)
        {
            ...
            if (arr.GetTypeCode == NPTypeCode.Half)
                return nanstd_axis_half(arr, axis, keepdims, ddof);

            if (arr.GetTypeCode == NPTypeCode.Complex)
                return nanstd_axis_complex(arr, axis, keepdims, ddof);
        ...
        private static NDArray nanstd_axis_half(NDArray arr, int axis, bool keepdims, int ddof)
        {
            var inputShape = arr.shape;
            var outputShapeList = new System.Collections.Generic.List<long>();
            for (int i = 0; i < inputShape.Length; i++)
            ...
            for (long outIdx = 0; outIdx < outputSize; outIdx++)
            {
                ...
                if (count <= ddof) { result.SetHalf(Half.NaN, outCoords); continue; }
                ...
                result.SetHalf((Half)Math.Sqrt(sumSq / (count - ddof)), outCoords);
            }

            return ApplyKeepdims(result, arr.ndim, axis, outputShape, keepdims);
        }

        private static NDArray nanstd_axis_complex(NDArray arr, int axis, bool keepdims, int ddof)
        {
            var inputShape = arr.shape;
            var outputShapeList = new System.Collections.Generic.List<long>();
            for (int i = 0; i < inputShape.Length; i++)
            ...
            for (long outIdx = 0; outIdx < outputSize; outIdx++)
            {
                ...
                if (count <= ddof) { result.SetDouble(double.NaN, outCoords); continue; }
