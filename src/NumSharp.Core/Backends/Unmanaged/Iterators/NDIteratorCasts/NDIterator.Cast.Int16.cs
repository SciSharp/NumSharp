...
namespace NumSharp.Backends.Unmanaged
{
    ...
    {
        protected void setDefaults_Int16() //Int16 is the input type
        {
            if (AutoReset)
            {
            ...
            if (typeof(TOut) == typeof(Int16))
            {
                setDefaults_NoCast();
                return;
            }

            var convert = Converts.FindConverter<Int16, TOut>();
            ...
            if (Shape.IsSliced)
            {
                ...
                switch (Type)
                {
                    case IteratorType.Scalar:
                        {
                            ...
                            if (offset != 0)
                            {
                                ...
                                {
                                    hasNext.Value = false;
                                    return convert(*((Int16*)localBlock.Address + offset));
                                };
                            ...
                            else
                            {
                                ...
                                {
                                    hasNext.Value = false;
                                    return convert(*((Int16*)localBlock.Address));
                                };
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                    ...
                    case IteratorType.Vector:
                        {
                            MoveNext = () => convert(*((Int16*)localBlock.Address + shape.GetOffset(index++)));
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            Reset = () => index = 0;
                    ...
                    case IteratorType.Tensor:
                        {
                            ...
                            MoveNext = () =>
                            {
                                var ret = convert(*((Int16*)localBlock.Address + getOffset(index)));
                                iterator.Next();
                                return ret;
                            };
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
            ...
            else
            {
                ...
                switch (Type)
                {
                    ...
                    case IteratorType.Vector:
                        MoveNext = () => convert(*((Int16*)localBlock.Address + index++));
                        MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                        Reset = () => index = 0;
                        HasNext = () => index < Shape.size;
                        break;
        ...
        protected void autoresetDefault_Int16()
        {
            ...
            if (Shape.IsSliced)
            {
                ...
                switch (Type)
                {
                    case IteratorType.Scalar:
                        {
                            ...
                            if (offset != 0)
                            {
                                MoveNext = () => convert(*((Int16*)localBlock.Address + offset));
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                            else
                            {
                                MoveNext = () => convert(*((Int16*)localBlock.Address));
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                    ...
                    case IteratorType.Vector:
                        {
                            var size = Shape.size;
                            MoveNext = () => convert(*((Int16*)localBlock.Address + shape.GetOffset(index >= size ? index = 0 : index++)));
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            Reset = () => index = 0;
                            HasNext = () => true;
                            break;
            ...
            else
            {
                ...
                switch (Type)
                {
                    ...
                    case IteratorType.Vector:
                        var size = Shape.size;
                        MoveNext = () => convert(*((Int16*)localBlock.Address + (index >= size ? index = 0 : index++)));
                        MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                        Reset = () => index = 0;
                        HasNext = () => true;
                        break;
