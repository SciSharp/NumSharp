//Generated by Regex Templating Engine at 27/07/2019 11:57:30 UTC
...
namespace NumSharp.Backends.Unmanaged
{
    ...
    {
        protected void setDefaults_Boolean() //Boolean is the input type
        {
            if (AutoReset)
            {
            ...
            if (typeof(TOut) == typeof(Boolean))
            {
                setDefaults_NoCast();
                return;
            }

            var convert = Converts.FindConverter<Boolean, TOut>();
            ...
            if (Shape.IsSliced)
            {
                ...
                switch (Type)
                {
                    case IteratorType.Scalar:
                        {
                            ...
                            if (offset != 0)
                            {
                                ...
                                {
                                    hasNext.Value = false;
                                    return convert(*((Boolean*)localBlock.Address + offset));
                                };
                            ...
                            else
                            {
                                ...
                                {
                                    hasNext.Value = false;
                                    return convert(*((Boolean*)localBlock.Address));
                                };
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                    ...
                    case IteratorType.Vector:
                        {
                            MoveNext = () => convert(*((Boolean*)localBlock.Address + shape.GetOffset(index++)));
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            Reset = () => index = 0;
                    ...
                    case IteratorType.Tensor:
                        {
                            ...
                            MoveNext = () =>
                            {
                                var ret = convert(*((Boolean*)localBlock.Address + getOffset(index)));
                                iterator.Next();
                                return ret;
                            };
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
            ...
            else
            {
                ...
                switch (Type)
                {
                    ...
                    case IteratorType.Vector:
                        MoveNext = () => convert(*((Boolean*)localBlock.Address + index++));
                        MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                        Reset = () => index = 0;
                        HasNext = () => index < Shape.size;
                        break;
        ...
        protected void autoresetDefault_Boolean()
        {
            ...
            if (Shape.IsSliced)
            {
                ...
                switch (Type)
                {
                    case IteratorType.Scalar:
                        {
                            ...
                            if (offset != 0)
                            {
                                MoveNext = () => convert(*((Boolean*)localBlock.Address + offset));
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                            else
                            {
                                MoveNext = () => convert(*((Boolean*)localBlock.Address));
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                    ...
                    case IteratorType.Vector:
                        {
                            var size = Shape.size;
                            MoveNext = () => convert(*((Boolean*)localBlock.Address + shape.GetOffset(index >= size ? index = 0 : index++)));
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            Reset = () => index = 0;
                            HasNext = () => true;
                            break;
            ...
            else
            {
                ...
                switch (Type)
                {
                    ...
                    case IteratorType.Vector:
                        var size = Shape.size;
                        MoveNext = () => convert(*((Boolean*)localBlock.Address + (index >= size ? index = 0 : index++)));
                        MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                        Reset = () => index = 0;
                        HasNext = () => true;
                        break;
