	                ...
	                case NPTypeCode.Boolean:
                    {
                        ...
                        switch (InfoOf<TOut>.NPTypeCode)
                        {
	                        case NPTypeCode.Boolean:
                            {
                                ...
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToBoolean(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Byte:
                            {
                                var dst = (byte*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToByte(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Int16:
                            {
                                var dst = (short*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToInt16(*(src + i));
                                break;
                            }
	                        case NPTypeCode.UInt16:
                            {
                                var dst = (ushort*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToUInt16(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Int32:
                            {
                                var dst = (int*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToInt32(*(src + i));
                                break;
                            }
	                        case NPTypeCode.UInt32:
                            {
                                var dst = (uint*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToUInt32(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Int64:
                            {
                                var dst = (long*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToInt64(*(src + i));
                                break;
                            }
	                        case NPTypeCode.UInt64:
                            {
                                var dst = (ulong*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToUInt64(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Char:
                            {
                                var dst = (char*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToChar(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Double:
                            {
                                var dst = (double*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToDouble(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Single:
                            {
                                var dst = (float*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToSingle(*(src + i));
                                break;
                            }
	                        case NPTypeCode.Decimal:
                            {
                                var dst = (decimal*)ret.Address;
                                for (long i = 0; i < len; i++) *(dst + i) = Converts.ToDecimal(*(src + i));
                                break;
                            }
	                        default:
		                        throw new NotSupportedException();
