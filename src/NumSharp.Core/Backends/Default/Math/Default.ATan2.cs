            ...
            unsafe
            {
                switch (out_y.GetTypeCode)
                {
	                ...
	                case NPTypeCode.#1:
	                {
                        ...
                        Parallel.For(0, len, i => out_addr[i] = Converts.To#1(Math.Atan2(out_addr[i])));
                        return @out;
	                }
	                %
	                ...
	                case NPTypeCode.Int16:
	                {
                        var out_addr = (short*)out_y.Address;
                            var out_addr_x = (byte*)out_x.Address;
                            Parallel.For(0, len, i => out_addr[i] = Converts.ToInt16(Math.Atan2(out_addr[i], out_addr_x[i])));
                        return out_y;
	                }
	                case NPTypeCode.UInt16:
	                {
                        var out_addr = (ushort*)out_y.Address;
                            var out_addr_x = (byte*)out_x.Address;
                            Parallel.For(0, len, i => out_addr[i] = Converts.ToUInt16(Math.Atan2(out_addr[i], out_addr_x[i])));
                        return out_y;
	                }
	                ...
	                case NPTypeCode.UInt32:
	                {
                        var out_addr = (uint*)out_y.Address;
                            var out_addr_x = (byte*)out_x.Address;
                            Parallel.For(0, len, i => out_addr[i] = Converts.ToUInt32(Math.Atan2(out_addr[i], out_addr_x[i])));
                        return out_y;
	                }
	                ...
	                case NPTypeCode.UInt64:
	                {
                        var out_addr = (ulong*)out_y.Address;
                            var out_addr_x = (byte*)out_x.Address;
                            Parallel.For(0, len, i => out_addr[i] = Converts.ToUInt64(Math.Atan2(out_addr[i], out_addr_x[i])));
                        return out_y;
	                }
	                case NPTypeCode.Char:
	                {
                        var out_addr = (char*)out_y.Address;
                            var out_addr_x = (byte*)out_x.Address;
                            Parallel.For(0, len, i => out_addr[i] = Converts.ToChar(Math.Atan2(out_addr[i], out_addr_x[i])));
                        return out_y;
	                }
                    ...
                    case NPTypeCode.Decimal:
	                {
                        var out_addr = (decimal*)out_y.Address;
                            var out_addr_x = (byte*)out_x.Address;
                            Parallel.For(0, len, i => out_addr[i] = (DecimalEx.ATan2(out_addr[i], out_addr_x[i])));
                        return out_y;
	                }
	                default:
		                throw new NotSupportedException();
