            ...
            unsafe
            {
                switch (@out.GetTypeCode)
                {
	                ...
	                case NPTypeCode.#1:
	                {
                        ...
                        for (int i = 0; i < len; i++) out_addr[i] = Converts.To#1(Math.Ceiling(out_addr[i]));
                        return @out;
	                }
	                %
                    case NPTypeCode.Decimal:
	                {
                        ...
                        for (int i = 0; i < len; i++) out_addr[i] = (DecimalEx.Ceiling(out_addr[i]));
                        return @out;
	                }
	                default:
	                ...
	                case NPTypeCode.Double:
	                {
                        ...
                        for (int i = 0; i < len; i++) out_addr[i] = Converts.ToDouble(Math.Ceiling(out_addr[i]));
                        return @out;
	                }
	                case NPTypeCode.Single:
	                {
                        var out_addr = (float*)@out.Address;
                        for (int i = 0; i < len; i++) out_addr[i] = Converts.ToSingle(Math.Ceiling(out_addr[i]));
                        return @out;
	                }
                    case NPTypeCode.Decimal:
	                {
