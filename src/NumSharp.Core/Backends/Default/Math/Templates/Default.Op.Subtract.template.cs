        ...
        public unsafe NDArray Subtract__1__(in NDArray lhs, in NDArray rhs)
        {
            ...
            switch (rhs.GetTypeCode)
            {
                ...
                case NPTypeCode.Boolean:
                {
                    ...
                    {
                        return NDArray.Scalar((ValueType)Convert.ChangeType(*((__2__*)lhs.Address) %(op) (*((bool*)rhs.Address) ? (__2__) 1 : (__2__) 0), (TypeCode)ret_type));
                    }
                ...
                case NPTypeCode.#1:
                {
                    ...
                    if (lhs.Shape.IsScalar && rhs.Shape.IsScalar)
                    {
                        return NDArray.Scalar((ValueType)Convert.ChangeType(*((__2__*)lhs.Address) #(op) *((#2*)rhs.Address), (TypeCode)ret_type));
                    }
                    (Shape BroadcastedLeftShape, Shape BroadcastedRightShape) = DefaultEngine.Broadcast(lhs.Shape, rhs.Shape);
                    var lhs_address = (__2__*)lhs.Address;
