            ...
            unsafe
            {
                switch (@out.GetTypeCode)
                {
                    ...
                    case NPTypeCode.Decimal:
	                {
                        ...
                        Parallel.For(0, len, i => *(out_addr + i) = (DecimalEx.Log(*(out_addr + i))));
                        return @out;
	                }
	                default:
	                ...
	                case NPTypeCode.Int16:
	                {
                        ...
                        Parallel.For(0, len, i => *(out_addr + i) = Converts.ToInt16(Math.Log(*(out_addr + i))));
                        return @out;
	                }
	                case NPTypeCode.UInt16:
	                {
                        var out_addr = (ushort*)@out.Address;
                        Parallel.For(0, len, i => *(out_addr + i) = Converts.ToUInt16(Math.Log(*(out_addr + i))));
                        return @out;
	                }
	                ...
	                case NPTypeCode.UInt32:
	                {
                        ...
                        Parallel.For(0, len, i => *(out_addr + i) = Converts.ToUInt32(Math.Log(*(out_addr + i))));
                        return @out;
	                }
	                ...
	                case NPTypeCode.UInt64:
	                {
                        ...
                        Parallel.For(0, len, i => *(out_addr + i) = Converts.ToUInt64(Math.Log(*(out_addr + i))));
                        return @out;
	                }
	                case NPTypeCode.Char:
	                {
                        var out_addr = (char*)@out.Address;
                        Parallel.For(0, len, i => *(out_addr + i) = Converts.ToChar(Math.Log(*(out_addr + i))));
                        return @out;
	                }
