...
// DirectILKernelGenerator.Reduction.Axis.Simd.cs. The only difference: input tiles
    ...
    public static partial class DirectILKernelGenerator
    {
            ...
            ReductionOp op, NPTypeCode inputTc, NPTypeCode accumTc)
        {
            ...
            if (isLeadingCContig || isLeadingInnerSlab)
            {
                ...
                // For axis > 0 the outer dims are slabs of (axisSize x innerSize)
                // input and innerSize output, contiguous because the array is
                // C-contig (or has a contig inner slab). Iterate outer slabs and
                // call the per-slab kernel once per slab.
                long inputSlab = axisSize * innerSize;
                long outputSlab = innerSize;

                if (inputTc == NPTypeCode.Int32 && accumTc == NPTypeCode.Int64)
                {
                    int* ip = (int*)input;
                    long* op_ = (long*)output;
                    for (long o = 0; o < outerSize; o++)
                    {
                        AxisReductionLeadingWideningInt32ToInt64(
                            ip + o * inputSlab, op_ + o * outputSlab,
                            axisSize, innerSize, axisStrideEl, op);
                    }
                    return;
                }
