...
namespace NumSharp
{
    ...
    {
        protected void setDefaults_Half() //Half is the input type
        {
            if (AutoReset)
            {
                autoresetDefault_Half();
                return;
            }
            ...
            var convert = Converts.FindConverter<Half, TOut>();
            ...
            if (!Shape.IsContiguous || Shape.offset != 0)
            {
                ...
                switch (Type)
                {
                    case IteratorType.Scalar:
                        {
                            ...
                            if (offset != 0)
                            {
                                ...
                                {
                                    hasNext.Value = false;
                                    return convert(*((Half*)localBlock.Address + offset));
                                };
                            ...
                            else
                            {
                                ...
                                {
                                    hasNext.Value = false;
                                    return convert(*((Half*)localBlock.Address));
                                };
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                    ...
                    case IteratorType.Vector:
                        {
                            MoveNext = () => convert(*((Half*)localBlock.Address + shape.GetOffset(index++)));
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            Reset = () => index = 0;
                    ...
                    case IteratorType.Tensor:
                        {
                            ...
                            MoveNext = () =>
                            {
                                var ret = convert(*((Half*)localBlock.Address + getOffset(index)));
                                iterator.Next();
                                return ret;
                            };
                            MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
            ...
            else
            {
                ...
                switch (Type)
                {
                    ...
                    case IteratorType.Vector:
                        MoveNext = () => convert(*((Half*)localBlock.Address + index++));
                        MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                        Reset = () => index = 0;
                        HasNext = () => index < Shape.size;
                        break;
                    ...
                    case IteratorType.Tensor:
                        ...
                        MoveNext = () => convert(*((Half*)localBlock.Address + iterator.Next()));
                        MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
        ...
        protected void autoresetDefault_Half()
        {
            if (typeof(TOut) == typeof(Half))
            {
                    ...
                    case IteratorType.Scalar:
                        {
                            ...
                            if (offset != 0)
                            {
                                MoveNext = () => convert(*((Half*)localBlock.Address + offset));
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                            else
                            {
                                MoveNext = () => convert(*((Half*)localBlock.Address));
                                MoveNextReference = () => throw new NotSupportedException("Unable to return references during iteration when casting is involved.");
                            }
                    ...
                    case IteratorType.Vector:
                        {
                            ...
                            MoveNext = () =>
                            {
                                var ret = convert(*((Half*)localBlock.Address + shape.GetOffset(index++)));
                                if (index >= size)
                                    index = 0;
                                return ret;
                            };
            ...
            else
            {
                ...
                switch (Type)
                {
                    ...
                    case IteratorType.Vector:
                        ...
                        MoveNext = () =>
                        {
                            var ret = convert(*((Half*)localBlock.Address + index++));
                            if (index >= size)
                                index = 0;
                            return ret;
                        };
