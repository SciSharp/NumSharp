...
// =============================================================================
// NDIter.Execution.cs — Kernel Integration Layer (DESIGN)
// =============================================================================
//
// RATIONALE
// ---------
...
//   - NDIter produces data pointers, strides, buffers, reduction scheduling
...
// ----------------------
// 1. ForEach(NDInnerLoopFunc, auxdata)
...
//      INDInnerLoop; JIT inlines the call site. Same capability as ForEach but
...
//      NDIter.DetectExecutionPath and materializes the matching IL kernel.
...
//     buffered paths through BufStrides, which NDIterBufferManager already
//     sets to the buffer element size.
//
// Both bugs are fixable in NDIter.cs. The bridge is careful not to trip them
...
namespace NumSharp.Backends.Iteration
{
    ...
    /// <see cref="NDInnerLoopFunc"/>. Implementations should be
    /// <c>readonly struct</c>; JIT specializes <see cref="NDIterRef.ExecuteGeneric"/>
    /// per type and inlines the call.
    /// </summary>
    public unsafe interface INDInnerLoop
    {
    ...
    /// </summary>
    public unsafe interface INDReducingInnerLoop<TAccum> where TAccum : unmanaged
    {
        bool Execute(void** dataptrs, long* strides, long count, ref TAccum accumulator);
    }

    // -------------------------------------------------------------------------
    // Execution partial of NDIterRef
    // -------------------------------------------------------------------------
    ...
    public unsafe ref partial struct NDIterRef
    {
        ...
        private static void InvokeInner(
            NDInnerLoopFunc kernel, void** dataptrs, long* strides, long count,
        ...
        ///     <see cref="NDIterState.IterIndex"/> by one per call, so the
        ...
        private long ResolveInnerLoopCount()
        {
            ...
            if ((f & (uint)NDIterFlags.BUFFER) != 0)
                return (f & (uint)NDIterFlags.REDUCE) != 0
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private void ExecuteGenericSingle<TKernel>(TKernel kernel) where TKernel : struct, INDInnerLoop
        {
        ...
        private void ExecuteGenericMulti<TKernel>(TKernel kernel) where TKernel : struct, INDInnerLoop
        {
            void** dataptrs = GetDataPtrArray();
            long* byteStrides = GetInnerLoopByteStrides();
        ...
        private bool IsSingleInnerLoop()
        {
            ...
            if ((f & (uint)NDIterFlags.ONEITERATION) != 0) return true;
            // Fully coalesced to one axis + EXLOOP: whole iteration is one inner loop.
            if ((f & (uint)NDIterFlags.EXLOOP) != 0 && _state->NDim <= 1) return true;
            // Buffered (reduce) and whole iteration fits in one buffer fill.
            if ((f & (uint)NDIterFlags.BUFFER) != 0 && _state->BufIterEnd >= _state->IterEnd) return true;
            return false;
        }
        ...
        public void ExecuteUnary(UnaryOp op)
        {
            ...
            int ndim = _state->NDim;
            bool isContig = (_state->ItFlags & (uint)NDIterFlags.CONTIGUOUS) != 0;
        ...
        public TResult ExecuteReduction<TResult>(ReductionOp op) where TResult : unmanaged
        {
            ...
            uint f = _state->ItFlags;
            bool isContig = (f & (uint)NDIterFlags.CONTIGUOUS) != 0;
        ...
        /// <see cref="NDIterState.BufferedReduceAdvance"/> and
        ...
        public void BufferedReduce<TKernel, TAccum>(TKernel kernel)
            ...
            where TAccum : unmanaged
        {
            if ((_state->ItFlags & ((uint)NDIterFlags.BUFFER | (uint)NDIterFlags.REDUCE))
                != ((uint)NDIterFlags.BUFFER | (uint)NDIterFlags.REDUCE))
            {
                throw new InvalidOperationException(
        ...
        public void ExecuteCopy()
        {
            ...
            bool bothContig = (_state->ItFlags & (uint)NDIterFlags.CONTIGUOUS) != 0;
        ...
        {
            if ((_state->ItFlags & (uint)NDIterFlags.CONTIGUOUS) != 0)
        ...
        /// <see cref="NDIterState.BufStrides"/> (already bytes); for
        /// non-buffered we convert element strides.
        /// </summary>
        private long* GetInnerLoopByteStrides()
        {
            bool buffered = (_state->ItFlags & (uint)NDIterFlags.BUFFER) != 0;
            if (buffered)
        ...
        private static NPTypeCode DetermineAccumulatorType(NPTypeCode src, ReductionOp op, Type result)
        {
            ...
            // uses — so this NDIter path can never drift from it. A hand-rolled switch here had
        ...
        /// write-back happens via NDIterBufferManager.CopyFromBuffer on the
        /// WRITE operand.
        /// </summary>
        ...
        private bool BufferedWindowAdvance()
        {
            ...
            long count = NDIterBufferManager.ComputeTransferSize(ref *_state);
            NDIterBufferManager.FillBufferWindow(ref *_state, count);
            return true;
        }
        ...
        private void FlushIfBuffered()
        {
            uint f = _state->ItFlags;
            if ((f & (uint)NDIterFlags.BUFFER) != 0 &&
                (f & (uint)NDIterFlags.REDUCE) == 0 &&
                (f & (uint)NDIterFlags.DELAYBUF) == 0)
            {
                NDIterBufferManager.FlushBufferWindow(ref *_state);
            }
        }
        ...
        public NDIterState* RawState => _state;
    }
}
