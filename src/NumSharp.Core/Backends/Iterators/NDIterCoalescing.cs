...
{
    /// <summary>
    /// Axis coalescing logic for NDIter.
    ...
    /// </summary>
    public static unsafe class NDIterCoalescing
    {
        /// <summary>
        ...
        public static void CoalesceAxes(ref NDIterState state)
        {
            ...
            state.ItFlags &= ~(uint)NDIterFlags.HASMULTIINDEX;
        ...
        /// </summary>
        private static void ReverseAxes(ref NDIterState state)
        {
            var shape = state.Shape;
            var strides = state.Strides;
            var perm = state.Perm;
        ...
        public static bool FlipNegativeStrides(ref NDIterState state)
        {
            ...
            for (int axis = 0; axis < state.NDim; axis++)
            {
                ...
                if (anyNegative && allNonPositive)
                {
                    ...
                    // This allows NDIter_ResetBasePointers(baseptrs) to recompute
            ...
            if (anyFlipped)
            {
                ...
                state.ItFlags = (state.ItFlags | (uint)NDIterFlags.NEGPERM) &
                                ~(uint)NDIterFlags.IDENTPERM;
            }
