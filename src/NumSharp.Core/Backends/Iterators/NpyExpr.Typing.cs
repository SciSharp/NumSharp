...
// =============================================================================
// NpyExpr.Typing.cs — NumPy result_type pass over expression trees (Wave 6.1)
// =============================================================================
//
// Resolves every node of an NpyExpr tree to the dtype NumPy 2.x would give the
...
//
// Rules below are pinned to NumPy 2.4.2 probes (see NpyEvaluateTests):
...
// the emission methods in NpyExpr.cs (ctx.TypeOf). Legacy Compile() passes no
...
namespace NumSharp.Backends.Iteration
{
    ...
    /// </summary>
    internal enum NpyExprWeak : byte
    {
        None = 0,
        Int = 1,
        Float = 2,
    ...
    /// </summary>
    internal readonly struct NpyExprTypeInfo
    {
        public readonly NPTypeCode Code;
        public readonly NpyExprWeak Weak;

        private NpyExprTypeInfo(NPTypeCode code, NpyExprWeak weak)
        {
            Code = code;
            Weak = weak;
        }

        public bool IsWeak => Weak != NpyExprWeak.None;

        /// <summary>NEP50 default when a weak scalar meets no array: int→int64, float→float64.</summary>
        public NPTypeCode DefaultCode => Weak == NpyExprWeak.Int ? NPTypeCode.Int64 : NPTypeCode.Double;

        public static NpyExprTypeInfo Strong(NPTypeCode code) => new(code, NpyExprWeak.None);
        public static readonly NpyExprTypeInfo WeakInt = new(NPTypeCode.Int64, NpyExprWeak.Int);
        public static readonly NpyExprTypeInfo WeakFloat = new(NPTypeCode.Double, NpyExprWeak.Float);
    }
    ...
    /// every non-complex).
    /// </summary>
    internal static class NpyExprTypeRules
    {
        ...
        /// </summary>
        internal static NPTypeCode PromoteMixed(in NpyExprTypeInfo l, in NpyExprTypeInfo r)
        {
            if (!l.IsWeak && !r.IsWeak)
                return PromoteStrong(l.Code, r.Code);

            if (l.IsWeak && r.IsWeak)
                return (l.Weak == NpyExprWeak.Int && r.Weak == NpyExprWeak.Int)
                    ? NPTypeCode.Int64
                    : NPTypeCode.Double;
    ...
    public abstract partial class NpyExpr
    {
        ...
        /// </summary>
        internal abstract NpyExprTypeInfo InferType(
        ...
        /// </summary>
        internal virtual void AdoptWeakType(NPTypeCode adopted, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        ...
        private protected static NPTypeCode ResolveChild(
            NpyExpr child, in NpyExprTypeInfo info, NPTypeCode adoption,
            Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            if (!info.IsWeak)
                return info.Code;
        ...
        internal NPTypeCode ResolveNumPyTypes(
            NPTypeCode[] inputTypes, out Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            nodeTypes = new Dictionary<NpyExpr, NPTypeCode>(ReferenceEqualityComparer.Instance);
        ...
        /// </summary>
        public NpyInnerLoopFunc CompileNumPy(
            NPTypeCode[] inputTypes, out NPTypeCode resolvedType, string? cacheKey = null)
        {
            ...
            Action<ILGenerator> scalarBody = il =>
            {
                ...
                var ctx = new NpyExprCompileContext(inputTypes, resolved, scalarLocals, vectorMode: false, nodeTypes);
                EmitScalar(il, ctx);
            };
            ...
            if (wantSimd)
            {
                vectorBody = il =>
                {
                    ...
                    var ctx = new NpyExprCompileContext(inputTypes, resolved, vectorLocals, vectorMode: true, nodeTypes);
                    EmitVector(il, ctx);
                };
            }
    ...
    public sealed partial class InputNode
    {
            ...
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            ...
            return NpyExprTypeInfo.Strong(inputTypes[_index]);
        }
    }
    ...
    public sealed partial class ConstNode
    {
        internal override NpyExprTypeInfo InferType(
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
            => _isIntegerLiteral ? NpyExprTypeInfo.WeakInt : NpyExprTypeInfo.WeakFloat;

        internal override void AdoptWeakType(NPTypeCode adopted, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            if (_isIntegerLiteral && (NpyExprTypeRules.IsIntegerKind(adopted) || adopted == NPTypeCode.Boolean))
                NpyExprTypeRules.CheckIntLiteralFits(_valueInt, adopted);
    ...
    public sealed partial class BinaryNode
    {
            ...
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            ...
            var common = NpyExprTypeRules.PromoteMixed(lt, rt);
            bool intish = common == NPTypeCode.Boolean || NpyExprTypeRules.IsIntegerKind(common);
            ...
            NPTypeCode result = _op switch
            {
                ...
                BinaryOp.ATan2 when intish => NpyExprTypeRules.FloatTier(common),
            ...
            if (_op == BinaryOp.Power && NpyExprTypeRules.IsIntegerKind(result) &&
    ...
    public sealed partial class UnaryNode
    {
            ...
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            ...
            {
                result = NpyExprTypeRules.UnaryFloatResult(childType);
            }
            else if (IsPredicateResult(_op) || _op == UnaryOp.LogicalNot)
            {
                result = NPTypeCode.Boolean;
                     ...
                     !NpyExprTypeRules.IsIntegerKind(childType) && childType != NPTypeCode.Boolean)
            {
    ...
    public sealed partial class ComparisonNode
    {
            ...
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            ...
            return NpyExprTypeInfo.Strong(NPTypeCode.Boolean);
        }
    }
    ...
    public sealed partial class MinMaxNode
    {
            ...
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            ...
            return NpyExprTypeInfo.Strong(common);
        }
    }
    ...
    public sealed partial class WhereNode
    {
            ...
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            ...
            var common = NpyExprTypeRules.PromoteMixed(at, bt);
    ...
    public sealed partial class CallNode
    {
            ...
            NPTypeCode[] inputTypes, Dictionary<NpyExpr, NPTypeCode> nodeTypes)
        {
            ...
            return NpyExprTypeInfo.Strong(_returnCode);
        }
    }
}
