...
{
    public unsafe interface INDAxisSameTypeKernel<T>
        where T : unmanaged
    {
        static abstract unsafe void Execute(T* src, T* dst, long srcStride, long dstStride, long length);
    }

    public readonly struct CumSumAxisKernel<T> : INDAxisSameTypeKernel<T>
    ...
    public readonly struct CumProdAxisKernel<T> : INDAxisSameTypeKernel<T>
    ...
    public interface INDAxisDoubleReductionKernel
    {
        static abstract unsafe double Execute(double* src, long srcStride, long length, int ddof);
    }

    public readonly struct VarAxisDoubleKernel : INDAxisDoubleReductionKernel
    {
    ...
    public readonly struct StdAxisDoubleKernel : INDAxisDoubleReductionKernel
    {
        public static unsafe double Execute(double* src, long srcStride, long length, int ddof)
            => Math.Sqrt(VarAxisDoubleKernel.Execute(src, srcStride, length, ddof));
    }

    public static unsafe class NDAxisIter
    {
