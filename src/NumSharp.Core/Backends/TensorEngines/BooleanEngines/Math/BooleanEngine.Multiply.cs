...
{
    public partial class BooleanEngine
    {
        public unsafe override NDArray Multiply(in NDArray lhs, in NDArray rhs)
        {
            //if return type is scalar
            var ret_type = np._FindCommonType(lhs, rhs);
            if (lhs.Shape.IsScalar && rhs.Shape.IsScalar)
                return NDArray.Scalar((*(byte*)lhs.Address * *(byte*)rhs.Address) > 0);
            ...
            if (leftLinear && rightLinear)
            {
                ...
                if (rightshape.IsBroadcasted && rightshape.BroadcastInfo.OriginalShape.IsScalar)
                {
                    ...
                    for (int i = 0; i < ret.size; i++)
                        *(ret_address + i) = *(lhs_address + i) * rval > 0;
                }
                else if (leftshape.IsBroadcasted && leftshape.BroadcastInfo.OriginalShape.IsScalar)
                {
                    var lval = *lhs_address;
                    for (int i = 0; i < ret.size; i++)
                        *(ret_address + i) = lval * *(rhs_address + i) > 0;
                }
                else
                {
                    for (int i = 0; i < len; i++)
                        *(ret_address + i) = *(lhs_address + i) * *(rhs_address + i) > 0;
                }
            }
            else if (leftLinear)
            { // && !rightLinear
                ...
                else
                {
                    ...
                    do
                    {
                        *(ret_address + retOffset++) = *(lhs_address + leftOffset++) * *(rhs_address + rightOffset(current)) > 0;
            ...
            { // !leftLinear && 
                ...
                else
                {
                    ...
                    do
                    {
                        *(ret_address + retOffset++) = *(lhs_address + leftOffset(current)) * *(rhs_address + rightOffset++) > 0;
                    } while (incr.Next() != null);
                }
            }
            else
            {
                ...
                do
                {
                    *(ret_address + retOffset++) = *(lhs_address + leftOffset(current)) * *(rhs_address + rightOffset(current)) > 0;
                } while (incr.Next() != null);
            }
