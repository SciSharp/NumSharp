    ...
    public partial class ByteEngine
    {
        public unsafe override NDArray Subtract(in NDArray lhs, in NDArray rhs)
        {
            ...
            if (leftLinear && rightLinear)
            {
                ...
                {
                    for (int i = 0; i < len; i++)
                        *(ret_address + i) = (byte)(*(lhs_address + i) - *(rhs_address + i));
                }
            }
            else if (leftLinear)
            { // && !rightLinear
                ...
                else
                {
                    ...
                    do
                    {
                        *(ret_address + retOffset++) = (byte)(*(lhs_address + leftOffset++) - *(rhs_address + rightOffset(current)));
            ...
            { // !leftLinear && 
                ...
                else
                {
                    ...
                    do
                    {
                        *(ret_address + retOffset++) = (byte)(*(lhs_address + leftOffset(current)) - *(rhs_address + rightOffset++));
                    } while (incr.Next() != null);
                }
            }
            else
            {
                ...
                do
                {
                    *(ret_address + retOffset++) = (byte)(*(lhs_address + leftOffset(current)) - *(rhs_address + rightOffset(current)));
                } while (incr.Next() != null);
            }
