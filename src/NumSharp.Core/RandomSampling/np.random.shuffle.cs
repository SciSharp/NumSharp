...
namespace NumSharp
{
    public partial class NumPyRandom
    {
        ...
        public void shuffle(NDArray x)
        {
            ...
            // Fisher-Yates swaps write through x.Address / SwapSlicesAxis0 directly, bypassing
            // the guarded setters, so a non-writeable target (broadcast view, or a read-only
            // interop / mmap('r') array) must be rejected up front — NumPy: "array is read-only".
