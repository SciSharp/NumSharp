#if _REGEN_TEMPLATE
%template "../Equals/Default.Equals.#1.cs" for every except(supported_currently_supported, "Boolean"), except(supported_currently_supported_lowercase, "bool")
#endif
        ...
        public unsafe NDArray<bool> Equals__1__(in NDArray lhs, in NDArray rhs)
        {
            ...
            switch (rhs.GetTypeCode)
            {
                ...
                case NPTypeCode.#1:
                {
                    ...
                    do
                    {
                        *(ret_address + retShape.GetIndexInShape(current)) = (*(lhs_address + BroadcastedLeftShape.GetIndexInShape(current)) #(op) (__2__) *(rhs_address + BroadcastedRightShape.GetIndexInShape(current)));
                    } while (incr.Next() != null);
