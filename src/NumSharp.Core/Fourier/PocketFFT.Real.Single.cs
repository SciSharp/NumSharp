...
// =============================================================================
// SINGLE-PRECISION port of pocketfft's rfftp<T0> — the real mixed-radix FFTPACK
// transform, T0 == float (numpy's rfft_*_loop<npy_float> / irfft_loop<npy_float>).
// Verbatim transcription of PocketFFT.Real.cs with the arithmetic narrowed to
// float: codelet locals are float, hardcoded constants are (float)<double-literal>
// (matching pocketfft's T0(<literal>)), stored twiddles are cast to float from the
// double table, and fct is applied in float. The FFTPACK half-complex packing is
// done by PocketFFTDriver, exactly as numpy's rfft_impl/irfft_loop. Operation
// ORDER preserved verbatim for bit-parity with numpy's complex64/float32 output.
// =============================================================================

namespace NumSharp.Fourier
{
    public sealed unsafe class RfftpF
    {
        private sealed class FctData
        {
            public long fct;
            public float[] tw;   // (ip-1)*(ido-1) interleaved r,i twiddles; null for the last factor
            public float[] tws;  // 2*ip twiddles for radfg/radbg (ip>5); else null
        }
        ...
        private void CompTwiddle()
        {
            ...
            for (int k = 0; k < fact.Count; ++k)
            {
                ...
                {
                    var tw = new float[(ip - 1) * (ido - 1)];
                    for (long j = 1; j < ip; ++j)
                        for (long i = 1; i <= (ido - 1) / 2; ++i)
                        {
                            Cmplx t = twid[j * l1 * i];
                            tw[(j - 1) * (ido - 1) + 2 * i - 2] = (float)t.r;
                            tw[(j - 1) * (ido - 1) + 2 * i - 1] = (float)t.i;
                        }
                    fact[k].tw = tw;
                }
                if (ip > 5) // special factors required by *g functions
                {
                    var tws = new float[2 * ip];
                    tws[0] = 1f;
                    tws[1] = 0f;
        ...
        private static void Radf2(long ido, long l1, float* cc, float* ch, float* wa)
        {
            ...
            for (long k = 0; k < l1; k++)
                ...
                {
                    long ic = ido - i;
                    MULPM(out float tr2, out float ti2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], cc[CC(i - 1, k, 1)], cc[CC(i, k, 1)]);
        ...
        private static void Radf3(long ido, long l1, float* cc, float* ch, float* wa)
        {
            const float taur = -0.5f, taui = (float)0.8660254037844386467637231707529362;
            long WA(long x, long i) => i + x * (ido - 1);
            long CC(long a, long b, long c) => a + ido * (b + l1 * c);
            ...
            for (long k = 0; k < l1; k++)
            {
                float cr2 = cc[CC(0, k, 1)] + cc[CC(0, k, 2)];
            ...
            for (long k = 0; k < l1; k++)
                ...
                {
                    long ic = ido - i;
                    MULPM(out float dr2, out float di2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], cc[CC(i - 1, k, 1)], cc[CC(i, k, 1)]);
                    MULPM(out float dr3, out float di3, wa[WA(1, i - 2)], wa[WA(1, i - 1)], cc[CC(i - 1, k, 2)], cc[CC(i, k, 2)]);
                    Rearrange(ref dr2, ref di2, ref dr3, ref di3);
                    ch[CH(i - 1, 0, k)] = cc[CC(i - 1, k, 0)] + dr2;
                    ch[CH(i, 0, k)] = cc[CC(i, k, 0)] + di2;
                    float tr2 = cc[CC(i - 1, k, 0)] + taur * dr2;
                    float ti2 = cc[CC(i, k, 0)] + taur * di2;
                    float tr3 = taui * dr3;
                    float ti3 = taui * di3;
        ...
        private static void Radf4(long ido, long l1, float* cc, float* ch, float* wa)
        {
            const float hsqt2 = (float)0.707106781186547524400844362104849;
            long WA(long x, long i) => i + x * (ido - 1);
            long CC(long a, long b, long c) => a + ido * (b + l1 * c);
            ...
            for (long k = 0; k < l1; k++)
            {
                float tr1 = cc[CC(0, k, 3)] + cc[CC(0, k, 1)];
                ch[CH(0, 2, k)] = cc[CC(0, k, 3)] - cc[CC(0, k, 1)];
                float tr2 = cc[CC(0, k, 0)] + cc[CC(0, k, 2)];
            ...
            if ((ido & 1) == 0)
                for (long k = 0; k < l1; k++)
                {
                    float ti1 = -hsqt2 * (cc[CC(ido - 1, k, 1)] + cc[CC(ido - 1, k, 3)]);
                    float tr1 = hsqt2 * (cc[CC(ido - 1, k, 1)] - cc[CC(ido - 1, k, 3)]);
            ...
            for (long k = 0; k < l1; k++)
                ...
                {
                    long ic = ido - i;
                    MULPM(out float cr2, out float ci2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], cc[CC(i - 1, k, 1)], cc[CC(i, k, 1)]);
                    MULPM(out float cr3, out float ci3, wa[WA(1, i - 2)], wa[WA(1, i - 1)], cc[CC(i - 1, k, 2)], cc[CC(i, k, 2)]);
                    MULPM(out float cr4, out float ci4, wa[WA(2, i - 2)], wa[WA(2, i - 1)], cc[CC(i - 1, k, 3)], cc[CC(i, k, 3)]);
                    float tr1 = cr4 + cr2, tr4 = cr4 - cr2;
                    float ti1 = ci2 + ci4, ti4 = ci2 - ci4;
                    float tr2 = cc[CC(i - 1, k, 0)] + cr3, tr3 = cc[CC(i - 1, k, 0)] - cr3;
                    float ti2 = cc[CC(i, k, 0)] + ci3, ti3 = cc[CC(i, k, 0)] - ci3;
        ...
        private static void Radf5(long ido, long l1, float* cc, float* ch, float* wa)
        {
            const float tr11 = (float)0.3090169943749474241022934171828191, ti11 = (float)0.9510565162951535721164393333793821,
                        tr12 = (float)-0.8090169943749474241022934171828191, ti12 = (float)0.5877852522924731291687059546390728;
            long WA(long x, long i) => i + x * (ido - 1);
            long CC(long a, long b, long c) => a + ido * (b + l1 * c);
            ...
            for (long k = 0; k < l1; k++)
            {
                float cr2 = cc[CC(0, k, 4)] + cc[CC(0, k, 1)], ci5 = cc[CC(0, k, 4)] - cc[CC(0, k, 1)];
                float cr3 = cc[CC(0, k, 3)] + cc[CC(0, k, 2)], ci4 = cc[CC(0, k, 3)] - cc[CC(0, k, 2)];
            ...
            for (long k = 0; k < l1; ++k)
                for (long i = 2, ic = ido - 2; i < ido; i += 2, ic -= 2)
                {
                    MULPM(out float dr2, out float di2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], cc[CC(i - 1, k, 1)], cc[CC(i, k, 1)]);
                    MULPM(out float dr3, out float di3, wa[WA(1, i - 2)], wa[WA(1, i - 1)], cc[CC(i - 1, k, 2)], cc[CC(i, k, 2)]);
                    MULPM(out float dr4, out float di4, wa[WA(2, i - 2)], wa[WA(2, i - 1)], cc[CC(i - 1, k, 3)], cc[CC(i, k, 3)]);
                    MULPM(out float dr5, out float di5, wa[WA(3, i - 2)], wa[WA(3, i - 1)], cc[CC(i - 1, k, 4)], cc[CC(i, k, 4)]);
                    ...
                    float tr2 = cc[CC(i - 1, k, 0)] + tr11 * dr2 + tr12 * dr3;
                    float ti2 = cc[CC(i, k, 0)] + tr11 * di2 + tr12 * di3;
                    float tr3 = cc[CC(i - 1, k, 0)] + tr12 * dr2 + tr11 * dr3;
                    float ti3 = cc[CC(i, k, 0)] + tr12 * di2 + tr11 * di3;
        ...
        private static void Radfg(long ido, long ip, long l1, float* cc, float* ch, float* wa, float* csarr)
        {
            long cdim = ip;
            long ipph = (ip + 1) / 2;
            long idl1 = ido * l1;
            ...
            if (ido > 1)
            {
                for (long j = 1, jc = ip - 1; j < ipph; ++j, --jc)
                {
                    ...
                    for (long k = 0; k < l1; ++k)
                    {
                        ...
                        {
                            float t1 = cc[C1(i, k, j)], t2 = cc[C1(i + 1, k, j)],
            ...
            for (long j = 1, jc = ip - 1; j < ipph; ++j, --jc)
                for (long k = 0; k < l1; ++k)
                {
                    ...
                    float av = cc[C1(0, k, jc)], bv = cc[C1(0, k, j)];
                    cc[C1(0, k, jc)] = av - bv;
                    cc[C1(0, k, j)] = av + bv;
                }
        ...
        private static void Radb2(long ido, long l1, float* cc, float* ch, float* wa)
        {
            long WA(long x, long i) => i + x * (ido - 1);
            ...
            for (long k = 0; k < l1; ++k)
                for (long i = 2; i < ido; i += 2)
                {
                    ...
                    float tr2 = cc[CC(i - 1, 0, k)] - cc[CC(ic - 1, 1, k)];
                    float ti2 = cc[CC(i, 0, k)] + cc[CC(ic, 1, k)];
                    ch[CH(i, k, 0)] = cc[CC(i, 0, k)] - cc[CC(ic, 1, k)];
                    MULPM(out float m1, out float m2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], ti2, tr2);
                    ch[CH(i, k, 1)] = m1;
                    ch[CH(i - 1, k, 1)] = m2;
                }
        }

        private static void Radb3(long ido, long l1, float* cc, float* ch, float* wa)
        {
            const float taur = -0.5f, taui = (float)0.8660254037844386467637231707529362;
            long WA(long x, long i) => i + x * (ido - 1);
            ...
            for (long k = 0; k < l1; k++)
            {
                float tr2 = 2 * cc[CC(ido - 1, 1, k)];
                float cr2 = cc[CC(0, 0, k)] + taur * tr2;
                ch[CH(0, k, 0)] = cc[CC(0, 0, k)] + tr2;
                float ci3 = 2 * taui * cc[CC(0, 2, k)];
                ch[CH(0, k, 2)] = cr2 + ci3;
                ch[CH(0, k, 1)] = cr2 - ci3;
            }
            if (ido == 1) return;
            for (long k = 0; k < l1; k++)
                for (long i = 2, ic = ido - 2; i < ido; i += 2, ic -= 2)
                {
                    float tr2 = cc[CC(i - 1, 2, k)] + cc[CC(ic - 1, 1, k)];
                    float ti2 = cc[CC(i, 2, k)] - cc[CC(ic, 1, k)];
                    float cr2 = cc[CC(i - 1, 0, k)] + taur * tr2;
                    float ci2 = cc[CC(i, 0, k)] + taur * ti2;
                    ch[CH(i - 1, k, 0)] = cc[CC(i - 1, 0, k)] + tr2;
                    ch[CH(i, k, 0)] = cc[CC(i, 0, k)] + ti2;
                    float cr3 = taui * (cc[CC(i - 1, 2, k)] - cc[CC(ic - 1, 1, k)]);
                    float ci3 = taui * (cc[CC(i, 2, k)] + cc[CC(ic, 1, k)]);
                    float dr3 = cr2 + ci3, dr2 = cr2 - ci3;
                    float di2 = ci2 + cr3, di3 = ci2 - cr3;
                    MULPM(out float m1, out float m2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], di2, dr2);
                    ch[CH(i, k, 1)] = m1; ch[CH(i - 1, k, 1)] = m2;
                    MULPM(out float n1, out float n2, wa[WA(1, i - 2)], wa[WA(1, i - 1)], di3, dr3);
                    ch[CH(i, k, 2)] = n1; ch[CH(i - 1, k, 2)] = n2;
                }
        }

        private static void Radb4(long ido, long l1, float* cc, float* ch, float* wa)
        {
            const float sqrt2 = (float)1.414213562373095048801688724209698;
            long WA(long x, long i) => i + x * (ido - 1);
            ...
            for (long k = 0; k < l1; k++)
            {
                float tr2 = cc[CC(0, 0, k)] + cc[CC(ido - 1, 3, k)];
                float tr1 = cc[CC(0, 0, k)] - cc[CC(ido - 1, 3, k)];
                float tr3 = 2 * cc[CC(ido - 1, 1, k)];
                float tr4 = 2 * cc[CC(0, 2, k)];
                ch[CH(0, k, 0)] = tr2 + tr3;
                ch[CH(0, k, 2)] = tr2 - tr3;
                ch[CH(0, k, 3)] = tr1 + tr4;
                ch[CH(0, k, 1)] = tr1 - tr4;
            ...
            if ((ido & 1) == 0)
                for (long k = 0; k < l1; k++)
                {
                    float ti1 = cc[CC(0, 3, k)] + cc[CC(0, 1, k)];
                    float ti2 = cc[CC(0, 3, k)] - cc[CC(0, 1, k)];
                    float tr2 = cc[CC(ido - 1, 0, k)] + cc[CC(ido - 1, 2, k)];
                    float tr1 = cc[CC(ido - 1, 0, k)] - cc[CC(ido - 1, 2, k)];
            ...
            for (long k = 0; k < l1; ++k)
                ...
                {
                    long ic = ido - i;
                    float tr2 = cc[CC(i - 1, 0, k)] + cc[CC(ic - 1, 3, k)];
                    float tr1 = cc[CC(i - 1, 0, k)] - cc[CC(ic - 1, 3, k)];
                    float ti1 = cc[CC(i, 0, k)] + cc[CC(ic, 3, k)];
                    float ti2 = cc[CC(i, 0, k)] - cc[CC(ic, 3, k)];
                    float tr4 = cc[CC(i, 2, k)] + cc[CC(ic, 1, k)];
                    float ti3 = cc[CC(i, 2, k)] - cc[CC(ic, 1, k)];
                    float tr3 = cc[CC(i - 1, 2, k)] + cc[CC(ic - 1, 1, k)];
                    float ti4 = cc[CC(i - 1, 2, k)] - cc[CC(ic - 1, 1, k)];
                    ch[CH(i - 1, k, 0)] = tr2 + tr3;
                    float cr3 = tr2 - tr3;
                    ch[CH(i, k, 0)] = ti2 + ti3;
                    float ci3 = ti2 - ti3;
                    float cr4 = tr1 + tr4, cr2 = tr1 - tr4;
                    float ci2 = ti1 + ti4, ci4 = ti1 - ti4;
                    MULPM(out float m1, out float m2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], ci2, cr2);
                    ch[CH(i, k, 1)] = m1; ch[CH(i - 1, k, 1)] = m2;
                    MULPM(out float n1, out float n2, wa[WA(1, i - 2)], wa[WA(1, i - 1)], ci3, cr3);
                    ch[CH(i, k, 2)] = n1; ch[CH(i - 1, k, 2)] = n2;
                    MULPM(out float o1, out float o2, wa[WA(2, i - 2)], wa[WA(2, i - 1)], ci4, cr4);
                    ch[CH(i, k, 3)] = o1; ch[CH(i - 1, k, 3)] = o2;
                }
        }

        private static void Radb5(long ido, long l1, float* cc, float* ch, float* wa)
        {
            const float tr11 = (float)0.3090169943749474241022934171828191, ti11 = (float)0.9510565162951535721164393333793821,
                        tr12 = (float)-0.8090169943749474241022934171828191, ti12 = (float)0.5877852522924731291687059546390728;
            long WA(long x, long i) => i + x * (ido - 1);
            ...
            for (long k = 0; k < l1; k++)
            {
                float ti5 = cc[CC(0, 2, k)] + cc[CC(0, 2, k)];
                float ti4 = cc[CC(0, 4, k)] + cc[CC(0, 4, k)];
                float tr2 = cc[CC(ido - 1, 1, k)] + cc[CC(ido - 1, 1, k)];
                float tr3 = cc[CC(ido - 1, 3, k)] + cc[CC(ido - 1, 3, k)];
                ch[CH(0, k, 0)] = cc[CC(0, 0, k)] + tr2 + tr3;
                float cr2 = cc[CC(0, 0, k)] + tr11 * tr2 + tr12 * tr3;
                float cr3 = cc[CC(0, 0, k)] + tr12 * tr2 + tr11 * tr3;
                MULPM(out float ci5, out float ci4, ti5, ti4, ti11, ti12);
                ch[CH(0, k, 4)] = cr2 + ci5;
                ch[CH(0, k, 1)] = cr2 - ci5;
                ch[CH(0, k, 3)] = cr3 + ci4;
                ch[CH(0, k, 2)] = cr3 - ci4;
            ...
            for (long k = 0; k < l1; ++k)
                ...
                {
                    float tr2 = cc[CC(i - 1, 2, k)] + cc[CC(ic - 1, 1, k)];
                    float tr5 = cc[CC(i - 1, 2, k)] - cc[CC(ic - 1, 1, k)];
                    float ti5 = cc[CC(i, 2, k)] + cc[CC(ic, 1, k)];
                    float ti2 = cc[CC(i, 2, k)] - cc[CC(ic, 1, k)];
                    float tr3 = cc[CC(i - 1, 4, k)] + cc[CC(ic - 1, 3, k)];
                    float tr4 = cc[CC(i - 1, 4, k)] - cc[CC(ic - 1, 3, k)];
                    float ti4 = cc[CC(i, 4, k)] + cc[CC(ic, 3, k)];
                    float ti3 = cc[CC(i, 4, k)] - cc[CC(ic, 3, k)];
                    ch[CH(i - 1, k, 0)] = cc[CC(i - 1, 0, k)] + tr2 + tr3;
                    ch[CH(i, k, 0)] = cc[CC(i, 0, k)] + ti2 + ti3;
                    float cr2 = cc[CC(i - 1, 0, k)] + tr11 * tr2 + tr12 * tr3;
                    float ci2 = cc[CC(i, 0, k)] + tr11 * ti2 + tr12 * ti3;
                    float cr3 = cc[CC(i - 1, 0, k)] + tr12 * tr2 + tr11 * tr3;
                    float ci3 = cc[CC(i, 0, k)] + tr12 * ti2 + tr11 * ti3;
                    MULPM(out float cr5, out float cr4, tr5, tr4, ti11, ti12);
                    MULPM(out float ci5, out float ci4, ti5, ti4, ti11, ti12);
                    float dr4 = cr3 + ci4, dr3 = cr3 - ci4;
                    float di3 = ci3 + cr4, di4 = ci3 - cr4;
                    float dr5 = cr2 + ci5, dr2 = cr2 - ci5;
                    float di2 = ci2 + cr5, di5 = ci2 - cr5;
                    MULPM(out float m1, out float m2, wa[WA(0, i - 2)], wa[WA(0, i - 1)], di2, dr2);
                    ch[CH(i, k, 1)] = m1; ch[CH(i - 1, k, 1)] = m2;
                    MULPM(out float n1, out float n2, wa[WA(1, i - 2)], wa[WA(1, i - 1)], di3, dr3);
                    ch[CH(i, k, 2)] = n1; ch[CH(i - 1, k, 2)] = n2;
                    MULPM(out float o1, out float o2, wa[WA(2, i - 2)], wa[WA(2, i - 1)], di4, dr4);
                    ch[CH(i, k, 3)] = o1; ch[CH(i - 1, k, 3)] = o2;
                    MULPM(out float p1, out float p2, wa[WA(3, i - 2)], wa[WA(3, i - 1)], di5, dr5);
                    ch[CH(i, k, 4)] = p1; ch[CH(i - 1, k, 4)] = p2;
                }
        }

        private static void Radbg(long ido, long ip, long l1, float* cc, float* ch, float* wa, float* csarr)
        {
            long cdim = ip;
            long ipph = (ip + 1) / 2;
            long idl1 = ido * l1;
            ...
            for (long j = 1, jc = ip - 1; j < ipph; ++j, --jc)
                for (long k = 0; k < l1; ++k)
                {
                    ...
                    float a = cc[C1(0, k, j)], b = cc[C1(0, k, jc)];
                    ch[CH(0, k, jc)] = a + b;
                    ch[CH(0, k, j)] = a - b;
                }
            ...
            for (long j = 1; j < ip; ++j)
            {
                ...
                for (long k = 0; k < l1; ++k)
                {
                    ...
                    for (long i = 1; i <= ido - 2; i += 2)
                    {
                        float t1 = ch[CH(i, k, j)], t2 = ch[CH(i + 1, k, j)];
        ...
        public void Exec(float* c, float fct, bool r2hc)
        {
            ...
            int nf = fact.Count;
            var chArr = ArrayPool<float>.Shared.Rent((int)length);
            try
            {
                fixed (float* chp = chArr)
                {
                    float* p1 = c, p2 = chp;
                    if (r2hc)
                    {
                        ...
                        for (int k1 = 0; k1 < nf; ++k1)
                        {
                            ...
                            long ido = length / l1;
                            l1 /= ip;
                            fixed (float* tw = fact[k].tw, tws = fact[k].tws)
                            {
                                ...
                                else { Radfg(ido, ip, l1, p1, p2, tw, tws); float* t = p1; p1 = p2; p2 = t; }
                            }
                            { float* t = p1; p1 = p2; p2 = t; }
                        }
                    }
                    else
                    {
            ...
            {
                ArrayPool<float>.Shared.Return(chArr);
            }
        }
    }
}
