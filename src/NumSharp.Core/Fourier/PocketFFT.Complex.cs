...
{
    public sealed unsafe class Cfftp
    {
        ...
        private static void Pass7(long ido, long l1, Cmplx* cc, Cmplx* ch, Cmplx* wa, bool fwd)
        {
            ...
            for (long k = 0; k < l1; ++k)
            {
                ...
                for (long i = 1; i < ido; ++i)
                {
                    ...
                    // pocketfft PARTSTEP7 (ido>1): da=ca+cb, db=ca-cb (PM) BEFORE special_mul — matching
                    // the ido==1 branch above. (Part7 returns the raw ca/cb; the PM is applied here.)
                    var (da1, db1) = Part7(t1, t2, t3, t4, t5, t6, t7, tw1r, tw2r, tw3r, tw1i, tw2i, tw3i);
                    ch[CH(i, k, 1)] = Cmplx.SpecialMul(fwd, da1 + db1, wa[WA(0, i)]);
                    ch[CH(i, k, 6)] = Cmplx.SpecialMul(fwd, da1 - db1, wa[WA(5, i)]);
                    var (da2, db2) = Part7(t1, t2, t3, t4, t5, t6, t7, tw2r, tw3r, tw1r, tw2i, -tw3i, -tw1i);
                    ch[CH(i, k, 2)] = Cmplx.SpecialMul(fwd, da2 + db2, wa[WA(1, i)]);
                    ch[CH(i, k, 5)] = Cmplx.SpecialMul(fwd, da2 - db2, wa[WA(4, i)]);
                    var (da3, db3) = Part7(t1, t2, t3, t4, t5, t6, t7, tw3r, tw1r, tw2r, tw3i, -tw1i, tw2i);
                    ch[CH(i, k, 3)] = Cmplx.SpecialMul(fwd, da3 + db3, wa[WA(2, i)]);
                    ch[CH(i, k, 4)] = Cmplx.SpecialMul(fwd, da3 - db3, wa[WA(3, i)]);
                }
            }
        }
