// <auto-generated>
// Vendored, faithful clone of dotnet/runtime's thread-safe ConcurrentDictionary<TKey,TValue>
...
//
// Deviations from the upstream source, all mechanical:
//   * namespace System.Collections.Concurrent -> NumSharp.Collections.Concurrent;
//   * SR / ThrowHelper / HashHelpers resolve to the vendored companions in ConcurrentDictionaryInternals.cs;
    ...
    public partial class ConcurrentDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue> where TKey : notnull
    {
        ...
        internal ConcurrentDictionary(int concurrencyLevel, int capacity, bool growLockArray, IEqualityComparer<TKey>? comparer)
        {
            ...
            else
            {
                ...
                // NOTE(NumSharp vendoring): upstream special-cases string keys with a non-randomized
                // comparer that upgrades to a randomized one under hash flooding; that hardening depends on
                // the CoreLib-internal NonRandomizedStringEqualityComparer and is omitted here. The provided
                // or default comparer is used directly, so only the DoS-hardening heuristic differs.
        ...
        private bool TryAddInternal(Tables tables, TKey key, int? nullableHashcode, TValue value, bool updateIfExists, bool acquireLock, out TValue resultingValue)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    // NOTE(NumSharp vendoring): the non-randomized-string collision-rehash upgrade is
                    // omitted along with NonRandomizedStringEqualityComparer (see the constructor).
                }
                finally
                {
                    if (lockTaken)
        ...
        public IEqualityComparer<TKey> Comparer
        {
            get
            {
                ...
                // NOTE(NumSharp vendoring): no NonRandomizedStringEqualityComparer to unwrap (see the
                // constructor); the stored comparer is returned directly.
