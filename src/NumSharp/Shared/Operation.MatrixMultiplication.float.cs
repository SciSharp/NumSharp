...
namespace NumSharp.Shared
{
   ...
   {
        //start 1 
        internal static float[][] MatrixMultiplyfloatMatrix(float[][] np1, float[][]np2)
        {
            ...
            var result = new float[numOfLines][];
            result = result.Select(x => new float[numOfColumns]).ToArray();
