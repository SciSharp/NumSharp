...
namespace NumSharp.Interop.MLNet
{
    ...
    ///     softmax / argmax / top-k every classification post-step re-implements with <c>Math.Exp</c>,
    ///     <c>OrderByDescending().Take(k)</c> and a max-index loop over a <c>float[]</c> score array. Nothing here is
    ///     a new kernel — each method is a few <c>np.*</c> calls over the NDArray an ML.NET score column became
    ///     (<see cref="NDArrayMLNetInterop.ToNDArray(Microsoft.ML.IDataView, string)"/>), kept in a separate type so
    ///     the conversion verbs stay conversion-only.
    ///
    ///     <para>Dtypes follow NumPy's ufunc rules: float inputs keep their precision (a float32 score vector gives a
    ///     float32 softmax), integer inputs land on NumPy's float tier for <c>exp</c> (int8 → float16, int16 →
    ///     float32, int32+ → float64). ML.NET score columns are typically <c>VBuffer&lt;float&gt;</c>, so feed the
    ///     float32 array <c>ToNDArray</c> produces.</para>
    /// </summary>
    public static class Postprocess
    {
        ...
        /// <summary>Element-wise logistic sigmoid <c>1 / (1 + exp(-x))</c> — multi-label heads and binary-classifier scores.</summary>
        ...
        /// <summary>
        ///     Index of the maximum along <paramref name="axis"/> (int64), first occurrence on ties — the predicted
        ///     class id of a classifier, the answer span ends of a QA head.
        /// </summary>
        ...
        /// <summary>
        ///     The <paramref name="k"/> largest (or smallest) values along <paramref name="axis"/> and their indices,
        ///     sorted — descending for <paramref name="largest"/>, ascending otherwise, and on equal values <b>the
        ///     lower index comes first</b>. Output shape is the input shape with <paramref name="axis"/> reduced to
        ///     <paramref name="k"/>; indices are int64.
        /// </summary>
        ...
        /// <param name="sorted">Accepted for API parity; the result is always sorted (an unsorted result is a valid sorted one).</param>
        public static (NDArray values, NDArray indices) TopK(NDArray scores, int k, int axis = -1, bool largest = true, bool sorted = true)
        {
            ...
            if (largest)
            {
                ...
                // j -> n-1-j. Exact for every dtype, no negation overflow.
