...
| `nd.ToVBuffer<T>()` | NumSharp → ML.NET | A dense `VBuffer<T>` (a copy — `VBuffer<T>` exposes no zero-copy constructor). |
...
| `vbuffer.ToNDArray()` | ML.NET → NumSharp | A dense 1-D `NDArray` (sparse buffers are densified). |
