...
///     and from the packaged <c>build/NumSharp.Build.targets</c> for any project that installs
///     the <c>NumSharp.Build</c> NuGet package (there NDScope lives in the REFERENCED NumSharp
...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        {
            Console.Error.WriteLine($"NumSharp.Build : error NDW000: assembly not found: {assemblyPath}");
            return 2;
        }
        ...
        if (snkPath != null)
        {
            ...
            {
                Console.Error.WriteLine($"NumSharp.Build : error NDW000: strong-name key not found: {snkPath}");
                return 2;
            }
        ...
        if (refsPath != null)
        {
            ...
            {
                Console.Error.WriteLine($"NumSharp.Build : error NDW000: reference list not found: {refsPath}");
                return 2;
            }
        ...
        try
        {
            ...
            Console.Out.WriteLine(
                $"NumSharp.Build: {Path.GetFileName(assemblyPath)} — woven {result.Woven}, " +
        ...
        {
            Console.Error.WriteLine($"NumSharp.Build : error NDW099: unhandled failure weaving '{assemblyPath}': {e}");
            return 2;
        }
    }
    ...
    {
        if (problem != null)
            Console.Error.WriteLine($"NumSharp.Build : error NDW000: {problem}");
        Console.Error.WriteLine("usage: NumSharp.Build <assembly.dll> [--snk <keyfile.snk>] [--refs <list-of-reference-paths.rsp>] [--verbose]");
