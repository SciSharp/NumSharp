...
internal static class ScopeWeaver
{
    ...
    /// <summary>Which compiler state machine a <c>[NDScoped]</c> method compiled into, if any.</summary>
    private enum StateMachineKind
    {
        None,
                                            ...
                                            bool verbose, TextWriter stdout, TextWriter stderr)
    {
        ...
        {
            // Async / iterator methods compile to state machines: the attributed method is a stub
            // whose real body — and every egress — lives in the state machine's MoveNext, so THAT
            // is what gets woven (one scope per logical invocation, suspended across awaits/yields).
            var smKind = GetStateMachineKind(method, out var smType);
            if (smKind != StateMachineKind.None)
            {
                switch (WeaveStateMachineTarget(method, smType, smKind, refs, stderr))
                {
                    case ValidationOutcome.AlreadyScoped:
                        skipped++;
                        if (verbose)
                            stdout.WriteLine($"NumSharp.Weaver: skip (state machine already woven): {method.FullName}");
                        continue;
                ...
                woven++;
                if (verbose)
                    stdout.WriteLine($"NumSharp.Weaver: woven ({smKind} state machine): {method.FullName}");
                continue;
            }
            ...
            // A Task-shaped return weaves against the deferral seam, which older NumSharp
            // assemblies do not carry — demand it only now that such a target exists.
            if (Classify(method.ReturnType) == RetKind.TaskLike && !refs.HasAsyncSurface)
            {
                stderr.WriteLine(
                    $"NumSharp.Weaver : error NDW008: [NDScoped] method '{method.FullName}' returns a Task/ValueTask, " +
                    "but the referenced NumSharp does not carry the async scope seam " +
                    "(NDScope.ReturnsTask/CloseUnlessDeferred) — update the NumSharp package");
                errors++;
                continue;
            }
    ...
    private static StateMachineKind GetStateMachineKind(MethodDefinition m, out TypeDefinition smType)
    {
        smType = null;
    ...
    /// </summary>
    private static ValidationOutcome WeaveStateMachineTarget(MethodDefinition stub, TypeDefinition sm,
                                                            StateMachineKind kind, Refs refs, TextWriter stderr)
    {
        if (sm is null)
        {
            stderr.WriteLine(
                $"NumSharp.Weaver : error NDW004: [NDScoped] method '{stub.FullName}' is an async/iterator method whose " +
        ...
        {
            stderr.WriteLine(
                $"NumSharp.Weaver : error NDW004: [NDScoped] method '{stub.FullName}' has an unrecognized state-machine " +
        ...
        {
            stderr.WriteLine(
                $"NumSharp.Weaver : error NDW008: [NDScoped] async/iterator method '{stub.FullName}' needs the async " +
        ...
        foreach (var instr in moveNext.Body.Instructions)
        {
            if (instr.OpCode == OpCodes.Tail)
            {
                stderr.WriteLine($"NumSharp.Weaver : error NDW007: [NDScoped] method '{stub.FullName}' contains a tail-call");
                return ValidationOutcome.Error;
            }
        }
        ...
        if (resultSites.Count > 0)
        {
            ...
            {
                stderr.WriteLine(
                    $"NumSharp.Weaver : error NDW003: [NDScoped] async method '{stub.FullName}' produces a result of type " +
        ...
        if (kind is StateMachineKind.Iterator or StateMachineKind.AsyncIterator)
        {
            ...
            {
                stderr.WriteLine(
                    $"NumSharp.Weaver : error NDW003: [NDScoped] iterator method '{stub.FullName}' yields elements of type " +
